...
function piRpcAdapterEntry(symbol) {
  return {
    ...
    verification:
      'pi-rpc-adapter fail-closed tests, Halo Workbench runtime contract tests, and core-boundary public API budget checks',
    p0: 'local Pi RPC execution projection for the Halo Workbench Runtime',
    contractSlice: contractSlices.haloWorkbenchPiRpcExecutionSeam,
    wireImpact: false,
    rationale:
      'ADR-0072 selects one production adapter behind PiRpcPort without exposing a generic executor selector',
    exit:
      'remove only after a reviewed replacement for the ADR-0072 production adapter and equivalent Workbench seam tests',
  };
}
...
export const publicApiAllowlistRules = [
  ...
  },
  {
    ...
    reason:
      'Pi RPC adapter public API must stay limited to the selected Pi RPC Workbench execution adapter and audited extension inventory',
