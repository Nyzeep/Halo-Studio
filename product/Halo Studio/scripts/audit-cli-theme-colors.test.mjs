...
test('collectPresetColorEntriesFromJson resolves defs and light/dark variants', () => {
  const entries = collectPresetColorEntriesFromJson('halo-light.json', JSON.stringify({
    defs: {
      neutral: '#00000080',
    },
    theme: {
  ...
  assert.deepEqual(entries, [
    { file: 'halo-light.json', key: 'background', color: '#7f7f7f' },
    { file: 'halo-light.json', key: 'primary.dark', color: '#808080' },
    { file: 'halo-light.json', key: 'primary.light', color: '#7f7f7f' },
  ]);
});
...
test('checkBaseline requires budgets to be lowered when CLI color debt drops', () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'halo-cli-theme-audit-'));
  try {
