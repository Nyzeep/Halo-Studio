...
const APP_NAME: &str = "Halo";
const UNINSTALL_KEY: &str = r"Software\Microsoft\Windows\CurrentVersion\Uninstall\Halo";

/// Matches Tauri NSIS `MANUFACTURER` (`bundle.publisher`).
const TAURI_MANUFACTURER: &str = "Halo Studio Team";
/// Matches Tauri NSIS `PRODUCTNAME` (`productName`).
const TAURI_PRODUCT_NAME: &str = "Halo";
...
/// Data read from `Uninstall\Halo` (Tauri NSIS / this installer).
#[derive(Debug, Clone)]
...
pub(super) fn remove_context_menu() -> Result<()> {
    let hkcu = RegKey::predef(HKEY_CURRENT_USER);
    let _ = hkcu.delete_subkey_all(r"Software\Classes\Directory\Background\shell\Halo");
    let _ = hkcu.delete_subkey_all(r"Software\Classes\Directory\shell\Halo");
    Ok(())
}
