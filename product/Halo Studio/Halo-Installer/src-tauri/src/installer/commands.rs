...
pub(crate) fn get_default_install_path() -> String {
    ...
    base.join("Halo").to_string_lossy().to_string()
}
...
/// Detect existing Halo install (Tauri NSIS or this installer) via Add/Remove Programs registry.
#[tauri::command]
...
pub(crate) async fn start_installation(
    ...
    let result: Result<(), String> = (|| {
        ...
        // Step 4: Save first-launch language preference for Halo app.
...
/// Uninstall Halo (for the uninstaller companion).
#[tauri::command]
...
fn schedule_windows_self_uninstall_cleanup(uninstall_exe_path: &Path) -> Result<(), String> {
    ...
    let script_path = temp_dir.join(format!("halo-uninstall-{}.cmd", pid));
    let log_path = temp_dir.join(format!("halo-uninstall-cleanup-{}.log", pid));
...
set "TARGET_DIR=%~dp1"
if "%TARGET%"=="" exit /b 2
if "%LOG%"=="" set "LOG=%TEMP%\halo-uninstall-cleanup.log"
...
fn append_uninstall_runtime_log(message: &str) {
    let ts = std::time::SystemTime::now()
        ...
        .map(|d| d.as_secs())
        .unwrap_or(0);
    let log_path = std::env::temp_dir().join("halo-uninstall-runtime.log");
...
pub(crate) fn launch_application(install_path: String) -> Result<(), String> {
    let exe = if cfg!(target_os = "windows") {
        ...
        PathBuf::from(&install_path).join("Halo")
    } else {
        PathBuf::from(&install_path).join("halo")
    };
    ...
    create_windows_silent_command(&exe)
        ...
        .spawn()
        .map_err(|e| format!("Failed to launch Halo: {}", e))?;
...
pub(crate) async fn test_model_config_connection(
    ...
    let ai_client = halo_ai_adapters::AIClient::new(ai_config);
...
/// Stored `request_url` aligned with settings `resolveRequestUrl` (no halo_core).
...
/// Actual install root is always under a `Halo` directory: `{user choice}/Halo`.
/// If the user already chose a path whose last segment is `Halo`, do not append again.
fn with_halo_install_subdir(path: PathBuf) -> PathBuf {
    let already_halo = path
        .file_name()
        .and_then(|n| n.to_str())
        .map(|s| s.eq_ignore_ascii_case("Halo"))
        .unwrap_or(false);
    if already_halo {
        path
    } else {
...
fn prepare_install_target(requested_path: &Path) -> Result<PathBuf, String> {
    ...
    let install_path = with_halo_install_subdir(requested_path.to_path_buf());
    ...
    if install_path.exists() {
        ...
        if directory_has_entries(&install_path)? && !install_path.join(MAIN_APP_EXE).exists() {
            return Err(format!(
                "{}directory_must_be_empty_or_halo",
    ...
    let test_file = writable_dir.join(".halo_install_test");
...
fn ensure_app_config_path() -> Result<PathBuf, String> {
    ...
    std::fs::create_dir_all(&config_root)
        .map_err(|e| format!("Failed to create Halo config directory: {}", e))?;
