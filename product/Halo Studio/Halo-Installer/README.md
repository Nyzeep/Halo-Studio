# Halo Studio Installer

A fully custom, branded installer for Halo Studio — built with **Tauri 2 + React** for maximum UI flexibility.
...
```text
src-tauri/target/release/halo-installer.exe
```

Fast build output:

```text
src-tauri/target/release-fast/halo-installer.exe
```
...
Place the built Halo Studio application files in `src-tauri/payload/` before building the installer. The build script handles this automatically.
During `cargo build`, the payload directory is packed into an embedded zip inside `halo-installer.exe`.
...
- name: Upload Installer
  ...
  with:
    name: Halo-Installer-Exe
    path: Halo-Installer/src-tauri/target/release/halo-installer.exe
```
