...
This file applies to `Halo-Installer`. Use the top-level `AGENTS.md` for repository-wide rules.

## What matters here

`Halo-Installer` is a separate Tauri + React app, not part of the main Cargo workspace.
...
```bash
pnpm --dir Halo-Installer run installer:dev
pnpm --dir Halo-Installer run tauri:dev
pnpm --dir Halo-Installer run type-check
pnpm --dir Halo-Installer run build            # React build / CI reproduction
pnpm --dir Halo-Installer run installer:build  # packaging only
```
...
pnpm --dir Halo-Installer run type-check                            # frontend i18n/runtime
cargo check --manifest-path Halo-Installer/src-tauri/Cargo.toml      # Tauri/Rust changes
```
...
```bash
pnpm --dir Halo-Installer run type-check && pnpm --dir Halo-Installer run installer:build
```

If you modify uninstall flow, also validate the uninstall mode entry points described in `Halo-Installer/README.md`.
