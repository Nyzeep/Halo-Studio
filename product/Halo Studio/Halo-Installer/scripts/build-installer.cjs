/**
 * Halo Installer build script.
 *
 * Steps:
 * 1. Build Halo main app (optional).
...
const HALO_ROOT = path.resolve(ROOT, "..");
...
Options:
  ...
  --skip-app-build       Skip building main Halo app
...
function getCandidateAppExePaths(mode) {
  ...
  for (const profile of preferredProfiles) {
    candidates.push(
      ...
      path.join(HALO_ROOT, "target", profile, "halo-desktop.exe")
    );
  }
...
// Step 1: Build main Halo app.
if (!skipAppBuild) {
  log("Step 1: Building Halo main application...");
  run(getMainAppBuildCommand(buildMode), HALO_ROOT);
} else {
...
if (!appExePath && STRICT_PAYLOAD_VALIDATION) {
  error(
    "Could not find built Halo executable for payload. Build the desktop app first or run with --dev for local debug."
  );
}
...
if (appExePath) {
  ...
  const destExe = path.join(PAYLOAD_DIR, "halo-desktop.exe");
  ...
  if (STRICT_PAYLOAD_VALIDATION && exeSize < MIN_APP_EXE_BYTES) {
    error(
      `halo-desktop.exe in payload is unexpectedly small (${exeSize} bytes). Refusing to continue.`
    );
  }
