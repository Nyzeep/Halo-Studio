...
export function ThemeSetup({ options, setOptions, onLaunch, onClose }: ThemeSetupProps) {
  ...
  const lightPreview = findInstallerThemeById('halo-light');
  const darkPreview = findInstallerThemeById('halo-dark');
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      setFinishError(typeof err === 'string' ? err : (err as Error)?.message || 'Failed to launch Halo');
    } finally {
