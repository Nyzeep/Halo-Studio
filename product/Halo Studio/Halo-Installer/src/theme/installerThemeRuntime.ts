...
/** Same rule as main app `getSystemPreferredDefaultThemeId`: dark -> halo-dark, else halo-light. */
export function getSystemPreferredBuiltinThemeId(): ThemeId {
  if (typeof window === 'undefined' || typeof window.matchMedia !== 'function') {
    return 'halo-light';
  }
  return window.matchMedia('(prefers-color-scheme: dark)').matches ? 'halo-dark' : 'halo-light';
}
 ...
 * When preference is `system`, follows `prefers-color-scheme` like the main Halo ThemeService.
 */
