...
Hooks 让你在 Halo Studio Agent 生命周期的固定节点运行自己的命令：工具调用前后、
即将弹出权限确认时、提交提示词时、上下文压缩前后、子 Agent 启动与结束时，
以及会话与回合的开始与结束。一个 Hook 可以观察 Agent 的行为、注入模型可见的
上下文、改写工具调用参数，或者直接阻止某个动作。

## Halo Studio Hooks 就是 Codex Hooks

Halo Studio 实现的是 **Codex Hook 契约**，不是 Halo Studio 自己的方言：
...
**Codex 的 Hook 脚本可以直接在 Halo Studio 中运行，反之亦然 —— 不需要做任何适配。**
...
本文其余部分只讲 Halo Studio 特有的内容：文件放在哪、怎么打开、以及目前哪里有差异。

## Halo Studio 从哪里读取 Hooks

Codex 读 `~/.codex/hooks.json`，Halo Studio 改为读自己的配置目录。文件内部结构完全相同。

| 层级 | 路径 |
| --- | --- |
| 用户 | `<用户配置目录>/config/hooks.json` |
| 项目 | `<工作区>/.halo-studio/config/hooks.json` |

用户配置目录在 Linux 为 `~/.config/Halo Studio`，macOS 为
`~/Library/Application Support/Halo Studio`，Windows 为 `%APPDATA%\Halo Studio`。

两个层级是叠加关系：所有匹配的处理器都会执行，用户层优先，层级之间不存在覆盖或
屏蔽。修改后无需重启 Halo Studio。
...
Codex 的 `[features] hooks = false` 在 Halo Studio 没有对应项，请使用
`app.hooks.enabled`。

## 导入 Claude Code 与 Codex 命令 Hooks

Halo Studio 可以把 Claude Code 或 Codex 中兼容的 `type: "command"` Hook 保存为一份
...
2. 选择来源，并审阅每条实际命令、Windows 覆盖命令、超时、复制或外部依赖、
   跳过项以及计划指纹。
3. 确认这份确切计划。若来源在审阅后发生变化，Halo Studio 不会写入，而是返回一份
   刷新后的计划，要求再次确认。

用户来源复制到 Halo Studio 的用户托管数据；项目来源复制到 Halo Studio 项目运行区内按工作区
...
每个已导入来源都可单独启用、停用、更新或移除。移除只删除 Halo Studio 的托管副本，绝不
...
仍使用启动时捕获的快照完成。Halo Studio 不会在启动时重新导入，也不会轮询或监听 Claude
...
`reset` 只用于显式恢复损坏的 Halo Studio 托管索引，绝不会修改来源文件。
...
## Halo Studio 与 Codex 的差异

未在此列出的部分，行为与 Codex 文档所述一致。

### 不支持

| Codex 能力 | Halo Studio |
| --- | --- |
...
配置问题、非零退出、超时以及 Hook 决策都会写入 Halo Studio 后端日志。提升日志级别的
方法见 [`src/crates/LOGGING.md`](../../src/crates/LOGGING.md)。

## 相关

- CLI 的 `/hooks` 同时展示手工与导入层，异步发现受支持的外部来源，并提供上述导入
  管理操作。只有手工 Halo Studio 层需要直接编辑 `hooks.json`。
