...
Hooks let you run your own commands at fixed points in the Halo Studio Agent's
...
## Halo Studio hooks are Codex hooks

Halo Studio implements **the Codex hook contract**, not a Halo Studio dialect:
...
**A Codex hook script runs in Halo Studio unchanged, and vice versa — there is
nothing to port.**
...
The rest of this page is only what is Halo Studio-specific: where the files live,
how to switch hooks on, and where Halo Studio currently differs.

## Where Halo Studio reads hooks

Codex reads `~/.codex/hooks.json`; Halo Studio reads its own config directory
instead. Everything inside the file is identical.

| Scope | Path |
| --- | --- |
| User | `<user config dir>/config/hooks.json` |
| Project | `<workspace>/.halo-studio/config/hooks.json` |

The user config directory is `~/.config/Halo Studio` on Linux,
`~/Library/Application Support/Halo Studio` on macOS, and `%APPDATA%\Halo Studio` on
Windows.
...
Codex's `[features] hooks = false` has no Halo Studio equivalent — use
`app.hooks.enabled` instead.

## Importing Claude Code and Codex command hooks

Halo Studio can take a reviewed local snapshot of compatible `type: "command"`
...
1. Open **Settings → Agent Hooks** or run `/hooks` in the TUI. Use
   `halo hooks list` from the root CLI for a scriptable view.
2. Choose a source and review every effective command, Windows override,
   ...
   fingerprint.
3. Confirm that exact plan. If the source changed after review, Halo Studio writes
   nothing and returns a refreshed plan for another confirmation.

User sources are copied to Halo Studio's user-managed data. Project sources are
copied to workspace-isolated data under Halo Studio's project runtime area. Safe
...
Removing it deletes only Halo Studio's managed copy; it never edits Claude Code or
...
with. Halo Studio does not re-import on startup, poll, or watch Claude Code/Codex
...
```text
halo hooks list [--refresh] [--format text|json]
halo hooks import --source <source-key> [--confirm <plan-fingerprint>]
halo hooks update <import-id> [--confirm <plan-fingerprint>]
halo hooks enable <import-id>
halo hooks disable <import-id>
halo hooks remove <import-id> --confirm
halo hooks reset <user|project> --confirm
```
...
recovery for a corrupt Halo Studio-managed index and never changes source files.
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "jq -r '.tool_input.command' >> ~/halo-commands.log"
          }
        ]
      }
    ]
...
runs is appended to `~/halo-commands.log`.
...
written to the Halo Studio backend log. See
...
- CLI `/hooks` shows manual and imported layers, discovers supported external
  ...
  above. Edit `hooks.json` directly only for manual Halo Studio layers.
