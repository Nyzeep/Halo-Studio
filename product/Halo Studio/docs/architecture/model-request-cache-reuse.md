# Model Request Cache Reuse In Halo Studio
...
This note summarizes the mechanisms Halo Studio uses to improve model-side prompt
...
Halo Studio tries to keep the model-visible request prefix as stable as possible.
...
cache writes so Halo Studio can measure whether these strategies are actually
working.

## 1. System Prompt And User Context Are Cached Separately

Halo Studio does not treat the whole request prefix as one opaque string.
...
- the file is stored under the session directory in `.halo-studio/sessions/...`
...
the system prompt cache. That lets Halo Studio keep the global instruction template
...
When Halo Studio branches a session from an existing turn, it copies more than turn
text:
...
When `Task` is called with `fork_context=true`, Halo Studio:
...
Halo Studio's four shared coding modes are:
...
In other words, Halo Studio aligns backend prompt identities and frontend mode-switch
...
Halo Studio explicitly avoids baking that surface into one permanently rebuilt prompt.
...
1. On the first turn, Halo Studio saves a full skill/subagent snapshot.
...
correctness do not always want the same baseline. In those cases Halo Studio can keep
...
Halo Studio does allow tool customization at the agent/profile level:
...
Skill and subagent listings are prompt-visible descriptive surfaces, so Halo Studio
can:
...
However, Halo Studio does not currently plan to implement this.
...
Halo Studio's cache-hit strategy.
...
Halo Studio does not send compression as a detached, prompt-from-scratch request.
...
Only after that shared prefix is rebuilt does Halo Studio append the final
...
When compression is applied, Halo Studio does three important things:
...
Halo Studio also normalizes provider-specific cache telemetry so cache reuse can be
...
Halo Studio keeps them separate on purpose. That avoids overstating hit rate and
...
Halo Studio improves request cache hit rate by combining prompt splitting, session
