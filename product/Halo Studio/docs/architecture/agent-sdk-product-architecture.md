# Halo Studio Agent SDK 产品与宿主架构设计

本文定义 Halo Studio Agent SDK 的公开产品心智、与 GUI/TUI/Headless CLI/ACP/Server 的关系、内部 SDK Host
...
`agent-runtime::sdk` 是供 Halo Studio 内部入口和受控 Rust 嵌入使用的低层 Rust Runtime SDK；
`interfaces/sdk-host` 与 `apps/sdk-host` 是本地协议和独立 Host 进程的实现候选。它们都不是已经发布的
Python/TypeScript Halo Studio Agent SDK，也不能据此宣称公开 SDK 已交付。

## 1. 最终决策

Halo Studio 不选择“复制 Claude”“复制 OpenCode”或“复制 Codex”中的任一条单一路线，而是按不同问题采用各自已经验证的做法：

| 问题 | 采用的成熟做法 | Halo Studio 决策 |
|---|---|---|
...
| 本地 Runtime 如何交付 | Claude/Copilot SDK 管理匹配原生 runtime，Codex App Server 的握手与 schema 纪律 | SDK 默认管理匹配的本地 `halo-sdk-host`；用户不需要安装 CLI |
...
> Halo Studio Agent SDK 是同一 Halo Studio Agent Runtime 面向应用开发者的公开开发接口；它不是新的 Runtime，
...
flowchart LR
  Need{"你要做什么？"}
  Need -->|"人工交互"| Product["Halo Studio GUI / TUI"]
  Need -->|"shell、一次性任务、普通 CI"| CLI["halo exec"]
  Need -->|"应用、服务、IDE、复杂自动化"| SDK["Halo Studio Agent SDK"]
```
...
滚动版本的新能力不自动进入 Halo Studio 稳定承诺；发布前必须固定对比版本和 fixture。

### 2.1 产品与生态对比

| 维度 | Claude Agent SDK | OpenCode SDK / Server | Codex SDK / App Server | Copilot SDK | 对 Halo Studio 的启发 |
|---|---|---|---|---|---|
...
| 多客户端/UI | SDK 示例为主，官方不提供通用 UI SDK | TUI/Web/Desktop/IDE 共享 Server；SDK 可控制 TUI | App、CLI、SDK 分层；App Server 面向 rich client | 面向各语言应用，无统一组件层 | Halo Studio 第一方界面直接用 Runtime API，外部 UI 经开发者后端使用 SDK |
...
| Copilot | 单一 JSON-RPC 合同、多语言 conformance、协议版本范围、CLI/SDK feature matrix | 不让 CLI app 成为 Halo Studio CLI 的隐藏 SDK server；不接受不同语言安装体验长期分裂 |
| Vercel AI SDK/AI Elements | UI state/transport 分离、组件可组合、copy-source 定制 | 不把 React 组件、框架 Hook 或浏览器 transport 塞进 Agent SDK 核心包 |

### 2.3 Halo Studio 的能力基线

“至少与 Claude Agent SDK 等价”是 GA 的能力下限之一，但不是架构只能以 Claude 为中心。Halo Studio 使用三组相互独立的门槛：
...
flowchart LR
  ...
  Capability --> Halo Studio["一个 Halo Studio Agent SDK"]
  Protocol --> Halo Studio
  Customization --> Halo Studio
```

这张图表达的是决策证据来源，而不是把多个竞品 API 拼成一个超集。能力、协议和界面定制分别取证，
最终仍统一为一套 Halo Studio 语义和一个公开 SDK。

对标状态只使用以下三个词：

| 状态 | 含义 |
|---|---|
| 原生（native） | Halo Studio 现有 owner 直接提供同类公开语义 |
| 映射（translated） | 达到同一用户目标，但生命周期或命名按 Halo Studio 语义表达；差异可查询 |
| 增强（additive） | Halo Studio 特有能力；不能用来替代行业基础能力 |
...
| 层次 | 使用者 | 状态与职责 |
|---|---|---|
| Agent Runtime API | Halo Studio 各产品 adapter | 唯一 Agent loop 与应用用例边界；不是语言包 |
| Rust Runtime SDK | Halo Studio 内部入口、受控 Rust 嵌入 | 当前 preview；不等于公开产品 |
| Halo Studio Agent SDK | Python/TypeScript 应用开发者 | 一个公开产品、多个语言绑定；尚未交付 |
...
flowchart TB
  ...
  CLI["halo exec"] --> CLIA["CLI adapter"]
...
```mermaid
flowchart LR
  App["Developer application"] --> SDK["Halo Studio Agent SDK"]
  SDK -->|"manage + authenticated local channel"| Host["halo-sdk-host"]
...
| 组件 | 必须 | 禁止 |
|---|---|---|
| `halo` CLI | 依赖共享 Runtime/Application 能力 | 依赖 SDK Host app、SDK protocol 或公开语言包 |
| GUI/TUI | 依赖共享应用用例和各自平台 adapter | 经公开 SDK 绕行 Runtime；共享 renderer/protocol |
| `halo-sdk-host` | 独立组装入口，选择 SDK profile | 依赖 CLI crate；成为第二个 Server 或 Runtime |
| SDK Host adapter | 协议、能力协商、连接/Query 资源清理责任和 DTO 转换 | stdin/stdout 入口、Agent 业务状态、Tool/MCP 注册表 |
| Python/TypeScript SDK | 管理或连接匹配 Host，提供一致公开 API | 要求用户安装 `halo` CLI；暴露内部 wire DTO |
...
也不能迫使第一次使用 SDK 的用户先学习 Halo Studio 特有概念。
...
flowchart LR
  ...
  SDK["Halo Studio Agent SDK"]
  Host["Managed SDK Host"]
  Runtime["Halo Studio Agent Runtime"]
...
- BFF 指面向该界面的开发者后端（Backend for Frontend），不是 Halo Studio 新增的公共服务。
- 浏览器不直接启动或连接本机 SDK Host，不持有 Provider/MCP/Halo Studio Host 凭据。
- 开发者后端负责用户认证、租户隔离、速率限制，并把公开 SDK 事件转换为自己的前端协议。
- Halo Studio SDK 负责 Agent 能力；它不负责 React 状态、路由、主题、组件或业务认证。
...
- 一次性 `halo exec` 默认使用 Embedded Runtime；只有恢复或控制 Shared Agent Runtime 中的共享 Session 时，才使用第一方
...
但用户不应先安装或升级 `halo` CLI。连接预启动 Host 是高级模式，需双向认证、端点 owner/ACL 校验和 capability negotiation。
...
| `halo-sdk-host` | 随 SDK 平台包/wheel 或受签名下载器提供 | 不是用户 API；协议是版本化兼容边界 |
...
| 外部应用/BFF | 认证、租户、应用策略、界面状态和自定义交互 | 直连本机 Host、复制 Agent loop、保存 Halo Studio 凭据 |
...
- 不发布 Halo Studio 第一方 React UI 作为稳定 SDK ABI。
