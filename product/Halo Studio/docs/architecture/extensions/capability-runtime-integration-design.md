# Halo Studio 能力装配、导入导出与外部宿主集成设计

本文定义 Halo Studio 如何把记忆、上下文、工作流、Subagent、工具和调度策略做成可装配能力，以及这些能力如何在
不修改外部产品内核的前提下接入 OpenCode、Claude Code、Codex、Trae 等宿主。本文同时约束反向路径：外部
配置、插件和能力如何进入 Halo Studio。
...
[OpenCode 扩展兼容总览](opencode-extension-compatibility.md)。公开 Halo Studio Agent SDK 的产品心智、SDK Host 和
...
Halo Studio 采用“一个能力核心，多种宿主适配”的方向，而不是试图发布一个能直接安装到所有产品的相同插件包：

1. **Halo Studio 内部装配**：产品组装选择已编译能力、Provider/factory 和能力上限；运行时能力归属模块使用不可变静态组装结果，并维护不可变的能力版本快照。
2. **外部能力进入 Halo Studio**：生态 adapter 保留外部来源、顺序和行为，再转换成 Halo Studio 能力专属贡献。
3. **Halo Studio 能力进入外部产品**：对外能力接口暴露少量明确操作，MCP、Skill、Plugin 或 Hook adapter 再映射到具体宿主；
   这条“能力导出”路径不同于使用公开 Halo Studio Agent SDK 构建完整 Agent 应用。
...
- 不设计跨 GUI/TUI 的通用组件协议，不承诺把外部原始 UI 组件树直接运行在 Halo Studio。
...
| 生态导入 adapter | 保留单一生态的来源、格式、顺序、错误和生命周期语义 | 定义跨生态最低公分母或直接提交 Halo Studio 权威状态。 |
...
对外能力接口不等于公开 Halo Studio Agent SDK。前者只为一个外部宿主暴露当前场景需要的最小能力子集；后者通过
`AgentClient`、`client.query()`、Session、Query、只读 Turn 事实、Tool/MCP、Permission、Hook 和 Event/Result
提供完整 Agent 应用心智，并通过 SDK Host
调用同一 Agent Runtime API。外部产品只需要调用一个 Halo Studio workflow 时，不应被迫嵌入完整 Agent Runtime
或公开 SDK。

### 3.4 宿主 adapter 的产品交付生命周期

每个实际交付的外部宿主集成都必须指定一个现有产品特性或产品入口负责；在首个集成出现前，不创建跨宿主安装器、
manifest、插件商店或通用生命周期 manager。外部宿主/包管理器仍是物理安装状态的权威来源，Halo Studio 只保存自己的
...
- `disable` 先阻止 Halo Studio 发起的新调用并撤下宿主贡献；若宿主仍报告生效，状态保持需处理而非静默成功。
- `uninstall` 只清理该 adapter 拥有的包、注册项、随附本地进程、缓存和凭据引用；删除用户或宿主数据需要单独确认。
- Halo Studio 被移除、宿主升级或 adapter 崩溃后的恢复/清理入口，以及无法自动清理时的精确人工步骤。
...
- 宿主映射：Halo Studio 身份与外部 host session/thread/task/tool-use ID 的可选映射。

外部 ID 只能作为映射事实，不能取代 Halo Studio 自己的 session/turn/run 身份。一个 Claude/Codex/OpenCode session
...
- 外部宿主无法表达 Halo Studio 的并发策略时，由 Halo Studio 侧收紧，不把宿主“已接受”当成已获得本地资源。
...
同名候选在 GUI/TUI 中固定先展示 Halo Studio 来源，其余生态按稳定 `provider_id` 排序，同一生态内部沿用 adapter 的
正式来源顺序。这个顺序只用于减少阅读成本；用户未选择时仍保持冲突未决，不能把“Halo Studio 排在第一”误实现为静默激活。
...
- 外部能力进入 Halo Studio，或外部宿主调用 Halo Studio 能力时，宿主 allow/ask/deny、Hook 合并和审批顺序由 adapter 保留，
  有效权限仍不能放宽 Halo Studio 上限。Halo Studio 插件参与宿主自身 Tool/Agent 流程时，宿主负责其状态和最终权限；
  Halo Studio 只能在自己发起或执行的调用内进一步收紧，不能宣称接管宿主权限。
...
- token、模型费用和外部服务成本的单一归属，避免宿主与 Halo Studio 重复计数。
...
不是当前实现状态。下表只是 2026-07-17 核对的公开能力上限，不是 Halo Studio 已实现兼容记录；除
...
|---|---|---|
| Claude Agent SDK | Query、Session、Tool、Permission、Hook、Subagent、MCP | 已替换 Claude Code 内核，或其会话与文件系统行为等同 Halo Studio。[Overview](https://code.claude.com/docs/en/agent-sdk/overview) |
...
Halo Studio 自身公开 SDK 不直接选择上述三种形态之一：公开心智使用 Agent、Session、Turn、Tool、MCP、
...
9. **宿主状态可对账**：安装、启停、升级和卸载以宿主实际状态为准；残留 Hook、sidecar、注册项或授权必须显示为
   需处理并给出清理动作，不能只更新 Halo Studio UI 后宣称完成。
...
- 能区分外部能力进入 Halo Studio、Halo Studio 能力进入现有宿主、以及用外部 Runtime 构建新产品三种路径。
