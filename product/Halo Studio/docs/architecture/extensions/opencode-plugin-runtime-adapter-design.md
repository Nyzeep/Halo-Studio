...
本文定义 Halo Studio 如何在自己的运行时中加载和执行 OpenCode 服务插件、自定义工具和稳定钩子。总体能力状态见
...
并行构建与 Node 启动器。该分支尚未形成稳定运行时承诺，因此 Halo Studio 保持 `ScriptToolRuntime`/worker protocol
...
Halo Studio 自行实现 OpenCode 插件兼容链路，不启动完整 OpenCode Runtime：
...
- 执行进程提供 OpenCode 兼容上下文和插件接口；插件无需感知 Halo Studio Rust 内部类型。
...
这里的“自行实现兼容链路”只表示 Halo Studio 负责来源发现、依赖准备、Plugin Host 监督、OpenCode API 适配、进程通信、
Rust 能力转发和状态恢复；并不表示重写 JavaScript 引擎，也不表示在后台启动 OpenCode 的 Agent Runtime。插件工厂和钩子
在所选脚本执行后端中运行，需要 Halo Studio 能力时通过兼容接口转发给 Rust 归属模块。
...
插件执行进程是兼容载体，不是第二智能体内核。它不拥有 Halo Studio 会话状态、最终工具结果、配置文件写入或
权限存储。
...
| 开发部分 | 负责 | 不能承担 |
|---|---|---|
| OpenCode 来源适配器 + `ExternalSourceControlPlane` | 前者保留生态解析与顺序，后者维护来源身份、监听、候选快照和切换决定 | 写 Halo Studio 配置、保存凭据、决定产品提示、管理 Plugin Host 或提交贡献 |
...
| `ScriptToolRuntime` 与 services 实现 | 选择脚本执行后端，唯一持有 Plugin Host 进程树/句柄，负责物理健康探测和资源回收 | 把 Rust 侧实现命名为 Host、理解 OpenCode Hook 语义、把物理后端固化进插件 ABI 或提交 Halo Studio 业务状态 |
| Plugin Host 内的 OpenCode 适配库 | 加载真实模块，提供插件工厂上下文、`client`、`$` 和生态入口 API | 暴露 Rust 内部类型或自行修改 Halo Studio 存储 |
...
- 不因缺少 Halo Studio 专用清单或未声明静态工具集合阻止发现；风险由实际能力摘要决定，不用一个“高风险”标签
...
| 控制层 | 可执行机制 | 控制粒度 |
|---|---|---|
| 经插件兼容接口 / Tool Runtime 的 Halo Studio 能力 | 在 Rust 归属模块检查来源、凭据、文件范围、工具覆盖、配置写入、模型请求修改和界面贡献 | 可按调用和贡献细分 |
...
执行版本记录是 Halo Studio 的内部缓存和诊断信息，不是源码备份、新的插件包规范或用户确认记录，也不要求所有动态 import 在启动前
...
Halo Studio 原生 `halo.plugin.json` 包继续使用现有来源校验；OpenCode 兼容来源与原生包最终进入相同 Plugin Host
...
- 稳定版会跳过已经内置的旧认证包名。Halo Studio 只有在存在行为等价的内置替代时才应用该别名；否则不得静默
  跳过，必须继续加载或报告缺失的替代能力。
...
稳定版的软件包缓存命中后不会主动刷新 bare `latest`。Halo Studio 必须先复现这一实际加载行为；“检查更新”和更新通知
...
- Halo Studio 产品边界优先于 OpenCode 的静默覆盖顺序：同名外部 Tool、Halo Studio 内置和 MCP 候选形成包含身份与内容版本
...
默认兼容策略下，`permission.ask` 可以按照 OpenCode 语义修改 ask/deny/allow。Halo Studio 用户或组织设置了不可
突破上限时：
...
| `client` | 插件专用方法代理，经类型化进程通信映射到 Halo Studio 归属模块。 |
...
worker 可见，不能自动成为 Halo Studio 面向外部用户的 OpenCode Server 兼容承诺。

该接口只存在于 OpenCode adapter 与 Plugin Host 之间，不是独立架构层或 Halo Studio 公共 SDK，也不要求其他产品
入口使用 OpenCode 类型。
...
- OpenCode 稳定实现允许同进程插件观察 `globalThis`、进程环境或模块缓存。Halo Studio 默认共享 Plugin Host，但只保证
...
- `halo.plugin.json` 不是所有 OpenCode 插件的强制作者格式。
