...
本文定义 OpenCode 开发工具包、Server、ACP、IDE、Web、GitHub、GitLab 和 Slack 如何与 Halo Studio 对接。它们是
...
目标是复用 Halo Studio 已有会话、工具、工作区和远程能力，为真实外部调用方提供必要兼容接口，并明确原始 OpenCode
客户端能否直接连接。
...
- 不把“Halo Studio 有同类 GitHub/IDE 功能”写成“原始 OpenCode 集成可直接替换”。

## 2. 能力与产品结论

| 外部入口 | OpenCode 依赖 | Halo Studio 方案 | 兼容结论 |
|---|---|---|---|
| 插件内 `client` | 冻结版本开发工具包方法 | 提供插件专用门面并转发到 Halo Studio 归属模块 | 按方法主要适配 |
...
| IDE 扩展 | 终端启动/聚焦、上下文注入、文件引用、`/tui` endpoint | 提供 Halo Studio IDE 扩展或兼容启动器；只在明确模式开放必要 `/tui` 子集 | 主要能力可做，原扩展直连需单独验收 |
| Web / attach | 完整 Server 协议和共享会话 | 优先使用 Halo Studio Web/Remote；原始客户端直连进入 Server 兼容项目 | 当前明确降级 |
| GitHub Action / App | `opencode` CLI、事件输入、分支/PR/评论流程 | 提供 Halo Studio Action/App 和事件到任务的映射 | 提供同类产品能力，不直接运行原 Action |
| GitLab CI / Duo | runner 中的 OpenCode CLI、CI/Duo 事件与回写 | 提供 Halo Studio CI 模板/触发器和 MR/Issue 回写 | 提供同类产品能力，不冒充 OpenCode CLI |
| Slack | `@opencode-ai/slack`、Socket Mode、线程会话和 OpenCode SDK | 复用 Halo Studio 会话/消息入口实现独立 Slack 连接器 | 可做原生适配；原包直连依赖 SDK/Server 覆盖 |
...
flowchart LR
  ...
  Protocol["入口专用协议适配器"]
  Transport["Halo Studio 能力服务 / 事件投影"]
...
- 普通 Halo Studio 启动不因项目 `server` 配置改变监听地址。
...
- 对外写入方法不得返回伪成功；认证、工作区和权限判断由 Halo Studio 归属模块执行。

### 4.2 IDE

冻结版 VS Code 扩展的可执行连接契约如下，验收不能只写“启动和 `/tui` 子集”：

| 原扩展行为 | Halo Studio 适配结论 |
|---|---|
| 创建名为 `opencode` 的终端 | Halo Studio 原生扩展创建自己的终端；原扩展直连仍要求可寻址的 `opencode` 命令。 |
| 设置 `_EXTENSION_OPENCODE_PORT=<随机端口>`、`OPENCODE_CALLER=vscode` | 显式兼容启动器保留两项环境变量；普通 Halo Studio 启动不读取并冒充 OpenCode。 |
| 发送 `opencode --port <port>` | 只在用户显式安装同名兼容启动器时可直连；默认不覆盖真实 OpenCode 命令。 |
| 每 200 ms 轮询 `GET /app`，最多 10 次 | 显式兼容服务实现该健康路由和确定错误；Halo Studio 原生扩展使用自身连接状态。 |
| `POST /tui/append-prompt`，JSON `{ "text": "..." }` | 映射到当前会话输入；无活动会话时返回确定错误，不伪造成功。 |
| 文件引用 `@path#Lx` 或 `@path#Lx-Ly` | 转换到 Halo Studio 文件/行号上下文，保留工作区相对路径。 |

IDE 产品流程还应提供快速启动/聚焦、新会话、当前选择区或标签页上下文、连接状态和恢复动作。原 OpenCode
扩展只有在上述命令、环境变量、轮询和路由全部通过冻结样例时，才可标记为可直连；否则提供 Halo Studio 扩展，
...
2. 复用 OpenCode 配置兼容结果和插件能力，但任务由 Halo Studio Runtime 执行。
3. 分支、提交、PR/MR、Issue、评论或 Slack thread 由对应连接器负责，失败可重试且不重复回写。
4. 文档明确这是 Halo Studio 集成还是原始 OpenCode 包直连，不能混用兼容结论。
...
6. 原始客户端直连与 Halo Studio 原生替代分别标记，不能用同类结果代替协议兼容测试。

当前不承诺原始 OpenCode Web/attach、GitHub Action、GitLab runner 或 Slack 包直接连接 Halo Studio；这些限制需要随
