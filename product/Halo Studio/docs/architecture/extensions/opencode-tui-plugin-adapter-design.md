...
本文定义 OpenCode TUI Plugin v1 如何适配到 Halo Studio 基于 Ratatui 的终端界面。总体能力矩阵见
[`opencode-extension-compatibility.md`](opencode-extension-compatibility.md)，插件执行进程和兼容 Client 见
[`opencode-plugin-runtime-adapter-design.md`](opencode-plugin-runtime-adapter-design.md)，Halo Studio CLI/TUI 归属模块见
...
它没有 experimental 前缀，仓库称其为当前 v1 体系，但公共文档成熟度低，因此 Halo Studio 必须按 release commit
锁定样例并监控变化。

本文是目标设计。当前 Halo Studio 尚未加载 `@opencode-ai/plugin/tui` 或运行 OpenCode 的终端组件。
...
2. 将 Route、Command、Keymap、Dialog、Prompt、Theme、Attention、State、KV、Client 和 Event 映射到
   Halo Studio TUI 归属模块；Toast 必须等 CLI 建立类型化状态/通知 owner 后再映射。
...
| 类别 | 示例 | 适配判断 |
|---|---|---|
| 宿主操作和声明 | 命令、键位、导航、通知、Prompt、Theme、Attention、State、KV、Client、Event、插件启停 | 可以通过 OpenCode adapter 的插件兼容接口和 Halo Studio TUI 归属模块良好适配。 |
...
消费接口；若 Halo Studio 没有真实消费位置，则返回具体降级或不支持，不预先发布新的 GUI/TUI 公共组件契约。
...
- TUI 插件只从合并后的 `tui.json/jsonc` `plugin` 列表加载；OpenCode 不自动扫描 TUI 插件目录。Halo Studio 不把
...
插件入口完成首次激活后，Halo Studio 默认兼容策略允许 TUI 插件按 OpenCode 行为加载。用户或产品策略可以禁用终端插件、声音、系统通知、
...
稳定源码中定义了 `TuiWorkspace` 类型，但它不在 `TuiPluginApi`，Halo Studio 不把它误列为插件接口。

### 5.2 配置、元数据与生命周期

| OpenCode API | Halo Studio 映射 | 兼容边界 |
|---|---|---|
| `api.app.version` | 返回当前固定的 OpenCode TUI 兼容版本，不冒充 Halo Studio 产品版本 | 协议没有能力位；renderer 依赖必须按插件运行结果判定，见下文。 |
...
由于 `api.app.version` 没有能力位，Halo Studio 无法让插件在版本分支前识别这一差异，这是需明确确认的协议限制。

### 5.3 Route 与导航

| OpenCode 能力 | Halo Studio 映射 | 兼容程度 |
|---|---|---|
...
| Halo Studio 额外提供的结构化页面 | Markdown/列表/状态等宿主视图 | 仅属 Halo Studio 增强，不能计为现有 OpenCode 插件兼容 |
...
- `api.keys.formatSequence/formatBindings` 按 OpenCode 字符串规则实现，不使用 Halo Studio 自有显示格式猜测。
- OpenCode command、layer、binding、mode 转成 Halo Studio TUI Input 事件和可解释绑定视图。
...
| 能力 | 映射 | 降级 |
|---|---|---|
| 文本/选择/确认 Dialog | Halo Studio TUI Dialog 归属模块 | 交互结果等价，布局不逐像素一致。 |
...
如果某个插件只需要文本或状态，Halo Studio 可以额外提供结构化终端贡献接口，但这属于 Halo Studio 增强，现有 OpenCode
...
- OpenCode 主题 JSON 先按自身键和覆盖顺序解析，再映射到 Halo Studio 终端语义色。
...
- `api.tuiConfig` 和 TUI synced state 通过实时兼容视图映射，不暴露 Halo Studio 内部 reducer 对象。状态至少覆盖
...
- OpenCode TUI API 有运行时 `list`，但稳定版 CLI 没有外部插件的 uninstall/list/update 命令。Halo Studio 可以提供
  额外管理命令，但必须标为 Halo Studio 增强，不能把推测行为标为 OpenCode 等价兼容。
- Halo Studio 增强的卸载入口必须显示将修改的配置层、server/tui 入口和受影响项目；卸载先撤下对应贡献、处理
  在途调用，再移除配置引用。共享缓存只有在没有其他来源引用时才能回收。
- 显式 `pkg@version` 固定版本且不检查更新。稳定版规范称 bare `pkg` 可在缓存过期时刷新，但同一提交的实际
  `Npm.add` 在缓存命中后直接复用。Halo Studio 的兼容加载先遵循可执行源码；“检查更新/更新”单独查询 bare spec 的
...
OpenCode TUI Plugin 可以持有原始 `CliRenderer` 并注册 Solid/OpenTUI component；Halo Studio CLI 使用 Rust
Ratatui。二者在以下方面不兼容：
