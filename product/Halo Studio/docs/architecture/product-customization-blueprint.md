...
本文定义 Halo Studio 如何从同一源码构建不同产品，以及产品内置扩展与用户插件如何共存。仓库级边界以
...
经 Halo Studio 能力接口的权限/审计、插件主机故障隔离或脚本执行域的真实操作系统边界。
...
| 维度 | 产品内置扩展 | Halo Studio 原生包 | OpenCode 标准来源 |
|---|---|---|---|
| 版本 | 由产品组装结果固定版本和内容摘要 | 按现有 Halo Studio 包记录管理 | 按配置、目录、npm/file spec 和执行版本记录管理 |
...
| 冲突 | 作为 Halo Studio 候选保留并在选择界面优先展示；少量产品保护项除外 | 与其他 Halo Studio 候选一并优先展示 | 生态内按 OpenCode 顺序；跨生态同名时由用户选择，不静默覆盖 |

管理、停用和更新使用包含生态、来源类型、规范化来源地址和插件身份的来源限定运行实例身份；声明 `id` 只参与
生态识别和贡献覆盖，不能单独作为管理键。因此同名产品内置、Halo Studio 原生和 OpenCode 来源可以共存，且状态与
更新互不串用。
...
阻止产品启动。产品内置扩展仍受用户或组织的有效策略约束，产品签名不能绕过经 Halo Studio 能力接口的权限/审计、
...
- `.halo` 中可跨产品共享的项目事实必须明确列出，不能携带用户授权或插件信任。
...
6. 同名候选先展示 Halo Studio、再稳定展示其他生态；跨生态选择、保护冲突、停用、失败和恢复均可解释。
