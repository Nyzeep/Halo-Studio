# Cache-Friendly Message Structure In Halo Studio
...
This note explains the cache-friendly request shape Halo Studio tries to preserve
...
Halo Studio's model requests are intentionally assembled in a stable order:
...
a compatibility facade that re-exports the owner types from `halo-agent-runtime`.
...
- when Halo Studio rebuilds the listing baseline to the latest snapshot, it also
...
Persisted session artifacts live under `.halo-studio/sessions/{session_id}/`
