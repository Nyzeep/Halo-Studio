...
能力进入 Halo Studio 的渐进导入轨道，不代表 Halo Studio 能力导出到 OpenCode 已经完成。兼容矩阵是审计库存，不是默认路线图。
...
Skill 多来源的身份与覆盖状态展示。Halo Studio 原有
...
- 保留 OpenCode 生态内部的名称和覆盖顺序；独立 provider 之间或与 Halo Studio 本地能力同名时不得按适配器优先级静默决胜，
...
- 外部文件始终只读；不要求安装 OpenCode CLI，不复制到 Halo Studio 配置，也不写回或升级来源。
...
- 外部 Tool 与 Halo Studio 内置、MCP 或其他外部 Tool 同名时，不按 adapter 或注册顺序静默覆盖。冲突键包含全部候选
...
- OpenCode adapter 把 model 解析为字段明确的 provider 提示和模型名；Subagent 归属模块在审批前将其或固定默认项解析并绑定为
  唯一、已启用的 Halo Studio 模型。继承、歧义、缺失或已停用模型保持不可用，不使用动态回退替代明确审批。
...
- Halo Studio 模型配置变化会在后台为后续调用生成新版本；审批绑定具体配置 ID 和运行配置内容摘要，同一 ID 下的 provider、
...
- 当前 Remote 工作区明确不支持外部来源发现与决策，不读取本机同名配置；静态 system prompt 未修改，只有审批后
  的通用 AgentInfo 动态视图进入现有可用 agent 上下文，且该视图使用 Halo Studio 自有的稳定描述，不注入来源 catalog 文案。

选择与执行仍由现有会话/Subagent owner 决定；adapter 不能替换 Halo Studio Agent Kernel。外部 agent 当前只支持新建的
...
- `NodeScriptToolRuntime` 在共享外部 Tool 运行时首次初始化时解析 Node.js 可执行文件，并在当前进程内持有成功结果；
  ...
  探测通过前，运行时状态不得写成“可用”。
- Subagent owner 读取 Halo Studio 模型配置失败时必须拒绝启动，并生成与“模型不存在”不同的通用诊断。GUI/TUI
  提示用户先确认 Halo Studio 模型设置能够正常读取和保存，再刷新；日志只保留经过脱敏的失败阶段和错误类别。不得用 `AIConfig::default()`
...
- 项目根保持 `.halo`、`.claude`、`.codex`、`.cursor`、`.opencode`、`.agents` 的现有顺序；项目根整体先于
  用户根。用户根、Halo Studio 用户目录、Halo Studio 内置目录、OpenCode config/home 延迟根继续沿用当前实现顺序。
- “Halo Studio 优先”只适用于当前已经如此定义的项目级 `.halo-studio/skills`，不得误写成所有 Halo Studio 用户或内置 Skill
  都高于其他生态。PR4 用精确顺序测试固定这一事实，来源元数据不参与排序或决胜。
- `source_slot` 继续标识具体发现槽位；新增开放的 `source_id` 和稳定产品名只负责把多个槽位归为 Halo Studio、
...
Command、Tool、Subagent、MCP 以及未来可执行扩展默认把 Halo Studio 原生/内置实现作为安全候选，但不允许外部候选
通过注册顺序静默覆盖。出现同名参与者时由用户选择；选择绑定参与者集合与行为版本，集合或行为更新后才重新询问。
冲突选择列表固定先展示 Halo Studio 候选，其余生态按稳定 `provider_id` 排序，同一生态内部保留该 adapter 的正式来源顺序；
展示顺序只帮助用户理解，不代替选择，也不把 Skill 的固定根优先级复制到可执行扩展。
被 Halo Studio/其他候选覆盖、被用户拒绝或尚未选择的外部项必须继续出现在统一管理入口，并以“已覆盖”“未启用”或
...
- 三类 owner 继续维护各自 DTO、冲突内容摘要和选择持久化，主体逻辑不按 OpenCode 等生态 ID 分支。候选列表由 owner 输出；
  存在 Halo Studio 原生/本地候选时固定在首位，其余外部候选使用 adapter 已确定的稳定顺序。Skill Registry 的发现顺序和覆盖
...
TUI 命令命名属于产品契约。新增命令前必须先核对 Halo Studio 既有入口以及至少一个同类竞品的用户可见命令；能力已有对应
