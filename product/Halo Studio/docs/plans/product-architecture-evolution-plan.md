...
| 公开面 | `halo-core` 仍有迁移期 re-export；CLI 主会话客户端已仅消费 Runtime SDK，其他产品入口仍保留兼容路径 | 按入口逐项迁移，不做全仓逐 symbol 台账或批量删除 |
...
| 入口迁移 | CLI 已消费 Runtime Parts；Desktop 主交互消费由现有 owner 构造的窄口径 Runtime SDK 门面，完整 Desktop Runtime Parts 尚未组装；CLI/ACP/Desktop 仍按需保留 `halo-core/product-full` 兼容 owner | 保持单一 owner，按真实端口逐项迁移，不批量删除兼容门面或用桩服务提前声明能力 |
...
当前宿主没有可靠的刷新前后证据，因此不主动建议或触发重启。读取 Halo Studio 模型配置失败时阻止外部 Subagent 激活并显示独立原因，不能伪装成
...
本工作流不与前五项绑定成一次性交付。启动条件是某项现有 Halo Studio 能力已经有清楚 owner、稳定调用路径，以及
...
3. 为该宿主冻结分发单元、注册作用域，以及 install/register、enable、disable、uninstall、升级和失败恢复语义；
   物理安装状态以宿主为准，Halo Studio 不伪造成功。
4. 证明身份映射、权限上限、取消、Generation、事件损失、成本归属和降级后，再评估第二个宿主或第二项能力。
5. 只有非 `halo-core` 嵌入方能在最小依赖下稳定运行，才冻结和发布 Agent Runtime SDK 子接口。
