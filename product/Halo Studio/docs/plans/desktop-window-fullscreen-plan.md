...
**Goal:** Add OS-level fullscreen support for the Halo Studio Desktop main window without changing maximize, panel fullscreen, CLI/TUI rendering, or product runtime logic.
...
**Tech Stack:** Tauri v2 window APIs, React hooks, Halo Studio `ShortcutManager` where appropriate, Vitest for pure shortcut helper coverage, existing locale JSON files.
...
- Windows/Linux: pressing `F11` asks the operating system to put the whole Halo Studio Desktop window into fullscreen.
- macOS: pressing `Control+Command+F` uses the platform fullscreen convention.
- The Halo Studio internal layout remains the same: NavBar, SceneBar, panels, chat, editor, terminal, browser, and diff surfaces continue to render.
