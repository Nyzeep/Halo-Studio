...
请求携带的项目、信任、策略和工具目录版本只描述 Halo Studio 原生包的当时状态；OpenCode 的来源、执行、
...
- 已抽取 `halo-core-types`、`halo-events`、`halo-runtime-ports`、`halo-agent-stream` 等基础契约；LSP protocol DTO 和 plugin manifest DTO 已进入 `halo-core-types`。
- 已建立 `halo-services-core`、`halo-services-integrations`、`halo-agent-tools`、`tool-runtime`、`halo-tool-packs`、`halo-agent-runtime`、`halo-runtime-services`、`halo-harness`、`halo-product-domains`、`halo-product-capabilities` 等归属 crate。
...
- `halo-events` 已承接 backend event DTO、agentic event DTO、framework-neutral Agentic frontend event projection 和 platform-neutral `EventEmitter` trait；当前 Tauri transport 只负责 delivery。
...
- `halo-core` 的 function-agent AI concrete acquisition 已从旧 `runtime_services` 路径收拢到明确的 core port adapter；Git / AI compatibility re-export 仍保留旧 public path。
...
- Public API、Tool ABI 和事件转换的基础流程已建立：Agent Runtime SDK 只暴露 preview 接口；`halo-agent-tools` 提供已准备快照、默认副作用信息和旧调用拒绝；`halo-events` 提供 Desktop 与 Peer Host 共用的事件视图；Core 兼容路径不再重复映射字段，Tauri adapter 只负责交付。

## 4. 适配器边界与后续专项

- `halo-core` 仍承载兼容接口 / `product-full` 组装和少量迁移期适配器；不应继续新增归属逻辑。
...
- H1 剩余 owner 决策已迁出：dialog start route / outcome lifecycle 继续由 `agent-runtime` 给出可测试决策，tool pipeline 的 Task batch 策略由 `tool-execution` 持有，prompt runtime / workspace / user-context 组合由 `agent-runtime` 持有，AI model selector / cache-key 解析由 `halo-ai-adapters` 持有。`halo-core` 仍只保留 coordinator 调用、config IO、credential overlay、prompt 事实收集和 prompt-cache persistence IO 等 concrete adapter。
...
- Skill registry 主体 owner 已收口到 `agent-runtime`：`halo-core` 保留本地/远端扫描、config/registry IO、缓存和加载错误映射；内置 skill 分组、root/slot/key 事实、mode default/override、visible resolution、shadow 标记、mode skill info 和加载后 assistant payload 由 runtime 统一给出。
- Workspace runtime provider owner 已继续外移：本地 workspace FS/shell 实现由 `halo-services-core` 的 `workspace-runtime` feature 持有，远端 SSH workspace FS/shell adapter 由 `halo-services-integrations::remote_ssh` 持有；`halo-core::agentic::workspace` 保留 binding、session storage 解析和旧路径 re-export。
- Review-platform 主体 owner 已从 `halo-core` 外移到 `services-integrations`：provider detection、remote discovery、token store、provider DTO mapping、pagination、CI log extraction 和 GitHub / GitLab / GitCode HTTP/Git 集成由 `review-platform` feature 持有；core 只保留旧 public path、产品 data-dir 注入和 remote SSH unsupported 分类。
