...
2. Docker image rebuild fails with `cannot find db/admin in halo_relay_service`.
...
2. Dockerfile dependency-cache cleanup uses `deps/halo_relay_service*`, which does not match Cargo artifacts `libhalo_relay_service-*`; the second build links the empty placeholder crate.

## Design

### Docker

In `src/apps/relay-server/Dockerfile`, invalidate placeholder artifacts with globs `*halo_relay_service*`, `*halo_relay_server*`, `*relay_admin*`, remove matching `.fingerprint` dirs, and `touch` real sources before the second `cargo build`.
...
- Focused Rust check for `halo-services-integrations`
- Web type-check
