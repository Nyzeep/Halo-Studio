...
Skip GC when `HALO_TARGET_GC=0`. Dry-run when `HALO_TARGET_GC_DRY_RUN=1`.
...
- Never delete the profile root or final binaries by name (`halo-desktop`, `.app`, etc.) except via normal cargo replacement.
