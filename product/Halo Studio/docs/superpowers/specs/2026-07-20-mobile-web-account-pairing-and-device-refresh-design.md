...
2. 扫码配对的「用户 ID」原用于 URL 防盗用；桌面已登录 Halo Studio 账号时，应改为校验与桌面相同的账号密码，且必须是被扫码桌面当前账号。
...
- `cargo test -p halo-services-integrations --features remote-connect --test remote_connect_contracts`（不带 `--features remote-connect` 时整个测试文件被 cfg 门掉，0 测试静默通过）
