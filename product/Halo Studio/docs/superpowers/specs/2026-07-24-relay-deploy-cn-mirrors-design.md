...
1. `HALO_MIRROR=cn|global` or flags `--cn-mirror` / `--global-mirror`
2. Auto (`HALO_MIRROR=auto` default):
...
|---|---|---|
| Docker Hub registry-mirrors | `https://docker.1ms.run`, `https://dockerproxy.net`, `https://docker.m.daocloud.io` | `HALO_DOCKER_REGISTRY_MIRRORS` (space/comma separated) |
| Debian/Ubuntu apt | `mirrors.aliyun.com` | `HALO_APT_MIRROR` |
| RHEL/CentOS yum/dnf docker-ce | Aliyun docker-ce | same family |
| Relay Docker build Cargo / crates.io | `sparse+https://rsproxy.cn/index/` | `HALO_CARGO_SPARSE_URL` |
| Rustup (host, if used) | `https://rsproxy.cn` | `HALO_RUSTUP_DIST_SERVER` |
| GitHub git / tarball | `https://ghfast.top/` prefix | `HALO_GITHUB_PROXY` |
| Docker Engine install | Aliyun docker-ce packages; fallback proxied `get.docker.com` | `HALO_DOCKER_INSTALL_URL` |
...
- Idempotent apply with backups under `/etc/halo-studio/mirror-backup-*` and
  `$HOME/.halo-studio/mirror-backup-*`
...
detect mode → if cn: apply host mirrors → export build/env vars
            ...
            → deploy.sh passes HALO_USE_CN_MIRROR=1 build-args
            → Dockerfile rewrites apt + writes build-local cargo config
```

## Persistence / merge rules

- **apt:** backup `sources.list` (+ `sources.list.d` halo file); write Halo Studio-owned
  `sources.list.d/halo-cn-mirror.list` when possible; otherwise rewrite
...
- Marker file: `$HOME/.halo-studio/mirror-mode` = `cn|global` for logs/idempotency.
- **Rollback:** `HALO_MIRROR=global` removes the Halo Studio apt list, restores
  source files renamed with `.halo-disabled`, removes only Docker mirrors
  recorded as Halo Studio additions, and cleans the legacy managed Cargo block from
  early deployments.
...
- `HALO_MIRROR=global` skips CN writes and rolls back Halo Studio-owned host
  mirror changes even on a China IP.

## Verification

- `bash -n` on `mirror.sh` / `deploy.sh`
- `cargo test -p halo-services-integrations --features remote-ssh` (include
  embedded mirror script + existing deploy tests)
- `cargo check -p halo-desktop` when orchestration wiring changes
