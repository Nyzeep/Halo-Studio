...
**Goal:** Make Halo Studio CLI a full Peer Device Mode host so Desktop A can control CLI B via HostInvoke / DeviceEvent.
...
**Tech Stack:** Rust (`halo-cli`), `halo-core` services, existing account relay WS.
...
- [x] `cargo check -p halo-cli` (clean)
- [x] `cargo test -p halo-cli peer_host`
- [x] Enter peer host via chat `/login` (same Auth Server / Username / Password
  + `~/.halo-studio` session/hint as Desktop). No separate `peer-host` CLI subcommand.
