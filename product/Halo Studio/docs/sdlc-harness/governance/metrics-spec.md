...
- Harness 主动配置未解决率上升时，需要区分恶意/未知配置、实现缺口和用户主动禁用。OpenCode/MCP 等外部
  owner 的 `policy-limited`、`denied` 或 `unavailable` 诊断单独统计，不进入该指标分母。
...
| PR 就绪度采用率 | 使用就绪度摘要的 PR 数 / Halo Studio 准备的 PR 总数 | 变更就绪度 | `readiness.generated`、PR 视图 | 每周 | P2+ 衡量 PR/CI 适配接入后的 PR 体验价值 |
