...
| [OpenCode Plugins](https://opencode.ai/docs/plugins/) / [SDK](https://opencode.ai/docs/sdk/) / [Server API](https://opencode.ai/docs/server/) | JS/TS plugin、hook、自定义工具、SSE 事件流、`opencode.json`、项目/全局插件目录、npm 插件缓存 | 可提供兼容层，但底层必须由 Halo Studio 自己的权限、策略、事件模型和插件来源事实约束 |
| [Codex config](https://developers.openai.com/codex/config-basic) / [MCP](https://developers.openai.com/codex/mcp) / [skills](https://developers.openai.com/codex/skills) / [AGENTS.md](https://developers.openai.com/codex/guides/agents-md) | user/project config、MCP 配置、AGENTS.md 层级、skills 工作流、plugin 分发单元 | 兼容层应区分配置层级、指令文件、skill 能力和 plugin 分发；Halo Studio 不应要求 Codex CLI/App 安装后才能读取可迁移事实 |
...
| 研究/基准 | 信号 | 设计启发 |
|---|---|---|
| [SWE-bench](https://github.com/swe-bench/SWE-bench) | 真实 GitHub issue 正成为代码 Agent 评测基础 | Halo Studio 需要真实 issue 黄金集和长期回归集 |
...
| [Agentic AI in the SDLC](https://arxiv.org/abs/2604.26275) | Agentic SDLC 需要从架构、证据、生产力和治理同时评估 | Halo Studio 可扩展到 SDLC，但必须以产品体验和可验证价值逐步推进 |
...
| [CDEvents](https://cdevents.dev/docs/primer/) | CI/CD 事件强调声明式、松耦合、跨工具互操作 | Halo Studio 生命周期事件应保持规范事实和松耦合互操作 |
