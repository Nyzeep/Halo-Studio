...
|---|---|---|---|---|---|
| 默认低摩擦 | PRD-01、PRD-02 | 快速路径轻量、用户视图层 | 配置化策略画像、项目画像 | P0 | 质量保障要求较低的探索性改动、无配置项目、文档改动任务回放；统计首次有用动作耗时和低风险任务完成率 |
| 内部策略不外露 | PRD-03、PRD-13 | 用户视图层、用户语言稳定 | 配置化策略画像 | P-1、P0 | 检查桌面、命令行、远程、PR 摘要是否只展示任务状态、原因和下一步，不展示内部枚举 |
...
| 团队配置治理 | PRD-05、PRD-06 | 配置层级、组织策略优先级 | 配置化策略画像、PR 门禁 | P2 | `.halo`、AGENTS、CODEOWNERS、CI、路径规则冲突用例；验证来源、优先级和覆盖限制 |
...
| Review 状态与记录独立 | PRD-21、PRD-22 | [Review 生命周期](../architecture/review-lifecycle.md)、[Review 执行架构](../architecture/deep-review.md) | 覆盖范围/目标新鲜度（coverage/freshness）、问题状态、用户处置和记录级操作 | P2 | `limited` 与 `stale` 可同时保留；问题观察与覆盖相互独立；“本轮未观察到”不等于用户已解决/忽略；验证问题处置继承、记录级归档/删除、断链只读和远端持久化 |
...
| 关键用例体验 | PRD-18 | 用户视图层、页面/面板布局、跨入口语义一致 | 产品需求、配置化策略画像、质量数据面 | P0、P1、P2 | P0/P1 回放质量保障要求较低的探索性改动、文档改动、安全确认、远程容器和 Harness 主动配置发现；P2 回放 PR 就绪和受管路径 |
| 产品指标完整流程 | PRD-10 | 质量数据面、评测与学习 | 指标规格、智能体评测 | P0-P4 | 指标采样回放、策略 A/B、保留集；验证速度、打断、安全、质量和成本联合解释 |
...
| [../architecture/review-lifecycle.md](../architecture/review-lifecycle.md) | Review 记录、修订、结果展示、问题连续性、持久化查询和复审边界 | 不替代 Review 目标证据、执行器、修复动作或通用数据面 |
