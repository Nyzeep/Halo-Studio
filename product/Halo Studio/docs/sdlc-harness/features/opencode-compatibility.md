...
> [插件运行时与 Plugin Host](../../architecture/extensions/plugin-runtime-design.md)

本文只说明 SDLC Harness 如何消费 Halo Studio 的 OpenCode 扩展能力，不另建一套插件格式、运行时或权限体系。
OpenCode 配置、工具、服务插件、Hook 和终端插件的完整范围、可实现性与降级项，以扩展兼容总览及其细分设计为准。

本文描述对应 OC-R 阶段完成后的目标消费方式。当前 P0 只能消费来源事实和静态诊断，不能消费真实 OpenCode
插件工具、Hook、Client、终端贡献或运行状态；以下能力只有在其 owner 形成生产完整流程后才对 Harness 可用。
...
这意味着 Harness 只依赖 Halo Studio 的稳定工具、事件、权限和诊断接口，不读取 JS 进程句柄、OpenCode 原始载荷或
插件运行时内部状态。
...
OpenCode Hook 是可变换接口，不应统一降级为“建议”或“只读候选”。适配器负责顺序和参数转换，对应 Halo Studio
...
flowchart LR
  ...
  Client["PluginRuntimeClient"] <--> Adapter
  Adapter <--> Runtime["ScriptToolRuntime / services 实现"]
  Runtime <--> PluginHost["Plugin Host 子进程"]
  Client --> Tools["工具归属模块"]
  Client --> Events["事件归属模块"]
  Client --> Permission["权限归属模块"]
...
兼容进程级事实相同的外部插件默认共享 Plugin Host；workspace、session 和插件数量不决定进程数量。Harness 发起
的调用仍使用统一的期限、取消、有界队列、大小限制、响应校验和崩溃回收路径，不增加第二层进程管理。
...
- 用户可以显式导入支持的配置字段到 Halo Studio；导入前显示直接使用、需要转换和会降级的项目。
- 插件或配置变化先在后台准备候选版本；来源仍启用、健康旧进程仍符合当前策略时，代码或依赖更新准备失败可
  继续使用旧进程。旧进程丢失后只有内容摘要匹配的完整旧版本文件仍可校验时才能重建；否则明确“上一版本不可恢复”。停用、
  删除、来源撤销、权限收紧或安全策略失效必须停止并确认共享 Host 进程树退出，再撤下旧贡献并只恢复仍合规的插件，不能为不中断评审而回退。
- 新增或删除工具、Hook、权限或依赖时，在任务启动前显示简短差异；不在每次工具调用中重复弹窗。
- Harness 报告只记录实际使用的插件来源、执行版本和降级结果，不复制整份插件配置。

## 5. 安全与可靠性

本地默认兼容策略允许插件使用当前用户通常可用的文件、网络、进程和环境能力。经 Halo Studio 能力接口的调用可以
细分收紧；脚本直接能力只能由真实 OS/容器边界粗粒度限制，无法落实时停用相应插件。策略限制必须与插件
代码错误分开显示。

无论权限是否开放，下列可靠性保护始终生效：

- Rust 主应用与 Plugin Host 的进程隔离；同一 Host 内插件不承诺彼此隔离；
- 初始化、Hook、工具和清理期限；
- 取消传播、有界队列、输入/输出大小限制；
- 无效响应丢弃、迟到响应不再提交；
- 有限次数恢复和重复错误聚合；更新失败只保留仍健康且合规的旧进程，或从摘要匹配的完整旧版本文件重建；
- 凭据和敏感环境变量不进入普通日志或报告。

未知配置字段保留并诊断，不阻断其他有效字段；未知事件按服务 v1/TUI v2 的固定规则局部降级。未知 Client 读接口
...
3. Hook 的顺序、变换、异常和最终提交由固定 OpenCode 样例验证。
4. 插件故障、策略限制和 Harness 业务失败在状态与日志中可以区分。
5. 远程执行域、停用、更新、健康旧进程保留、精确旧版本重建和重启都有端到端测试。
6. 未支持能力返回明确降级；普通插件错误不影响无关调用，Plugin Host 崩溃的共享影响范围可见且不终止 Rust 主应用。
