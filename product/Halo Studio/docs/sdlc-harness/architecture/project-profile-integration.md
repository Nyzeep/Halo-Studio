# Halo Studio 子模块设计：项目画像与集成

> 上游文档：[design.md](../design.md)
> 模块角色：在 Halo Studio 加载外部目标项目后，渐进发现、归一化、版本化项目画像，并通过适配器连接项目依赖的代码托管、议题（issue）、CI、文档、发布和观测系统。

## 1. 模块定位

项目画像是渐进项目理解能力。它让 Halo Studio 更快进入有用状态，并在风险出现时解释检查、提示、审查和配置建议的来源。
...
- 适配器只负责读取、同步和投影外部系统语义，不改变 Halo Studio 规范模型。
...
- 目标项目的配置管理、需求管理、CI 和发布系统保留系统主权，Halo Studio 通过适配器读取和投影。
- 项目画像以实际仓库信号为准，避免把单一技术栈或单一组织流程作为模板。
- 目标项目无需先改造成 Halo Studio 推荐结构；Halo Studio 通过路径、规则和适配器逐步理解。
...
| 对 Halo Studio 自身验证样本过拟合 | 默认模式不能内置 Halo Studio 路径、语言或验证命令 |
...
- 用户无需先配置完整 `.halo` 就能完成普通任务。
- 风险分类器、变更就绪度和安全边界能解释所用项目事实来自哪里。
- 未知或冲突规则会降低信心，并进入建议态或降级状态。
- 外部适配器接入不会改变 Halo Studio 规范事件、交付物、权限和策略模型的一致性。
