...
安全边界是 Halo Studio Agent 执行层的安全域。它判断当前动作是否可能越权、泄露、破坏环境或被恶意上下文操控，并输出允许、沙箱允许、询问、应急放行、拒绝或策略拒绝。
...
安全边界不按工具名称、命令关键词或模型判断结果直接授权。凡经 Halo Studio owner/facade 发起的 tool、MCP、skill、
插件、Hook、shell、文件写入、网络访问或浏览器/桌面动作，每次调用都应统一转换为风险评估请求：
...
| OpenCode 扩展 | plugin/Hook、custom tool 和终端插件 | 首次激活、启动或 import 及关键事实变化时重新检查来源许可；本地默认兼容策略可允许，调用仍走归属模块的权限路径 |
| MCP 与其他外部能力 | MCP server/tool 及其他归属模块管理的执行面 | 由对应归属模块决定来源/连接许可和 allow/ask/deny；Harness 不重复其决定 |
...
沙箱不是单一能力。Halo Studio 需要同时区分三类保护：
...
| Plugin Host | `PluginRuntimeClient`、生态适配器、`ScriptToolRuntime` | 来源、插件身份、执行机器、OS 用户、策略、凭据和环境 | 独立子进程；仅兼容插件共享 Host；可用时增加 Job Object、process group 或容器 | 共享 Host 不代表插件彼此隔离；工具结果、权限、审计和业务状态仍由原模块提交 |
...
| P3 | `ScriptToolRuntime` 所在 services 实现的进程树、平台资源限制和执行域隔离增强；只按真实隔离能力展示保护、降级与残余风险，不以独立进程冒充完整沙箱 |
...
| OpenCode plugin/Hook 来源变化 | OpenCode 归属模块在首次激活、启动或 import 前重新检查来源许可；调用时权限独立判断，Harness 不追加激活步骤 |
