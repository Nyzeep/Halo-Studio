# Halo Studio 子模块设计：质量数据面

> 上游文档：[design.md](../design.md)
> 模块角色：为 Halo Studio 加载的目标项目提供统一事件、证据、指标和审计数据模型，用于解释、恢复、回放和校准产品治理策略。
...
- 质量数据面聚焦 Halo Studio 工程治理事实，不建设通用日志平台。
- 终端输出、模型上下文和第三方载荷按摘要、引用、脱敏和保留策略处理。
- CI、APM、SIEM 和数据仓库保留系统主权，Halo Studio 做事件映射和投影。
...
| `plugin_suggested` | 第三方插件明确产生的建议性内容 | 不可以，必须经过 Halo Studio 策略或人工确认；合法 Hook 变换和插件工具结果不归入该等级，而由对应 owner 校验并产生事实 |
