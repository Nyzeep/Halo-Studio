...
Halo Studio is a desktop-grade Agent runtime (Rust core + Tauri shell) that runs on your own machine with broad capabilities—filesystem, terminal, Git, MCP/LSP, and remote control. Because of this reach, we take security reports seriously and appreciate the community's help in keeping users safe.

## Supported Versions

Halo Studio is currently in active `0.x` development and ships as a rolling release. Security fixes land on the latest release; older versions are not patched separately.
...
Please upgrade to the latest [release](https://github.com/Nyzeep/Halo-Studio/releases) before reporting an issue to confirm it still reproduces.
...
➡️ **[Report a vulnerability](https://github.com/Nyzeep/Halo-Studio/security/advisories/new)**
...
- The Halo Studio runtime, official Agents, desktop/CLI/server apps, web UI, and the mobile-web pairing/remote-control flow in this repository.

Out of scope:

- Issues in third-party dependencies (please report those upstream; let us know if a Halo Studio update is needed).
...
Thank you for helping keep Halo Studio and its users safe.
