...
async function readFrontendNavigationMarker(): Promise<FrontendNavigationMarker | null> {
  return browser.execute(() => {
    try {
      ...
      const snapshot = startupTraceWindow.__HALO_STARTUP_TRACE__?.snapshot?.();
  ...
  await browser.waitUntil(async () => {
    try {
      return browser.execute((expectedProjectName: string) => {
        ...
        const startupOverlayGone = document.getElementById('halo-startup-overlay') === null;
        const labels = Array.from(document.querySelectorAll('.halo-nav-panel__workspace-item-label'))
...
export async function getWorkspaceState(): Promise<WorkspaceState> {
  return browser.execute(async () => {
    ...
    if (typeof invoke === 'function') {
      ...
      const workspaceLabels = Array.from(document.querySelectorAll('.halo-nav-panel__workspace-item-label'))
        .map(element => element.textContent?.trim() || '')
        .filter(Boolean);
...
export async function ensureCodeSessionOpen(): Promise<void> {
  ...
  const selectors = [
    '.halo-nav-panel__workspace-create-main--split-left',
