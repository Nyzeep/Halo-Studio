...
const PERF_RUN_ROOT = path.join(ROOT, 'tests', 'e2e', '.halo', 'perf-runs');
...
const scenarios = {
  ...
  'rapid-switch-zero-delay': {
    ...
    env: {
      HALO_E2E_PERF_RAPID_SWITCH_DELAY_MS: '0',
    },
  },
  'first-scroll': {
    spec: './specs/performance/startup-session-perf.spec.ts',
    ...
    reportPrefix: 'long-session-first-open-',
    env: {
      HALO_E2E_PERF_POST_VISIBLE_INTERACTION: 'first-scroll',
    },
  },
  'scroll-down': {
    spec: './specs/performance/startup-session-perf.spec.ts',
    ...
    reportPrefix: 'long-session-first-open-',
    env: {
      HALO_E2E_PERF_POST_VISIBLE_INTERACTION: 'scroll-down',
    },
  },
  'turn-navigation': {
    ...
    reportPrefix: null,
    env: {
      HALO_E2E_TURN_NAV_SESSION_ID: DEFAULT_LONG_SESSION_TARGET_ID,
      HALO_E2E_TURN_NAV_TARGET_INDEX: '20',
    },
  },
  'resize-window': {
    spec: './specs/performance/startup-session-perf.spec.ts',
    ...
    reportPrefix: 'long-session-first-open-',
    env: {
      HALO_E2E_PERF_POST_VISIBLE_INTERACTION: 'resize-window',
    },
  },
  'resize-window-width': {
    spec: './specs/performance/startup-session-perf.spec.ts',
    ...
    reportPrefix: 'long-session-first-open-',
    env: {
      HALO_E2E_PERF_POST_VISIBLE_INTERACTION: 'resize-window-width',
    },
  },
  'input-layout': {
...
function runnerStdioOptions() {
  if (process.env.HALO_E2E_PERF_RUNNER_STREAM_LOGS === '1') {
...
function allowMissingReports() {
  return (
    ...
    process.env.HALO_E2E_PERF_ALLOW_MISSING_REPORTS === '1'
  );
}
...
function prepareScenarioRuntime(name, baseEnv) {
  ...
  fs.writeFileSync(
    ...
    '# Halo performance fixture workspace\n',
    'utf8',
  );

  const env = {
    ...baseEnv,
    HALO_E2E_STORAGE_ROOT: storageRoot,
    HALO_E2E_HOME: homeRoot,
    HALO_HOME: homeRoot,
    HALO_E2E_USER_ROOT: userRoot,
    HALO_USER_ROOT: userRoot,
    HALO_E2E_LOG_DIR: logRoot,
    E2E_TEST_WORKSPACE: workspace,
    HALO_E2E_PERF_SESSION_ID: DEFAULT_LONG_SESSION_TARGET_ID,
    HALO_E2E_PERF_RAPID_SWITCH_SESSION_IDS: DEFAULT_RAPID_SWITCH_SESSION_IDS.join(','),
  };
...
function selectedScenarioNames() {
  const requested =
    argValue('--profile') ||
    argValue('--scenarios') ||
    process.env.HALO_E2E_PERF_MATRIX_PROFILE ||
    'core';
...
const baseEnv = {
  ...process.env,
  HALO_E2E_APP_MODE: process.env.HALO_E2E_APP_MODE || 'release-fast',
  E2E_LOG_LEVEL: process.env.E2E_LOG_LEVEL || 'warn',
};

if (hasFlag('--dry-run')) {
  console.log(
    `[long-session-matrix] dry-run profile=${requested} appMode=${baseEnv.HALO_E2E_APP_MODE} ` +
