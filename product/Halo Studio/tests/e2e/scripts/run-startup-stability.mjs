...
const iterations = readIntegerArgOrEnv(
  ['--samples', '--iterations'],
  'HALO_E2E_PERF_STARTUP_ITERATIONS',
  5,
);
const thresholds = {
  interactiveShellReadyMs: readNumberEnv('HALO_E2E_PERF_STARTUP_MAX_INTERACTIVE_MS', 5000),
  firstScriptEvalMs: readNumberEnv('HALO_E2E_PERF_STARTUP_MAX_FIRST_SCRIPT_MS', 3000),
...
if (readFlag('--dry-run')) {
  console.log(
    `[startup-stability] dry-run iterations=${iterations} appMode=${env.HALO_E2E_APP_MODE} ` +
