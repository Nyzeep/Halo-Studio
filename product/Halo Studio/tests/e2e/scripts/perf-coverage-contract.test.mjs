...
test('performance scripts expose focused startup stability and interaction profiles', () => {
  ...
  assert.match(startupRunner, /HALO_E2E_PERF_STARTUP_ITERATIONS/);
  assert.match(startupRunner, /--samples/);
  assert.match(startupRunner, /--iterations/);
  assert.match(startupRunner, /readIntegerArgOrEnv/);
  assert.match(startupRunner, /HALO_E2E_PERF_STARTUP_MAX_INTERACTIVE_MS/);
  ...
  assert.match(interactionRunner, /HALO_E2E_PERF_MATRIX_PROFILE/);
  ...
  assert.match(interactionRunner, /HALO_E2E_PERF_RAPID_SWITCH_DELAY_MS/);
  assert.match(interactionRunner, /HALO_E2E_PERF_ALLOW_MISSING_REPORTS/);
...
test('long session interaction matrix isolates user data and avoids active-session preload bias', () => {
  ...
  assert.match(interactionRunner, /HALO_E2E_STORAGE_ROOT/);
  assert.match(interactionRunner, /generate-long-session-fixture\.mjs/);
  assert.match(interactionRunner, /HALO_E2E_PERF_SESSION_ID/);
  assert.match(interactionRunner, /perf-long-session-001/);
  assert.match(interactionRunner, /HALO_E2E_PERF_RAPID_SWITCH_SESSION_IDS/);
