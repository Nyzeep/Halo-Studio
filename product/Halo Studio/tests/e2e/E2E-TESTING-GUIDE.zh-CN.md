...
# Halo Studio E2E 测试指南

使用 WebDriverIO + Halo Studio 内嵌 WebDriver 进行 Halo Studio 项目的端到端测试完整指南。
...
Halo Studio E2E 测试专注于**用户旅程**和**关键路径**，确保桌面应用从用户角度正常工作。我们使用分层测试方法来平衡覆盖率和执行速度。
...
#### Debug 模式（默认）
- **应用路径**: `target/debug/halo-desktop.exe`
...
**核心原理**: 功能 E2E 默认使用 `target/debug/halo-desktop.exe`。性能 E2E 需要显式设置 `HALO_E2E_APP_MODE=release-fast`，并先执行 `pnpm run desktop:build:release-fast`。

不要直接手工启动 `target/release-fast/halo-desktop.exe` 做性能验证。直接启动会使用普通用户 profile，除非手动提供隔离存储环境变量。E2E launcher 会自动设置 `HALO_USER_ROOT`、`HALO_HOME` 和 `HALO_E2E_STORAGE_GUARD=1`，确保性能测试不会静默写入真实 Halo Studio profile。
...
测试报告会写入 `tests/e2e/reports/performance/`，包含启动阶段、Tauri API 聚合、首次打开长 session、后台 full hydrate 等时间片。可选阈值环境变量：`HALO_E2E_PERF_MAX_INTERACTIVE_MS`、`HALO_E2E_PERF_MAX_SESSION_FRAME_MS`。
...
6. **不要测试第三方代码** - 只测试 Halo Studio 功能
...
# 用 debug 模式运行测试，Halo Studio 会在进程内启动 WebDriver
...
**症状**: `Application not found at target/debug/halo-desktop.exe`
