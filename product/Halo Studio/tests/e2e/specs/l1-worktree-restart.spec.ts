...
const phase = process.env.HALO_E2E_WORKTREE_RESTART_PHASE;
const manifestPath = process.env.HALO_E2E_WORKTREE_RESTART_MANIFEST;
...
async function cleanupFixture(manifest: Partial<RestartManifest>): Promise<void> {
  ...
  if (
    ...
    && path.basename(manifest.fixtureRoot).startsWith('halo-worktree-restart-e2e-')
  ) {
...
describe('L1 managed Worktree desktop restart recovery', () => {
  if (!manifestPath) {
    it('requires a runner-provided manifest path', () => {
      throw new Error('HALO_E2E_WORKTREE_RESTART_MANIFEST is required');
    });
    return;
  }
  ...
  if (phase === 'create') {
    ...
    it('persists a managed Worktree and session before Desktop exits', async () => {
      const fixtureRoot = fs.mkdtempSync(
        path.join(os.tmpdir(), 'halo-worktree-restart-e2e-'),
      );
      ...
      git(repositoryPath, ['config', 'user.name', 'Halo E2E']);
      git(repositoryPath, ['config', 'user.email', 'halo-e2e@example.invalid']);
  ...
  it('requires a valid restart phase', () => {
    throw new Error(
      'HALO_E2E_WORKTREE_RESTART_PHASE must be "create" or "verify"',
    );
  });
});
