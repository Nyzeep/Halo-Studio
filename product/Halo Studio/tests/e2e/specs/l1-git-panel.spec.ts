...
describe('L1 Git Panel', () => {
  ...
  describe('Git panel existence', () => {
    ...
    it('git panel should detect repository status', async function () {
      ...
      const notRepo = await $('.halo-git-scene--not-repository');
      const isLoading = await $('.halo-git-scene--loading');
      const isRepo = await $('.halo-git-scene-working-copy');
  ...
  describe('Branch display', () => {
    it('current branch should be displayed if in git repo', async function () {
      ...
      const branchElement = await $('.halo-git-scene-working-copy__branch');
