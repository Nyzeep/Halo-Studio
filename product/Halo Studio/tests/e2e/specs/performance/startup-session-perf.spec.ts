...
function defaultHaloHome(): string {
...
async function assertReleaseFastPerfRuntime(): Promise<{
  ...
  const appMode = (process.env.HALO_E2E_APP_MODE ?? '').toLowerCase();
  const allowDevServer = process.env.HALO_E2E_ALLOW_RELEASE_FAST_DEV_SERVER === '1';
  ...
  if (appMode === 'release-fast' && isDevServerRuntime && !allowDevServer) {
    throw new Error(
      `release-fast perf run loaded a dev-server URL: ${runtime.runtimeUrl}. ` +
        ...
        'HALO_E2E_ALLOW_RELEASE_FAST_DEV_SERVER=1 only for explicit dev-server diagnostics.',
    );
  }
...
async function findSessionItem(sessionId: string): Promise<ReturnType<typeof $> | null> {
  ...
  if (process.env.HALO_E2E_PERF_VERBOSE_REPORT === '1') {
...
async function findLongSessionMetadataPath(sessionId: string): Promise<string | null> {
  const haloHome = defaultHaloHome();
  ...
  try {
    const projectsDir = path.join(haloHome, 'projects');
...
async function readLongSessionViewportState(expectedLatestTurnId?: string | null): Promise<LongSessionViewportState> {
  return browser.execute((targetTurnId) => {
    ...
    const inputOverlay = document.querySelector<HTMLElement>('.halo-chat-input-drop-zone');
...
async function startLongSessionViewportTimelineRecorder(
  ...
  await browser.execute((targetTurnId, clickTime, shouldEnableRenderProfile) => {
    const globalWindow = window as typeof window & {
      __haloLongSessionViewportTimeline?: LongSessionViewportTimelineSample[];
      __haloLongSessionMainThreadTasks?: LongSessionMainThreadTask[];
      __haloLongSessionMutationEvents?: LongSessionDomMutationEvent[];
      __haloLongSessionVisualStateEvents?: LongSessionVisualStateEvent[];
      __haloLongSessionLayoutShiftEvents?: LongSessionLayoutShiftEvent[];
      __haloLongSessionViewportTimelineTimer?: number;
      __haloLongSessionVisualFrameRequest?: number;
      __haloLongSessionVisualProbeTimers?: number[];
      __haloLongSessionLongTaskObserver?: PerformanceObserver;
      __haloLongSessionLayoutShiftObserver?: PerformanceObserver;
      __haloLongSessionMutationObserver?: MutationObserver;
      __haloLongSessionOpenIntentAt?: number;
      __haloLongSessionOpenIntentSessionId?: string | null;
      __haloLongSessionOpenIntentHandler?: EventListener;
      __haloLongSessionUserInteractionHandler?: EventListener;
      __HALO_RENDER_PROFILE_ENABLED__?: boolean;
    };
    globalWindow.__HALO_RENDER_PROFILE_ENABLED__ = shouldEnableRenderProfile;
    if (globalWindow.__haloLongSessionViewportTimelineTimer !== undefined) {
      window.clearInterval(globalWindow.__haloLongSessionViewportTimelineTimer);
    }
    if (globalWindow.__haloLongSessionVisualFrameRequest !== undefined) {
      window.cancelAnimationFrame(globalWindow.__haloLongSessionVisualFrameRequest);
    }
    for (const timerId of globalWindow.__haloLongSessionVisualProbeTimers ?? []) {
      window.clearTimeout(timerId);
    }
    globalWindow.__haloLongSessionVisualProbeTimers = [];
    globalWindow.__haloLongSessionLongTaskObserver?.disconnect();
    globalWindow.__haloLongSessionLayoutShiftObserver?.disconnect();
    globalWindow.__haloLongSessionMutationObserver?.disconnect();
    if (globalWindow.__haloLongSessionOpenIntentHandler) {
      window.removeEventListener(
        'flowchat:history-session-open-intent',
        globalWindow.__haloLongSessionOpenIntentHandler,
      );
    }
    if (globalWindow.__haloLongSessionUserInteractionHandler) {
      window.removeEventListener(
        'halo:e2e-long-session-user-interaction',
        globalWindow.__haloLongSessionUserInteractionHandler,
      );
    }
    globalWindow.__haloLongSessionOpenIntentAt = undefined;
    globalWindow.__haloLongSessionOpenIntentSessionId = undefined;
    const handleOpenIntent: EventListener = event => {
      const detail =
        event instanceof CustomEvent && typeof event.detail?.sessionId === 'string'
          ? event.detail
          : null;
      globalWindow.__haloLongSessionOpenIntentAt = performance.now();
      globalWindow.__haloLongSessionOpenIntentSessionId = detail?.sessionId ?? null;
      ...
      for (const delayMs of [100, 250, 500, 1_000]) {
        ...
        globalWindow.__haloLongSessionVisualProbeTimers?.push(timerId);
      }
    };
    globalWindow.__haloLongSessionOpenIntentHandler = handleOpenIntent;
    ...
    globalWindow.__haloLongSessionUserInteractionHandler = handleUserInteraction;
    window.addEventListener('halo:e2e-long-session-user-interaction', handleUserInteraction);
    ...
    const LOADING_SURFACE_SELECTORS = [
      ...
      '.halo-scene-viewport__lazy-fallback',
      '.halo-assistant-scene__loading',
    ...
    const isLoadingSurfaceElement = (element: HTMLElement): boolean =>
      Boolean(
        ...
        element.closest('.halo-scene-viewport__lazy-fallback') ||
        element.closest('.halo-assistant-scene__loading') ||
    ...
    const classifySurfaceElement = (
      ...
      if (element.closest('.halo-chat-input-drop-zone')) {
        return {
    ...
    const readVisualStateEvent = (reason: string): LongSessionVisualStateEvent => {
      ...
      const signature = [
        ...
        globalWindow.__haloLongSessionOpenIntentSessionId ?? 'no-open-intent-session',
      ...
      return {
        atMs,
        sinceClickMs: atMs - clickTime,
        sinceHistoryOpenIntentMs: typeof globalWindow.__haloLongSessionOpenIntentAt === 'number'
          ? atMs - globalWindow.__haloLongSessionOpenIntentAt
          : null,
        historyOpenIntentSessionId: globalWindow.__haloLongSessionOpenIntentSessionId ?? null,
        frame: visualFrame,
        reason,
        signature,
        activeSessionId: messages?.dataset.activeSessionId || null,
    ...
    const recordAnimationFrame = () => {
      ...
      globalWindow.__haloLongSessionVisualFrameRequest =
    ...
    globalWindow.__haloLongSessionMutationObserver = mutationObserver;
    try {
      if (PerformanceObserver.supportedEntryTypes.includes('longtask')) {
        ...
        globalWindow.__haloLongSessionLongTaskObserver = observer;
      }
    } catch {
      globalWindow.__haloLongSessionLongTaskObserver = undefined;
    }
    try {
      if (PerformanceObserver.supportedEntryTypes.includes('layout-shift')) {
        ...
        globalWindow.__haloLongSessionLayoutShiftObserver = observer;
      }
    } catch {
      globalWindow.__haloLongSessionLayoutShiftObserver = undefined;
    }
    const readSample = (): LongSessionViewportTimelineSample => {
      ...
      const effectiveViewportHeight = scrollerRect && effectiveScrollerBottom !== null
        ...
        : null;
      const atMs = performance.now();
      const historyOpenIntentAtMs = globalWindow.__haloLongSessionOpenIntentAt ?? null;
      const historyOpenIntentSessionId = globalWindow.__haloLongSessionOpenIntentSessionId ?? null;
    ...
    globalWindow.__haloLongSessionViewportTimeline = samples;
    globalWindow.__haloLongSessionMainThreadTasks = mainThreadTasks;
    globalWindow.__haloLongSessionMutationEvents = mutationEvents;
    globalWindow.__haloLongSessionVisualStateEvents = visualStateEvents;
    globalWindow.__haloLongSessionLayoutShiftEvents = layoutShiftEvents;
    record();
    globalWindow.__haloLongSessionViewportTimelineTimer = window.setInterval(record, 50);
...
async function stopLongSessionViewportTimelineRecorder(): Promise<LongSessionViewportTimeline> {
  return browser.execute(() => {
    ...
    if (globalWindow.__haloLongSessionViewportTimelineTimer !== undefined) {
      window.clearInterval(globalWindow.__haloLongSessionViewportTimelineTimer);
      globalWindow.__haloLongSessionViewportTimelineTimer = undefined;
    }
    if (globalWindow.__haloLongSessionVisualFrameRequest !== undefined) {
      window.cancelAnimationFrame(globalWindow.__haloLongSessionVisualFrameRequest);
      globalWindow.__haloLongSessionVisualFrameRequest = undefined;
    }
    for (const timerId of globalWindow.__haloLongSessionVisualProbeTimers ?? []) {
      window.clearTimeout(timerId);
    }
    globalWindow.__haloLongSessionVisualProbeTimers = undefined;
    globalWindow.__haloLongSessionLongTaskObserver?.disconnect();
    globalWindow.__haloLongSessionLongTaskObserver = undefined;
    globalWindow.__haloLongSessionLayoutShiftObserver?.disconnect();
    globalWindow.__haloLongSessionLayoutShiftObserver = undefined;
    globalWindow.__haloLongSessionMutationObserver?.disconnect();
    globalWindow.__haloLongSessionMutationObserver = undefined;
    if (globalWindow.__haloLongSessionOpenIntentHandler) {
      window.removeEventListener(
        'flowchat:history-session-open-intent',
        globalWindow.__haloLongSessionOpenIntentHandler,
      );
      globalWindow.__haloLongSessionOpenIntentHandler = undefined;
    }
    if (globalWindow.__haloLongSessionUserInteractionHandler) {
      window.removeEventListener(
        'halo:e2e-long-session-user-interaction',
      ...
      globalWindow.__haloLongSessionUserInteractionHandler = undefined;
    }
    globalWindow.__haloLongSessionOpenIntentAt = undefined;
    const samples = globalWindow.__haloLongSessionViewportTimeline ?? [];
    const mainThreadTasks = globalWindow.__haloLongSessionMainThreadTasks ?? [];
    const mutationEvents = globalWindow.__haloLongSessionMutationEvents ?? [];
    const visualStateEvents = globalWindow.__haloLongSessionVisualStateEvents ?? [];
    const layoutShiftEvents = globalWindow.__haloLongSessionLayoutShiftEvents ?? [];
    globalWindow.__haloLongSessionViewportTimeline = undefined;
    globalWindow.__haloLongSessionMainThreadTasks = undefined;
    globalWindow.__haloLongSessionMutationEvents = undefined;
    globalWindow.__haloLongSessionVisualStateEvents = undefined;
    globalWindow.__haloLongSessionLayoutShiftEvents = undefined;
    globalWindow.__HALO_RENDER_PROFILE_ENABLED__ = false;
...
async function maybeSavePerfScreenshot(name: string): Promise<string | null> {
  if (process.env.HALO_E2E_PERF_SCREENSHOTS !== '1') {
    return null;
  }
...
function readPostVisibleInteractionOption(
  ...
  const envValue = process.env.HALO_E2E_PERF_POST_VISIBLE_INTERACTION;
  if (
...
function webDriverEndpoint(pathname: string): string {
  const port = Number(process.env.HALO_E2E_WEBDRIVER_PORT || 4445);
...
async function recordLongSessionPostVisibleInteraction(
  ...
  await browser.execute((eventDetail) => {
    window.dispatchEvent(new CustomEvent('halo:e2e-long-session-user-interaction', {
      detail: eventDetail,
    }));
  }, detail);
}
...
function readRapidSwitchSessionIds(): string[] {
  const raw = process.env.HALO_E2E_PERF_RAPID_SWITCH_SESSION_IDS;
...
async function readActiveSessionNavId(): Promise<string | null> {
  return browser.execute(() => {
    const active = document.querySelector<HTMLElement>(
      ...
      '.halo-nav-panel__inline-item.is-active[data-session-id]',
    );
...
async function collectRapidLongSessionSwitchMeasurement(
  ...
  await startLongSessionViewportTimelineRecorder(
    ...
    clickedAtMs,
    process.env.HALO_E2E_RENDER_PROFILE === '1',
  );
  ...
  for (let index = 0; index < sessionIds.length; index += 1) {
    ...
    if (index < sessionIds.length - 1) {
      const delayMs = numericEnv('HALO_E2E_PERF_RAPID_SWITCH_DELAY_MS') ?? 75;
  ...
  const postVisibleObserveMs =
    numericEnv('HALO_E2E_PERF_RAPID_SWITCH_OBSERVE_MS') ??
    numericEnv('HALO_E2E_PERF_POST_VISIBLE_OBSERVE_MS') ??
  ...
  const verboseTimelineReport = process.env.HALO_E2E_PERF_VERBOSE_REPORT === '1';

  return {
    appMode: process.env.HALO_E2E_APP_MODE ?? 'auto',
    requestedSessionIds,
    sessionIds,
...
async function collectLongSessionOpenMeasurement(
  ...
  latestUsable ??= await latestUsablePromise;
  const postVisibleObserveMs =
    numericEnv('HALO_E2E_PERF_POST_VISIBLE_OBSERVE_MS') ?? DEFAULT_POST_VISIBLE_OBSERVE_MS;
  const observeRemainingMs = latestUsable.usableAtMs + postVisibleObserveMs - await readPerformanceNow();
  if (observeRemainingMs > 0) {
    await browser.pause(Math.ceil(observeRemainingMs));
  }
...
async function clearLongSessionPendingQueue(sessionId: string): Promise<void> {
  ...
  await browser.waitUntil(async () => {
    const pendingQueueCount = await browser.execute(() =>
      document.querySelectorAll('[data-testid="pending-queue-panel"] .halo-pending-queue-panel__item').length
    );
...
async function setLongSessionChatInputValue(message: string): Promise<void> {
  await browser.execute((value) => {
    const input = document.querySelector<HTMLElement>(
      ...
      '.halo-chat-input [contenteditable="true"]',
    );
    if (!input) {
  ...
  await browser.waitUntil(async () => {
    const button = await $('[data-testid="chat-input-send-btn"], button.halo-chat-input__send-button');
...
async function collectLongSessionSendAfterOpenMeasurement(
  ...
  const postSendObserveMs = numericEnv('HALO_E2E_PERF_SEND_AFTER_OPEN_OBSERVE_MS') ?? 2000;
...
describe('Performance telemetry', () => {
  ...
  it('collects startup timing from the current build', async () => {
    ...
    const maxInteractiveMs = numericEnv('HALO_E2E_PERF_MAX_INTERACTIVE_MS');
  ...
  it('collects first-open timing for a generated long session', async function () {
    ...
    const sessionId = process.env.HALO_E2E_PERF_SESSION_ID || DEFAULT_PERF_SESSION_ID;
    ...
    const maxLatestFrameMs = numericEnv('HALO_E2E_PERF_MAX_SESSION_FRAME_MS');
