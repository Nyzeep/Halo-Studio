/**
 * Diagnostic-only spec for comparing the real local stream inside Halo's
 * embedded browser. Run with HALO_BROWSER_STREAM_URL=http://127.0.0.1:41953/
 */

import { browser, $ } from '@wdio/globals';

const streamUrl = process.env.HALO_BROWSER_STREAM_URL || 'http://127.0.0.1:41953/';
const describeRealStream = process.env.HALO_BROWSER_STREAM_URL ? describe : describe.skip;
...
describeRealStream('L1 Built-in browser real stream diagnostic', () => {
  it('opens the real local stream and prints browser telemetry', async () => {
    ...
    if (process.env.HALO_BROWSER_LONG_DRAG === '1') {
      await runLongDrag();
    } else {
