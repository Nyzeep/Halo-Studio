...
const NAV_ENTRY_SELECTORS = [
  ...
  '.halo-nav-panel__item-slot',
  '.halo-nav-panel__workspace-item-name-btn',
  '.halo-nav-panel__inline-item',
  '.halo-nav-panel__workspace-create-main',
  '.halo-nav-panel__miniapp-entry',
];
...
describe('L1 Navigation', () => {
  ...
  describe('Navigation panel structure', () => {
    it('navigation panel should be visible', async function () {
      ...
      const navPanel = await $('.halo-nav-panel');
    ...
    it('should have navigation sections', async function () {
      ...
      const sections = await browser.$$('.halo-nav-panel__section');
  ...
  describe('Navigation item clicking', () => {
    ...
    it('clicking navigation item should change view', async function () {
      ...
      const navItems = await browser.$$('.halo-nav-panel__item');
  ...
  describe('Active item highlighting', () => {
    it('should have active state on navigation item', async function () {
      ...
      const activeItems = await browser.$$('.halo-nav-panel__item.is-active, .halo-nav-panel__inline-item.is-active, .halo-nav-panel__miniapp-entry.is-active');
    ...
    it('clicking item should update active state', async function () {
      ...
      const initialActive = await browser.$$('.halo-nav-panel__item.is-active, .halo-nav-panel__inline-item.is-active, .halo-nav-panel__miniapp-entry.is-active');
      ...
      const afterActive = await browser.$$('.halo-nav-panel__item.is-active, .halo-nav-panel__inline-item.is-active, .halo-nav-panel__miniapp-entry.is-active');
  ...
  describe('Navigation expand/collapse', () => {
    it('navigation sections should be expandable', async function () {
      ...
      const expandableSections = await browser.$$('.halo-nav-panel__section-header');
    ...
    it('inline sections should be collapsible', async function () {
      ...
      const inlineLists = await browser.$$('.halo-nav-panel__inline-list');
