...
> **实际全局对象为 `window.app`**（非 `window.__HALO__`），以下各节均基于 `window.app`。

## 能力边界（先看这一节）

MiniApp **能且只能**用以下 API，没有任何"通用 Halo Studio 后端通道"。生成代码前请先确认你需要的能力在表内：
...
**框架不暴露**的 Halo Studio 后端能力（截至当前版本）：WorkspaceService（结构化搜索 / 索引）、GitService（结构化 status/diff/blame）、TerminalService、Session/AgenticSystem、LSP / Snapshot / Mermaid / Skills / Browser / Computer Use / Config 等。需要这些能力时：

1. 能用裸命令行解决就用 `app.shell.exec`（如 git → 在 `permissions.shell.allow` 加 `"git"`，参考 `builtin-coding-selfie`）；
2. 只是要读 Halo Studio 工作区里的文件就用 `app.fs.*`（把 `{workspace}` 加到 `permissions.fs.read`）；
...
- DOM、CSS（含 CSS 变量 `--halo-bg`, `--halo-text`, `--halo-accent` 等）
- Canvas 2D / WebGL
- Web Audio
...
app.onThemeChange((payload) => {
  // payload: { type: 'dark'|'light', vars: { '--halo-bg': '...', ... } }
});
...
宿主会把这些框架原语直接路由到 Rust 实现（`halo_core::miniapp::host_dispatch`），完全不需要 Bun/Node 运行时；权限策略与 Worker 路径共用同一份 `resolve_policy`，行为完全等价。在这种模式下：
