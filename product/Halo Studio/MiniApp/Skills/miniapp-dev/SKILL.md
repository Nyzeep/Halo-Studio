---
name: miniapp-dev
description: Develops, maintains, and generates Halo Studio MiniApps (Zero-Dialect Runtime). Use when (1) working on miniapp framework code under src/crates/assembly/core/src/miniapp/ or src/web-ui/src/app/scenes/miniapps/; or (2) generating / creating / designing a NEW MiniApp for the user — including any request like "做一个小应用 / 生成 MiniApp / 写个 Halo Studio 小工具 / 创建 mini app". Also triggers on MiniApp, miniapps, bridge, zero-dialect, InitMiniApp, app.fs / app.shell / app.storage, or any work under MiniApp/Demo/ and MiniApp/Skills/.
---

# Halo Studio MiniApp V2 指南
...
### 工具型 vs 展示型
绝大多数 Halo Studio MiniApp 是**工具型**——信息密集、操作短、配色冷静，仿照 `regex-playground` / `coding-selfie` / `git-graph` 的克制感。只有用户明确要"对外展示 / 灵感型 / 作品集"时才放飞视觉。
...
MiniApp 使用 **标准 Web API + window.app**：UI 侧为 ESM 模块（`ui.js`），后端逻辑在独立 JS Worker 进程（Bun 优先 / Node 回退）中执行。Rust 负责进程管理、权限策略和 Tauri 独占 API；Bridge 从旧的 `require()` shim + `__HALO__` 替换为统一的 **window.app** Runtime Adapter。
...
iframe 内 window.app.call(method, params)
  ...
  │   │                          → halo_core::miniapp::host_dispatch（纯 Rust，无需 Bun/Node）
...
MiniApp 框架**只暴露下列能力**，没有任何"通用 Halo Studio 后端通道"。设计 / 生成新小应用前请先比对，能力不在表内的需求请走相应替代方案，**不要假设有 `app.halo.*` / `app.workspace.*` / `app.git.*` / `app.session.*` 之类的接口存在。**
...
### 框架**不**直接暴露的 Halo Studio 后端能力（截至本文档）

下面这些 Halo Studio 内部服务，目前**没有**给小应用开放调用通道：
...
2. **只是要读 Halo Studio 工作区内的文件**（如某些项目元数据） → 把 `{workspace}` 加到 `permissions.fs.read`，自己用 `app.fs.*` 读 + 在前端解析；
3. **必须真调用某个内部服务** → 暂不支持，先记录到需求池。**不要**自己起一个 worker 去模拟服务行为，会和真正的 service 行为漂移。

> 维护者：以后若新增 `app.halo.*` / `app.workspace.*` 这类宿主直通通道，请同步更新本节，避免"文档说没有、代码偷偷加了"的不一致。
...
宿主会将主应用主题映射为以下 CSS 变量并注入 iframe 的 `:root`。在 MiniApp 的 CSS 中建议用 `var(--halo-*, <fallback>)` 引用，以便在 Halo Studio 内与主应用一致，导出为独立应用时 fallback 生效。

**背景**

- `--halo-bg` — 主背景
- `--halo-bg-secondary` — 次级背景（如工具栏、面板）
- `--halo-bg-tertiary` — 第三级背景
- `--halo-bg-elevated` — 浮层/卡片背景

**文字**

- `--halo-text` — 主文字
- `--halo-text-secondary` — 次要文字
- `--halo-text-muted` — 弱化文字

**强调与语义**

- `--halo-accent`、`--halo-accent-hover` — 强调色及悬停
- `--halo-success`、`--halo-warning`、`--halo-error`、`--halo-info` — 语义色

**边框与元素**

- `--halo-border`、`--halo-border-subtle` — 边框
- `--halo-element-bg`、`--halo-element-hover` — 控件背景与悬停

**圆角与字体**

- `--halo-radius`、`--halo-radius-lg` — 圆角
- `--halo-font-sans`、`--halo-font-mono` — 无衬线与等宽字体

**滚动条**

- `--halo-scrollbar-thumb`、`--halo-scrollbar-thumb-hover` — 滚动条滑块

示例（在 `style.css` 中）：

```css
:root {
  --bg: var(--halo-bg, #121214);
  --text: var(--halo-text, #e8e8e8);
  --accent: var(--halo-accent, #60a5fa);
}
body {
  font-family: var(--halo-font-sans, system-ui, sans-serif);
  color: var(--text);
  background: var(--bg);
}
```
...
- iframe 加载后 bridge 会向宿主发送 `halo/request-theme`，宿主回推当前主题变量，iframe 内 `_applyThemeVars` 写入 `:root`。
...
- [ ] 本地清掉 `~/.halo-studio/miniapps/<app_id>/.builtin-manifest.json` 或直接整目录删，再启动验证 reseed 生效
