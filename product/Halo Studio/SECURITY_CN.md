...
Halo Studio 是一个运行在你本机的桌面级 Agent 运行时（Rust 内核 + Tauri 外壳），具备文件系统、终端、Git、MCP/LSP 以及远程控制等广泛能力。正因为权限范围较大，我们非常重视安全问题，并感谢社区帮助我们保护用户安全。

## 受支持的版本

Halo Studio 目前处于 `0.x` 活跃开发阶段，采用滚动发布。安全修复只会合入最新发布版本，旧版本不会单独打补丁。
...
在上报问题前，请先升级到最新[发布版](https://github.com/Nyzeep/Halo-Studio/releases)，确认问题仍可复现。
...
➡️ **[上报漏洞](https://github.com/Nyzeep/Halo-Studio/security/advisories/new)**
...
- 本仓库中的 Halo Studio 运行时、官方 Agent、桌面端/CLI/服务端应用、Web UI，以及移动端配对/远程控制流程。

不在适用范围内：

- 第三方依赖中的问题（请向上游上报；如需 Halo Studio 侧更新请告知我们）。
