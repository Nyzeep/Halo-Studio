...
export function useFlowChatToolActions() {
  const handleToolConfirm = useCallback(async (
    ...
    try {
      ...
      log.warn('Ignoring legacy Halo tool confirmation without a permission request id', { toolId });
    } catch (error) {
  ...
  const handleToolReject = useCallback(async (toolId: string, options?: ToolRejectOptions) => {
    try {
      ...
      log.warn('Ignoring legacy Halo tool rejection without a permission request id', { toolId });
    } catch (error) {
