...
describe('scheduleAfterStartupPaint', () => {
  ...
  it('waits for the startup signal before scheduling paint-delayed work', () => {
    ...
    scheduleAfterStartupSignal(task, {
      frameCount: 1,
      signalName: 'halo:main-window-shown',
    ...
    expect(addEventListener).toHaveBeenCalledWith('halo:main-window-shown', expect.any(Function));
    ...
    expect(task).toHaveBeenCalledTimes(1);
    expect(removeEventListener).toHaveBeenCalledWith('halo:main-window-shown', expect.any(Function));
  });
