  ...
  it('adds a locale-only guardrail for i18n-only frontend review targets', () => {
    ...
    const target = classifyReviewTargetFromFiles(
      [
        ...
        'Halo-Installer/src/i18n/locales/zh-TW.json',
      ],
      'session_files',
    );
    ...
    expect(promptBlock).toContain('Halo-Installer/src/i18n/locales/zh-TW.json');
    expect(promptBlock.match(/Halo-Installer\/src\/i18n\/locales\/zh-TW\.json/g)).toHaveLength(1);
    expect(promptBlock).not.toContain('Halo-Installer/src/i18n/Halo-Installer/src/i18n/locales/zh-TW.json');
  });
