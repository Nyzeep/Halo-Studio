...
describe('session metadata startup scheduling', () => {
  ...
  it('keeps deferred metadata tied to the startup overlay handoff', () => {
    expect(SESSION_METADATA_DEFERRED_SIGNAL).toBe('halo:startup-overlay-hidden');
  ...
  it('uses a stable bounded stagger for background workspace metadata', () => {
    const first = getDeferredSessionMetadataDelayMs('D:\\workspace\\Halo');
    const second = getDeferredSessionMetadataDelayMs('D:\\workspace\\Halo');
