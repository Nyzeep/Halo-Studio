...
const TerminalScene: React.FC<TerminalSceneProps> = ({ isActive = true }) => {
  ...
  return (
    <div className="halo-terminal-scene" aria-hidden={!isActive} data-testid="shell-panel">
      {activeSessionId ? (
        <ConnectedTerminal
          key={activeSessionId}
        ...
        <div className="halo-terminal-scene__empty" data-testid="shell-command-list">
          <SquareTerminal size={32} className="halo-terminal-scene__empty-icon" />
          <p className="halo-terminal-scene__empty-hint" data-testid="shell-panel-title">{t('emptyState')}</p>
        </div>
      )}
    </div>
  );
