...
describe('SettingsScene lazy tab routing', () => {
  ...
  async function renderActiveTab(
    tab: 'appearance' | 'editor' | 'keyboard' | 'archived-sessions'
  ) {
  ...
  it('renders the lazy appearance config tab', async () => {
    await renderActiveTab('appearance');

    expect(container.querySelector('[data-testid="appearance-config"]')).not.toBeNull();
  });

  it('renders the lazy editor config tab', async () => {
    await renderActiveTab('editor');

    expect(container.querySelector('[data-testid="editor-config"]')).not.toBeNull();
  });

  it('renders the lazy keyboard shortcuts tab', async () => {
    await renderActiveTab('keyboard');

    expect(container.querySelector('[data-testid="keyboard-shortcuts-config"]')).not.toBeNull();
  });

  it('renders the lazy archived sessions tab', async () => {
    await renderActiveTab('archived-sessions');

    expect(container.querySelector('[data-testid="archived-sessions-config"]')).not.toBeNull();
  });
