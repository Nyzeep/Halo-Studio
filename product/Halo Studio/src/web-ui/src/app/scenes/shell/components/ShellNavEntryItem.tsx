...
const ShellNavEntryItem: React.FC<ShellNavEntryItemProps> = ({
  ...
  return (
    ...
    >
      <div className="halo-shell-nav__terminal-item-row">
        <Tooltip content={entry.name} placement="right">
          <span className="halo-shell-nav__terminal-item-main">
            {showSavedBadge ? (
              <Bookmark size={14} className="halo-shell-nav__terminal-icon halo-shell-nav__terminal-icon--saved" />
            ) : (
              <SquareTerminal size={14} className="halo-shell-nav__terminal-icon" />
            )}

            <span className="halo-shell-nav__terminal-label" data-testid="shell-command-text">{entry.name}</span>

            {showSavedBadge ? (
              <span className="halo-shell-nav__saved-indicator">{savedBadgeLabel}</span>
            ) : null}

            {entry.startupCommand ? (
              <span className="halo-shell-nav__cmd-indicator">{startupCommandBadgeLabel}</span>
            ) : null}

            <span
              className={`halo-shell-nav__terminal-dot${entry.isRunning ? ' is-running' : ' is-stopped'}`}
        ...
        <Tooltip content={quickAction.title} placement="right">
          <button
            type="button"
            className="halo-shell-nav__terminal-close"
            onClick={(event) => {
      ...
      {displayCwd ? (
        <span className="halo-shell-nav__terminal-cwd" title={displayCwd}>
          {displayCwd}
        </span>
      ) : null}
    </div>
