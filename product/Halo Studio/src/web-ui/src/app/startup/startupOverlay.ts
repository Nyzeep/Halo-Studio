const STARTUP_OVERLAY_ID = 'halo-startup-overlay';
const EXIT_CLASS = 'halo-startup-overlay--exiting';
const EXIT_FALLBACK_MS = 350;

declare global {
  interface Window {
    __HALO_STARTUP_OVERLAY_STARTED_AT__?: number;
  }
}
...
export function getStartupOverlayElapsedMs(): number {
  const startedAt = window.__HALO_STARTUP_OVERLAY_STARTED_AT__;
