...
describe('startup preload shell', () => {
  it('uses injected startup locale text and wires static window controls', () => {
    ...
    const dom = new JSDOM(readIndexHtml(), {
      ...
      beforeParse(window) {
        Object.assign(window, {
          __HALO_BOOTSTRAP_LOCALE__: 'zh-CN',
          __HALO_BOOTSTRAP_MESSAGES__: {
            loadingApp: '正在启动 Halo...',
            minimize: '最小化',
            maximize: '最大化',
            close: '关闭',
          },
          __HALO_SHOW_STARTUP_WINDOW_CONTROLS__: true,
    ...
    expect(dom.window.document.getElementById('halo-startup-overlay')).not.toBeNull();
    expect(hint?.textContent).toBe('正在启动 Halo...');
  ...
  it('shows the independent pet preload for the companion window', () => {
    ...
    const dom = new JSDOM(html, {
      url: 'http://localhost:1422/?haloWindow=agent-companion',
      runScripts: 'dangerously',
      beforeParse(window) {
        Object.assign(window, {
          __HALO_BOOTSTRAP_LOCALE__: 'en-US',
          __HALO_BOOTSTRAP_MESSAGES__: {
    ...
    expect(dom.window.document.body.classList.contains('halo-pet-preload-body')).toBe(true);
    expect(dom.window.document.getElementById('halo-startup-overlay')).toBeNull();
    expect(dom.window.document.querySelector('.halo-pet-preload__sprite')).not.toBeNull();
    expect(dom.window.document.querySelector('.splash-screen__logo')).toBeNull();
    expect(dom.window.document.querySelector('.halo-sr-only')?.textContent).toBe('Loading companion...');
    const spriteCss = html.match(/\.halo-pet-preload__sprite \{(?<css>[\s\S]*?)\n      \}/)?.groups?.css;
