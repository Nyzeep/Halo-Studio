...
function App() {
  ...
  useEffect(() => {
    ...
    void import('@/infrastructure/workbench-runtime').then(({ workbenchRuntimeStore }) => {
      ...
      log.warn('Failed to start Halo Workbench Runtime projection', error);
    });
  ...
  useEffect(() => {
    if (!isTauriRuntime() || isHaloLocalCodingScope() || !interactiveShellReady) return;
  ...
  useEffect(() => {
    if (!isTauriRuntime() || isHaloLocalCodingScope()) {
      return;
    }
