...
import workbenchSessionsSectionSource from '../../app/components/NavPanel/sections/sessions/WorkbenchSessionsSection.tsx?raw';
...
describe('Halo workbench formal path', () => {
  it('keeps transport details and secrets behind the infrastructure client', () => {
    ...
    expect(typesSource).toContain('pi-rpc-p0');
  });
  ...
  it('keeps public phase and error presentation behind semantic selectors and i18n', () => {
    expect(typesSource).toContain("'waitingDeveloper'");
    expect(typesSource).toContain("'interrupted'");
    expect(workbenchSessionSceneSource).toContain('selectWorkbenchRuntimePhaseMessageKey');
    expect(workbenchSessionSceneSource).toContain('selectWorkbenchRuntimeErrorMessageKey');
    expect(workbenchSessionSceneSource).not.toContain('runtimeState.stableErrorCode ??');
    expect(workbenchSessionSceneSource).not.toContain(
      ">{snapshot?.phase ?? 'disconnected'}",
    );
    expect(workbenchSessionsSectionSource).toContain(
      'selectWorkbenchRuntimeSessionNeedsDecision',
    );
    expect(selectorsSource).toContain("operation.phase === 'awaitingDecision'");
    expect(workbenchSessionsSectionSource).not.toContain(
      '.some(operation => operation.sessionId === session.sessionId)',
    );
  });
});
