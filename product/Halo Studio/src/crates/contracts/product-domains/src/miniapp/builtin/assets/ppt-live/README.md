...
PPT Live 是 Halo Studio 的内置 MiniApp，用于 AI 驱动的 PPT 生成、编辑和导出。

## Agentic MiniApp 样板间：复用悬浮会话气泡

PPT Live 同时是 Halo Studio **Agentic MiniApp** 的样板间：它自己**没有输入框，也没有
...
├── style.css               # 全局样式
├── ui.js                   # UI 入口 JS（build-halo.mjs 的打包入口）
├── worker.js               # 空文件（PPT Live 不使用 worker）
├── build-halo.mjs        # 唯一的构建脚本 → 产出 dist/ui.bundle.js
...
└── src/
    ...
    └── halo-backend-adapter.js # Halo Studio 后端适配器
```
...
| `build-halo.mjs` | ❌ 否（本身是构建工具） | ❌ 否 | 下次构建时自动生效 |
...
```bash
# 从 repo 根目录
node src/crates/contracts/product-domains/src/miniapp/builtin/assets/ppt-live/build-halo.mjs
...
4. cargo check -p halo-product-domains
5. 重启 pnpm run desktop:dev（或 touch builtin.rs 触发 Rust 重编译）让新 bundle 生效
```

### 构建原理

`build-halo.mjs` 使用 esbuild 从 `ui.js` 入口打包所有 `src/*.js` 和 npm 依赖
...
这些包在 `build-halo.mjs` 打包时从 `node_modules` 解析并内联到最终 bundle 中。
