...
fn static_hook_command_copies_only_referenced_assets_and_marks_external_paths() {
    ...
    assert!(prepared
        .command
        .contains("__HALO_MANAGED_HOOK_ROOT__/hooks/check.py"));
    assert_eq!(
    ...
    assert!(prepared.dependencies.iter().any(|dependency| matches!(
        dependency,
        halo_product_domains::external_hook_import::ExternalHookImportDependencyV1::Managed { relative_path }
            if relative_path == "hooks/check.py"
    )));
    assert!(prepared.dependencies.iter().any(|dependency| matches!(
        dependency,
        halo_product_domains::external_hook_import::ExternalHookImportDependencyV1::External { location }
