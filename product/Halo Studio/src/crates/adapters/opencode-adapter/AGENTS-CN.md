...
- 当前已将 OpenCode Command、standalone Tool 和 Subagent 的标准配置与目录作为只读实时来源；完整插件目录和
  package spec 仍是后续目标，不是可执行的生产来源。源文件无需导入 Halo Studio。低风险声明式结果按用户的自动应用/
...
- `ExternalSourceControlPlane` 负责候选版本，并通过现有来源协调模块替换 provider；本 adapter 通过窄 provider 契约提供 OpenCode 限定的来源
  身份/顺序和观察根，可复用文件观察服务只提供变化事实。配置归属模块提供规范化配置快照，`ScriptToolRuntime`
...
- 依赖 `halo-runtime-ports` 等稳定接口和 `PluginRuntimeAdapter` 边界 trait，不依赖
  `halo-core`、app crate、Tauri API、产品界面或具体服务管理器。
- OpenCode 配置 JSON、来源顺序、加载器兼容和参数展开保留在本 crate 内。跨 crate 输出使用类型化来源快照、
  adapter 绑定和 `PluginRuntimeClient` DTO，不得把 OpenCode 原始 JSON 或源码语法暴露为产品接口。
...
- Product Assembly 只允许从经过评审的组装模块（如 `halo-core/plugin_runtime` 或
  `halo-core/external_sources`）消费本 crate；增加其他消费方时必须同步边界脚本和聚焦组装路径测试。
- 本 crate 不得依赖 Codex、Claude Code 或其他生态 adapter。新生态是由 Product Assembly 注册的同级 adapter，
  不是本 adapter 的模式。
- 生产 crate 不得直接依赖 `halo_opencode_adapter` 内部类型。未支持能力必须诊断化，
  不得因外部插件内容导致运行时崩溃。
