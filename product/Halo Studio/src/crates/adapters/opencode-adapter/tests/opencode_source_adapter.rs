...
impl ManagedPackageFixture {
    ...
    fn new_with_path(name: &str, source: &str, relative_path: &str) -> Self {
        ...
        let root = std::env::temp_dir().join(format!(
            "halo-opencode-managed-{name}-{}-{nonce}",
            process::id()
        ));
    ...
    fn add_declared_file(&self, relative_path: &str, content: &[u8]) {
        ...
        let manifest_path = self.package.join("halo.plugin.json");
    ...
    async fn approved_input(&self) -> halo_product_domains::plugin_source::PluginPackageInput {
...
async fn activated_package_projects_permission_required_candidate_through_client() {
    ...
    assert!(response
        .effects
        .iter()
        .all(|effect| effect.risk_level == halo_runtime_ports::PluginRiskLevel::High));
}
...
async fn managed_source_uri_encodes_reserved_path_characters() {
    ...
    assert!(source.starts_with("halo://managed-plugins/"));
...
fn dispatch_envelope(source: halo_runtime_ports::PluginSourceRef) -> PluginDispatchEnvelope {
    dispatch_envelope_for(source, "project-1", "workspace-1", 20)
}

fn dispatch_envelope_for(
    source: halo_runtime_ports::PluginSourceRef,
    project_domain_id: &str,
    workspace_id: &str,
    trust_epoch: u64,
