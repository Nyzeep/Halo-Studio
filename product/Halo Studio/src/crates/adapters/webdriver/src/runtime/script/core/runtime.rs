pub(super) fn script() -> &'static str {
    ...
    const EVENT_NAME = "halo_webdriver_result";
    const STORE_KEY = "__haloWdElements";
    const LOG_KEY = "__haloWdLogs";
    const consolePatchedKey = "__haloWdConsolePatched";
    ...
    const ensureRuntimeState = () => {
      if (!window.__haloWdRuntimeState) {
        window.__haloWdRuntimeState = {
          pointer: {
            x: 0,
            y: 0,
            target: null,
      ...
      return window.__haloWdRuntimeState;
    };
    ...
    const ensureAlertState = (targetWindow = window) => {
      if (!targetWindow.__haloWdAlertState) {
        targetWindow.__haloWdAlertState = {
          open: false,
          type: null,
          text: "",
          defaultValue: null,
      ...
      return targetWindow.__haloWdAlertState;
    };

    const patchDialogs = (targetWindow = window) => {
      const patchedKey = "__haloWdDialogsPatched";
