...
mod imp {
    ...
    pub(super) async fn print_page<R: Runtime>(
        ...
        let css = format!(
            r#"(function() {{
                let style = document.getElementById('__halo_webdriver_print_style');
                if (!style) {{
                    style = document.createElement('style');
                    style.id = '__halo_webdriver_print_style';
        ...
        let _ = webview.eval(
            "(() => { document.getElementById('__halo_webdriver_print_style')?.remove(); })();",
        );
        response
    }
