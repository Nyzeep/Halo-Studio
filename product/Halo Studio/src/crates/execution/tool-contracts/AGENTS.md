...
`halo-agent-tools` owns portable tool contracts. It must stay independent of
the product tool runtime.

## Guardrails

- Do not depend on `halo-core`, concrete service crates,
...
- ACP protocol/client lifecycle stays in `halo-acp`; this crate owns only the
  external-agent tool bridge contract.

## Verification

```bash
cargo test -p halo-agent-tools
cargo test -p halo-agent-tools --test tool_contracts
