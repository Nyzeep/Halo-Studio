...
use halo_agent_runtime::prompt::UserContextPolicy;
use std::fs;
...
fn custom_mode_markdown_save_omits_default_fields() {
    let dir = TestTempDir::new("halo-runtime-custom-mode-defaults");
...
fn custom_mode_markdown_save_round_trips_custom_policy_and_model() {
    let dir = TestTempDir::new("halo-runtime-custom-mode-custom");
    let path = dir.join("planner.md");
...
fn custom_mode_markdown_save_round_trips_empty_custom_policy() {
    let dir = TestTempDir::new("halo-runtime-custom-mode-empty-policy");
    let path = dir.join("planner.md");
...
fn custom_mode_discovery_rejects_project_scoped_modes_without_dropping_valid_agents() {
    let workspace = TestTempDir::new("halo-runtime-custom-mode-workspace");
    let user_root = TestTempDir::new("halo-runtime-custom-mode-user");
    let project_agents_dir = workspace.path.join(".halo-studio").join("agents");
    ...
    let report = load_custom_agent_definitions(&CustomAgentDiscoveryRoots {
        ...
        halo_user_agents_dir: Some(user_agents_dir),
        home_dir: None,
    });
...
#[test]
fn custom_agent_discovery_ignores_non_halo_agent_dirs() {
    let workspace = TestTempDir::new("halo-runtime-custom-agent-workspace");
    let user_root = TestTempDir::new("halo-runtime-custom-agent-user");
    let home = TestTempDir::new("halo-runtime-custom-agent-home");
    ...
    write_subagent(
        &project_halo.join("project-halo.md"),
        "HaloProject",
        CustomAgentLevel::Project,
    );
    write_subagent(
    ...
    let roots = CustomAgentDiscoveryRoots {
        workspace_root: Some(workspace.path.clone()),
        halo_user_agents_dir: Some(user_agents.clone()),
    ...
    assert_eq!(
        custom_agent_possible_dirs(&roots)
            ...
            .collect::<Vec<_>>(),
        vec![project_halo.as_path(), user_agents.as_path()]
    );
