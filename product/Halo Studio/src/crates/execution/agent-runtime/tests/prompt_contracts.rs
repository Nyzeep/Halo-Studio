...
use halo_core_types::{SessionExecutionTarget, SessionExecutionTargetKind, WorktreeLifecycle};
...
fn prompt_environment_info_preserves_local_and_remote_guidance() {
    ...
    assert!(remote.contains("- Local Halo client OS: linux (unix)"));
...
fn runtime_context_renderer_preserves_local_exec_and_computer_use_guidance() {
    ...
    assert!(reminder.contains("- Local Halo client OS: windows (windows)"));
...
fn runtime_context_renderer_preserves_remote_workspace_split() {
    ...
    assert!(reminder.contains("Computer use and UI automation operate on the local Halo desktop"));
}
...
fn workspace_and_user_context_renderers_preserve_section_shape() {
    ...
    let managed_worktree = render_workspace_context(&WorkspaceContextFacts {
        workspace_path: "/managed/Halo-wt-1".to_string(),
        related_paths: Vec::new(),
        remote_execution: None,
        worktree: Some(WorktreeContextFacts {
            project_workspace_path: "/projects/Halo".to_string(),
    ...
    assert!(managed_worktree.contains("/projects/Halo"));
