use halo_runtime_ports::DialogTriggerSource;
use std::path::Path;
...
mod tests {
    ...
    fn remote_delivery_links_use_computer_scheme() {
        assert_eq!(
            user_workspace_relative_file_link(r".halo-studio\sessions\s1\research\report.md", true),
            "computer://.halo-studio/sessions/s1/research/report.md"
        );
    }
}
