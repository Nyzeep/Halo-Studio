...
//!
//! Halo reads the same `hooks.json` document shape as Codex:
//!
//! ```json
//! {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            AgentHookSettingsIssue::HandlerUnsupported {
                ...
                f,
                "Hook handler type '{handler_type}' under '{event}' is recognized but not executable by Halo; only 'command' handlers run: {source}"
            ),
...
fn parse_handler(handler: &Value) -> ParsedHandler {
    ...
    match handler_type {
        ...
        // native command handlers; Halo skips them the same way.
