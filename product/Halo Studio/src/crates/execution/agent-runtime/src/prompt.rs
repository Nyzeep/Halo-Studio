...
use halo_core_types::{SessionExecutionTarget, SessionExecutionTargetKind};
...
<environment_details>
- Local Halo client OS: {} ({}) — applies to Computer use / UI automation on this machine only.
...
pub struct RuntimeContextNeeds {
    ...
    /// the prominent "Local Halo client OS" block that belongs to desktop
    /// automation.
    ...
    if facts.needs.computer_use {
        ...
        if facts.remote_execution.is_some() && facts.needs.workspace_tools {
            local_client_lines.push(
                "- Computer use and UI automation operate on the local Halo desktop, even when workspace file and shell tools target a remote host."
                    .to_string(),
            );
        }
        local_client_lines.push(format!(
            "- Local Halo client OS: {} ({})",
...
fn runtime_computer_use_key_chord_guidance(host_os: &str) -> &'static str {
    match host_os {
        "macos" => "- Computer use / `key_chord`: the local Halo desktop is macOS. Use `command`, `option`, `control`, and `shift` modifier names.",
        "windows" => "- Computer use / `key_chord`: the local Halo desktop is Windows. Use `meta`/`super` for the Windows key, plus `alt`, `control`, and `shift`.",
        "linux" => "- Computer use / `key_chord`: the local Halo desktop is Linux. Use `control`, `alt`, `shift`, and `meta`/`super` as appropriate for the desktop environment.",
        _ => "- Computer use / `key_chord`: match modifier names to the local Halo desktop OS.",
    }
}

fn computer_use_key_chord_guidance(host_os: &str) -> &'static str {
    match host_os {
        "macos" => "Computer use / `key_chord`: the **local Halo desktop** is **macOS** — use `command`, `option`, `control`, `shift` (not Win/Linux modifier names). **ACTION PRIORITY:** 1) Terminal/CLI/system commands (use ExecCommand for `osascript`, AppleScript, shell scripts) 2) Keyboard shortcuts: command+a/c/x/v (clipboard), command+space (Spotlight), command+tab (switch app) 3) UI control (AX/OCR/mouse) only when above fail.",
        "windows" => "Computer use / `key_chord`: the **local Halo desktop** is **Windows** — use `meta`/`super` for Windows key, `alt`, `control`, `shift`. **ACTION PRIORITY:** 1) Terminal/CLI/system commands (use ExecCommand for PowerShell, cmd, scripts) 2) Keyboard shortcuts: control+a/c/x/v (clipboard), meta (Start menu), Alt+Tab (switch) 3) UI control only when above fail.",
        "linux" => "Computer use / `key_chord`: the **local Halo desktop** is **Linux** — typically `control`, `alt`, `shift`, and sometimes `meta`/`super`. **ACTION PRIORITY:** 1) Terminal/CLI/system commands (use ExecCommand for shell scripts and system commands) 2) Keyboard shortcuts: control+a/c/x/v (clipboard) 3) UI control (AX/OCR/mouse) only when above fail.",
        _ => "Computer use / `key_chord`: match modifier names to the **local Halo desktop** OS below. **ACTION PRIORITY:** 1) Terminal/CLI/system commands first 2) Keyboard shortcuts second 3) UI control (mouse/OCR) last resort.",
    }
}
