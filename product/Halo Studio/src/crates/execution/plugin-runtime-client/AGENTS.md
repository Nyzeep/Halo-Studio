...
`halo-plugin-runtime-client` is the default implementation of the existing,
...
- Depend only on stable contracts such as `halo-runtime-ports`.
- Do not depend on `halo-core`, product assembly, app crates, Tauri, concrete
  services, concrete adapters, `halo-opencode-adapter`, or UI code.
