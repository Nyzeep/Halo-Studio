...
`halo-agent-stream` owns provider-neutral stream DTOs, tool-call accumulation,
...
- Do not depend on `halo-core`, app crates, Tauri, concrete services,
  transport adapters, AI adapters, terminal, tool-runtime, or product-domain
  implementations.
- Keep provider-specific SSE or response parsing in `halo-ai-adapters`; this
...
```bash
cargo test -p halo-agent-stream
node scripts/check-core-boundaries.mjs
```

When provider fixture parsing changes, run the focused `halo-ai-adapters`
