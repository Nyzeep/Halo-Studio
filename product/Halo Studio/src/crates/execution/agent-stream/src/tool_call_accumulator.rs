    ...
    pub fn finalize_with_options(
        ...
        let (arguments, is_error, repair_kind, parse_error) = match parsed_arguments {
            ...
            Err(parse_err) => {
                ...
                {
                    let repaired =
                        halo_tool_call_jsonrepair::repair_tool_call_json(&raw_arguments)
                            .ok()
                            .and_then(|candidate| {
...
mod tests {
    ...
    fn does_not_repair_incomplete_json_object_for_multifield_tools() {
        ...
        pending.append_arguments(
            "{\"description\":\"Explore Halo project structure\",\"prompt\":\"read README\\n\\nthoroughness: very",
        );
    ...
    fn write_like_recovery_classification_matches_tool_presentation_contract() {
        for tool_name in [
            ...
            assert_eq!(
                ...
                halo_agent_tools::is_write_like_tool_name(tool_name),
                "tool_name={tool_name}"
            );
        }
    }
