...
mod tests {
    ...
    fn streams_hidden_tag_across_chunk_boundaries() {
        ...
        let first = parser.push_str("hello <halo-mem-");
        let second = parser.push_str("citation>doc</halo-mem-citation> world");
    ...
    fn auto_closes_unterminated_hidden_tag_at_finish() {
        ...
        let first = parser.push_str("x<halo-mem-citation>doc");
        let tail = parser.finish();
    ...
    fn preserves_partial_open_tag_at_finish_when_not_complete() {
        ...
        assert_eq!(first.visible_text, "hello ");
        assert_eq!(tail.visible_text, "<halo-mem-");
