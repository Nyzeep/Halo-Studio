...
/// How Halo enters a configured container workspace.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
...
pub struct ContainerWorkspaceConfig {
    ...
    /// Run Docker on the local Halo machine instead of an SSH host.
    #[serde(default)]
    pub local: bool,
...
pub struct RemoteWorkspace {
    ...
    #[serde(default)]
    pub connection_name: String,
    /// SSH config `host`; used for `~/.halo-studio/remote_ssh/{host}/...` session storage.
    #[serde(default)]
    pub ssh_host: String,
}
