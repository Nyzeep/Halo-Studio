(() => {
  const root = document.getElementById('halo-canvas-root');
  let theme = makeTheme({
    type: 'auto',
  ...
  function applyHostTheme(nextTheme) {
    ...
    theme = makeTheme({
      ...theme,
      ...Object.fromEntries(allowed.map(key => [key, getComputedStyle(document.documentElement).getPropertyValue(`--halo-canvas-${key}`).trim() || theme[key]])),
      type,
    });
  }
  const state = new Map();
  ...
  function markCanvasNode(value, component) {
    for (const item of toArray(value)) {
      if (item instanceof Element) {
        if (!item.dataset.haloCanvasNode) item.dataset.haloCanvasNode = `node-${++nodeSeq}`;
        if (!item.dataset.haloCanvasComponent) item.dataset.haloCanvasComponent = component;
      }
    }
    return value;
  }
  ...
  function elementReference(element) {
    ...
    return {
      nodeId: element.dataset.haloCanvasNode || null,
      component: element.dataset.haloCanvasComponent || element.tagName.toLowerCase(),
  ...
  function inspectableElement(target) {
    ...
    return target.closest('[data-halo-canvas-node]') || target;
  }
  ...
  function highlightInspectElement(element) {
    ...
    element.style.outline = '2px solid var(--halo-canvas-accent)';
    element.style.outlineOffset = '2px';
    element.style.cursor = 'crosshair';
  }
  function setDesignMode(enabled) {
    designMode = Boolean(enabled);
    document.body.dataset.haloCanvasDesignMode = designMode ? 'true' : 'false';
  ...
  function rerender() {
    ...
    try {
      ...
      if (!readySent) {
        readySent = true;
        window.parent?.postMessage({ type: 'halo-canvas-ready' }, '*');
      }
    } catch (error) {
  ...
  function errorView(error) {
    return el('main', { style: { maxWidth: '860px', margin: '0 auto', padding: '12px', border: '1px solid var(--halo-canvas-border)', borderRadius: '8px' } }, [
      el('h1', { style: { margin: '0 0 8px', fontSize: '18px' } }, ['Canvas runtime error']),
      el('pre', { style: { whiteSpace: 'pre-wrap', margin: 0, color: 'var(--halo-canvas-danger)' } }, [String(error?.stack || error?.message || error)])
    ]);
  }
  ...
  const categoryPaletteLight = {
    gray: 'var(--halo-canvas-muted)',
    purple: 'var(--halo-canvas-accent)',
    green: 'var(--halo-canvas-success)',
    yellow: 'var(--halo-canvas-warning)',
    cyan: 'var(--halo-canvas-info)',
    pink: 'var(--halo-canvas-danger)',
    blue: 'var(--halo-canvas-accent)',
    orange: 'var(--halo-canvas-warning)',
  };
  const categoryPaletteDark = categoryPaletteLight;
  const canvasTokensLight = {
    bg: 'var(--halo-canvas-panel)',
    panel: 'var(--halo-canvas-bg)',
    elevated: 'var(--halo-canvas-bg)',
    chrome: 'var(--halo-canvas-panel)',
    text: 'var(--halo-canvas-fg)',
    textSecondary: 'var(--halo-canvas-muted)',
    textMuted: 'var(--halo-canvas-muted)',
    border: 'var(--halo-canvas-border)',
    accent: 'var(--halo-canvas-accent)',
    success: 'var(--halo-canvas-success)',
    warning: 'var(--halo-canvas-warning)',
  ...
  const Link = ({ children, href, style } = {}) => el('a', { href, target: '_blank', rel: 'noreferrer', style: { color: 'var(--halo-canvas-accent)', textDecoration: 'none', ...style } }, children);
  const Card = ({ children, variant = 'default', size = 'base', style, ...props } = {}) => el('section', {
    ...
    style: {
      ...
      background: variant === 'borderless' ? 'transparent' : 'var(--halo-canvas-bg)',
      overflow: 'hidden',
      ...style,
    }
  }, children);
  const CardHeader = ({ children, trailing, style } = {}) => el('header', { style: { minHeight: '34px', display: 'flex', alignItems: 'center', gap: '10px', justifyContent: 'space-between', padding: '9px 12px', borderBottom: '1px solid rgba(127,127,127,0.16)', fontSize: '12px', fontWeight: 650, lineHeight: 1.25, ...style } }, [
    ...
    trailing ? el('div', { style: { flexShrink: 0, color: 'var(--halo-canvas-muted)' } }, trailing) : null
  ]);
  const CardBody = ({ children, style } = {}) => el('div', { style: { padding: '12px', ...style } }, children);
  const Empty = ({ description = 'No data', children, style } = {}) => el('div', { style: { display: 'grid', placeItems: 'center', gap: '6px', minHeight: '96px', padding: '18px', border: '1px dashed rgba(127,127,127,0.24)', borderRadius: '8px', color: 'var(--halo-canvas-muted)', textAlign: 'center', ...style } }, [
  ...
  const Tabs = ({ items = [], children, activeKey, defaultActiveKey, onChange, style } = {}) => {
    ...
    return el('div', { style: { display: 'grid', gap: '10px', ...style } }, [
      list.length ? el('div', { role: 'tablist', style: { display: 'flex', gap: '6px', borderBottom: '1px solid rgba(127,127,127,0.16)' } }, list.map(item =>
        el('button', { role: 'tab', 'aria-selected': item.key === selected?.key, disabled: item.disabled, onClick: () => onChange?.(item.key), style: { border: 0, borderBottom: item.key === selected?.key ? '2px solid var(--halo-canvas-accent)' : '2px solid transparent', background: 'transparent', color: item.key === selected?.key ? 'var(--halo-canvas-fg)' : 'var(--halo-canvas-muted)', padding: '6px 8px', font: 'inherit', cursor: item.disabled ? 'default' : 'pointer' } }, [item.label])
      )) : null,
  ...
  const Alert = ({ children, type = 'info', tone, title, message, description, showIcon = true, style } = {}) => {
    ...
    return el('div', { role: 'alert', 'aria-live': type === 'error' ? 'assertive' : 'polite', style: { display: 'grid', gridTemplateColumns: showIcon ? '18px minmax(0, 1fr)' : 'minmax(0, 1fr)', gap: '9px', border: '1px solid rgba(127,127,127,0.20)', borderLeft: `3px solid ${color}`, borderRadius: '8px', padding: '10px 12px', background: 'rgba(127,127,127,0.04)', ...style } }, [
      ...
      el('span', { style: { minWidth: 0, display: 'grid', gap: '3px' } }, [
        title ? el('strong', { style: { color: 'var(--halo-canvas-fg)', fontSize: '13px', lineHeight: 1.35 } }, [title]) : null,
        message || children ? el('span', { style: { color: 'var(--halo-canvas-muted)', fontSize: '12px', overflowWrap: 'anywhere' } }, [message ?? children]) : null,
        description ? el('span', { style: { color: 'var(--halo-canvas-muted)', fontSize: '12px', overflowWrap: 'anywhere' } }, [description]) : null,
      ]),
    ]);
  };
  ...
  const Pill = ({ children, active = false, size = 'md', leadingContent, keyboardHint, disabled, title, onClick, style } = {}) => {
    ...
    return el(tag, { title, disabled, onClick, style: { display: 'inline-flex', alignItems: 'center', gap: '5px', border: compact ? '0' : '1px solid rgba(127,127,127,0.22)', borderRadius: '999px', padding: compact ? '1px 6px' : '2px 8px', background: active ? 'rgba(52,120,246,0.16)' : 'rgba(127,127,127,0.05)', color: 'var(--halo-canvas-fg)', font: 'inherit', fontSize: compact ? '11px' : '12px', lineHeight: '18px', cursor: isButton && !disabled ? 'pointer' : 'default', opacity: disabled ? 0.55 : 1, ...style } }, [leadingContent, children, keyboardHint ? el('span', { style: { color: 'var(--halo-canvas-muted)', marginLeft: 2 } }, [keyboardHint]) : null]);
  };
  ...
  function CollapsibleSection({ title, leading, count, trailing, children, defaultOpen = false, style } = {}) {
    ...
    return el('section', { style: { ...style } }, [
      el('button', { onClick: () => setOpen(!open), style: { width: '100%', minHeight: '28px', display: 'flex', alignItems: 'center', gap: '7px', border: 0, padding: '4px 0', background: 'transparent', color: 'var(--halo-canvas-fg)', font: 'inherit', cursor: 'pointer', textAlign: 'left' } }, [
        Chevron({ expanded: open }),
        leading || null,
        el('span', { style: { fontSize: '13px', fontWeight: 650, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, [title]),
        count !== undefined ? el('span', { style: { color: 'var(--halo-canvas-muted)', fontSize: '12px' } }, [String(count)]) : null,
        el('span', { style: { flex: 1 } }),
        trailing ? el('span', { style: { color: 'var(--halo-canvas-muted)', fontSize: '12px', flexShrink: 0 } }, trailing) : null,
      ]),
  ...
  const Stat = ({ value, label, tone, style } = {}) => el('div', { style: { display: 'grid', gap: '2px', ...style } }, [el('strong', { style: { color: toneColor(tone), fontSize: '22px', lineHeight: 1.1, fontVariantNumeric: 'tabular-nums' } }, [value]), el('span', { style: { color: 'var(--halo-canvas-muted)', fontSize: '12px' } }, [label])]);
  const Table = ({ headers = [], rows = [], columnAlign = [], rowTone = [], framed = true, striped = false, stickyHeader = false, style, emptyMessage = 'No rows' } = {}) => {
    ...
    }))) : [el('tr', {}, [el('td', { colspan: headers.length || 1, style: { ...cellStyle(false), color: 'var(--halo-canvas-muted)' } }, [emptyMessage])])];
    return el('div', { style: { overflow: 'auto', border: framed ? '1px solid rgba(127,127,127,0.20)' : 0, borderRadius: framed ? '8px' : 0, background: 'var(--halo-canvas-bg)', ...style } }, [el('table', { style: { width: '100%', borderCollapse: 'collapse', fontSize: '12px' } }, [
      el('thead', {}, [el('tr', {}, headers.map((h, index) => el('th', { style: { ...cellStyle(true, columnAlign[index]), position: stickyHeader ? 'sticky' : undefined, top: stickyHeader ? 0 : undefined, background: 'var(--halo-canvas-panel)' } }, [h])))]),
      el('tbody', {}, bodyRows)
    ])]);
  };
  const KeyValueList = ({ items = [], columns = 1, compact = false, emptyMessage = 'No details', style } = {}) => {
    ...
    return el('dl', { style: { display: 'grid', gridTemplateColumns: `repeat(${columnCount}, minmax(0, 1fr))`, gap: compact ? '6px' : '10px', margin: 0, ...style } }, entries.length ? entries.map((item, index) => el('div', { key: item.key || index, style: { minWidth: 0, padding: compact ? '0 0 6px' : '8px 0', borderBottom: '1px solid rgba(127,127,127,0.16)' } }, [
      el('dt', { style: { margin: 0, color: 'var(--halo-canvas-muted)', fontSize: '11px', lineHeight: 1.35 } }, [item.label]),
    ...
    ])) : [el('div', { style: { color: 'var(--halo-canvas-muted)', fontSize: '12px' } }, [emptyMessage])]);
  };
  const Timeline = ({ items = [], emptyMessage = 'No events', style } = {}) => el('ol', { style: { display: 'grid', gap: '10px', margin: 0, padding: 0, listStyle: 'none', ...style } }, items.length ? items.map((item, index) => el('li', { key: item.key || index, style: { display: 'grid', gridTemplateColumns: '18px minmax(0, 1fr)', gap: '9px', minWidth: 0 } }, [
    ...
    el('span', { style: { minWidth: 0, display: 'grid', gap: '2px' } }, [
      el('span', { style: { display: 'flex', gap: '8px', alignItems: 'baseline', justifyContent: 'space-between', minWidth: 0 } }, [
        el('strong', { style: { minWidth: 0, color: 'var(--halo-canvas-fg)', fontSize: '13px' } }, [item.title]),
        item.time ? el('time', { style: { flex: '0 0 auto', color: 'var(--halo-canvas-muted)', fontSize: '11px' } }, [item.time]) : null,
      ]),
      item.description ? el('span', { style: { color: 'var(--halo-canvas-muted)', fontSize: '12px', overflowWrap: 'anywhere' } }, [item.description]) : null,
    ]),
  ])) : [el('li', { style: { color: 'var(--halo-canvas-muted)', fontSize: '12px' } }, [emptyMessage])]);
  ...
  function renderFileTreeItems(items, depth, defaultExpanded) {
    return (items || []).map((item, index) => {
      ...
      const row = el('span', { style: { display: 'flex', alignItems: 'center', gap: '7px', minWidth: 0, padding: '3px 0', paddingLeft: `${depth * 16}px` } }, [
        el('span', { style: { flex: '0 0 auto', width: 14, color: isFolder ? 'var(--halo-canvas-accent)' : 'var(--halo-canvas-muted)' } }, [isFolder ? '▸' : '•']),
        ...
        item.meta ? el('span', { style: { flex: '0 0 auto', marginLeft: 'auto', color: 'var(--halo-canvas-muted)', fontSize: '11px' } }, [item.meta]) : null,
      ]);
  ...
  const Swatch = ({ color = 'gray', style, title } = {}) => el('span', {
    ...
    style: {
      ...
      border: '1px solid var(--halo-canvas-border)',
      flex: '0 0 auto',
      ...style,
    },
  });
  ...
  const UsageBar = ({ segments = [], total = 0, topLeftLabel, topRightLabel, style } = {}) => {
    ...
    return el('div', { style }, [
      topLeftLabel || topRightLabel ? el('div', { style: { display: 'flex', justifyContent: 'space-between', gap: 12, marginBottom: 6, color: 'var(--halo-canvas-muted)', fontSize: '12px', lineHeight: 1.35 } }, [
  ...
  function todoStatusColor(status) {
    if (status === 'completed') return 'var(--halo-canvas-success)';
    if (status === 'in_progress') return 'var(--halo-canvas-warning)';
    return 'var(--halo-canvas-muted)';
  }
    ...
    return el('div', { style: { display: 'grid', gap: 4, ...style } }, list.map(todo => {
      ...
      const body = [
        ...
        el('span', { style: { minWidth: 0, display: 'grid', gap: 2 } }, [
          el('span', { style: { color: todo.status === 'completed' ? 'var(--halo-canvas-muted)' : 'var(--halo-canvas-fg)', fontSize: '12px', lineHeight: 1.45, textDecoration: todo.status === 'completed' ? 'line-through' : undefined, overflowWrap: 'anywhere' } }, [content]),
  ...
  const TodoListCard = ({ todos = [], dimmedTodoIds, defaultExpanded = false, onTodoClick, style } = {}) => {
    ...
    return el('section', { style: { border: '1px solid var(--halo-canvas-border)', borderRadius: '8px', background: 'var(--halo-canvas-bg)', overflow: 'hidden', ...style } }, [
      el('button', { type: 'button', 'aria-expanded': open, onClick: () => setOpen(!open), style: { width: '100%', minHeight: 34, display: 'flex', alignItems: 'center', gap: 8, border: 0, borderBottom: open ? '1px solid var(--halo-canvas-border)' : 0, background: 'transparent', color: 'var(--halo-canvas-fg)', padding: '8px 10px', font: 'inherit', cursor: 'pointer', textAlign: 'left' } }, [
        el('span', { 'aria-hidden': true, style: { color: 'var(--halo-canvas-muted)', transform: open ? 'rotate(90deg)' : 'rotate(0deg)' } }, ['›']),
        el('span', { style: { fontWeight: 650, fontSize: '12px' } }, ['Tasks']),
        el('span', { style: { marginLeft: 'auto', color: 'var(--halo-canvas-muted)', fontSize: '12px' } }, [`${completed}/${list.length} done`]),
      ]),
  ...
  const Button = ({ children, variant = 'secondary', onClick, disabled, type = 'button', style } = {}) => {
    ...
    return el('button', { type, onClick, disabled, style: { border: ghost ? '1px solid transparent' : '1px solid rgba(127,127,127,0.22)', borderRadius: '6px', background: primary ? 'var(--halo-canvas-accent)' : ghost ? 'transparent' : 'rgba(127,127,127,0.06)', color: primary ? '#fff' : 'var(--halo-canvas-fg)', padding: '4px 10px', minHeight: '24px', font: 'inherit', fontSize: '12px', cursor: disabled ? 'default' : 'pointer', opacity: disabled ? 0.55 : 1, ...style } }, children);
  };
  const Toggle = ({ checked, onChange, label, disabled, size = 'sm', style } = {}) => {
    const width = size === 'md' ? 34 : 28;
    const height = size === 'md' ? 20 : 16;
    const knob = height - 4;
    return el('button', { disabled, onClick: () => onChange?.(!checked), style: { width, height, border: 0, borderRadius: 999, background: checked ? 'var(--halo-canvas-accent)' : 'rgba(127,127,127,0.20)', padding: 2, cursor: disabled ? 'default' : 'pointer', opacity: disabled ? 0.55 : 1, ...style } }, [
  ...
  const Checkbox = ({ checked, onChange, disabled, label, style } = {}) => el('label', { style: { display: 'inline-flex', gap: '6px', alignItems: 'center', fontSize: '12px', opacity: disabled ? 0.55 : 1, ...style } }, [el('input', { type: 'checkbox', checked, disabled, onChange: event => onChange?.(event.target.checked), style: { accentColor: 'var(--halo-canvas-accent)' } }), label]);
  const Select = ({ value, options = [], placeholder, disabled, onChange, style } = {}) => el('select', { value, disabled, onChange: event => onChange?.(event.target.value), style: { border: '1px solid rgba(127,127,127,0.22)', borderRadius: '6px', minHeight: '28px', padding: '4px 8px', background: 'var(--halo-canvas-panel)', color: 'var(--halo-canvas-fg)', ...style } }, [placeholder ? el('option', { value: '' }, [placeholder]) : null, ...options.map(option => typeof option === 'string' ? el('option', { value: option }, [option]) : el('option', { value: option.value, disabled: option.disabled }, [option.label]))]);
  const TextInput = ({ value, placeholder, disabled, type = 'text', onChange, style } = {}) => el('input', { value, placeholder, disabled, type, onInput: event => onChange?.(event.target.value), style: { border: '1px solid rgba(127,127,127,0.22)', borderRadius: '6px', minHeight: '28px', padding: '4px 8px', background: 'var(--halo-canvas-panel)', color: 'var(--halo-canvas-fg)', ...style } });
  const Input = ({ value, placeholder, disabled, type = 'text', onChange, label, hint, prefix, suffix, error, errorMessage, style } = {}) => el('label', { style: { display: 'grid', gap: '5px', color: 'var(--halo-canvas-fg)', fontSize: '12px', ...style } }, [
    label ? el('span', { style: { fontWeight: 600 } }, [label]) : null,
    el('span', { style: { display: 'flex', alignItems: 'center', gap: '6px', border: `1px solid ${error ? 'var(--halo-canvas-danger)' : 'rgba(127,127,127,0.22)'}`, borderRadius: '6px', minHeight: '30px', padding: '0 8px', background: 'var(--halo-canvas-panel)' } }, [
      prefix || null,
      el('input', { value, placeholder, disabled, type, onInput: event => onChange?.(event.target.value), style: { flex: 1, minWidth: 0, border: 0, outline: 0, background: 'transparent', color: 'var(--halo-canvas-fg)', font: 'inherit' } }),
      suffix || null,
    ]),
    error && errorMessage ? el('span', { style: { color: 'var(--halo-canvas-danger)' } }, [errorMessage]) : hint ? el('span', { style: { color: 'var(--halo-canvas-muted)' } }, [hint]) : null,
  ]);
  const TextArea = ({ value, placeholder, disabled, rows = 3, onChange, style } = {}) => el('textarea', { value, placeholder, disabled, rows, onInput: event => onChange?.(event.target.value), style: { border: '1px solid rgba(127,127,127,0.22)', borderRadius: '6px', padding: '7px 8px', background: 'var(--halo-canvas-panel)', color: 'var(--halo-canvas-fg)', font: 'inherit', fontSize: '13px', resize: 'vertical', width: '100%', boxSizing: 'border-box', ...style } });
  const IconButton = ({ children, onClick, disabled, title, variant = 'default', size = 'md', style } = {}) => {
    const px = size === 'sm' ? 18 : 24;
    return el('button', { title, onClick, disabled, style: { width: px, height: px, display: 'inline-grid', placeItems: 'center', border: 0, borderRadius: variant === 'circle' ? 999 : 5, background: variant === 'circle' ? 'rgba(127,127,127,0.12)' : 'transparent', color: 'var(--halo-canvas-muted)', cursor: disabled ? 'default' : 'pointer', opacity: disabled ? 0.55 : 1, ...style } }, children);
  };
  const DiffStats = ({ additions = 0, deletions = 0, style } = {}) => {
    ...
    return el('span', { style: { display: 'inline-flex', gap: '6px', alignItems: 'center', fontSize: '12px', fontVariantNumeric: 'tabular-nums', ...style } }, [
      additions ? el('span', { style: { color: 'var(--halo-canvas-success)' } }, [`+${additions}`]) : null,
      deletions ? el('span', { style: { color: 'var(--halo-canvas-danger)' } }, [`-${deletions}`]) : null,
    ]);
  };
  function diffLineStyle(type) {
    if (type === 'added') return { background: 'rgba(36,138,61,0.12)', color: 'var(--halo-canvas-fg)', accent: 'var(--halo-canvas-success)', sign: '+' };
    if (type === 'removed') return { background: 'rgba(209,36,47,0.12)', color: 'var(--halo-canvas-fg)', accent: 'var(--halo-canvas-danger)', sign: '-' };
    return { background: 'transparent', color: 'var(--halo-canvas-fg)', accent: 'transparent', sign: ' ' };
  }
  const DiffView = ({ lines = [], showLineNumbers = true, coloredLineNumbers = true, showAccentStrip = true, style } = {}) => el('div', { style: { overflow: 'auto', fontFamily: 'ui-monospace,SFMono-Regular,Menlo,monospace', fontSize: '12px', lineHeight: 1.55, background: 'rgba(127,127,127,0.035)', ...style } }, lines.map((line, index) => {
    ...
    return el('div', { style: { display: 'grid', gridTemplateColumns: `${showAccentStrip ? '3px ' : ''}${showLineNumbers ? '52px ' : ''}18px minmax(0,1fr)`, minWidth: '100%', background: meta.background, color: meta.color, whiteSpace: 'pre' } }, [
      ...
      showLineNumbers ? el('span', { style: { color: coloredLineNumbers && line.type !== 'unchanged' ? meta.accent : 'var(--halo-canvas-muted)', textAlign: 'right', padding: '0 8px', userSelect: 'none' } }, [line.lineNumber ?? index + 1]) : null,
      el('span', { style: { color: meta.accent === 'transparent' ? 'var(--halo-canvas-muted)' : meta.accent, userSelect: 'none' } }, [meta.sign]),
  ...
  function chartShell(title, height, style, child) {
    return el('div', { style: { border: '1px solid var(--halo-canvas-border)', borderRadius: '8px', padding: '10px', background: 'rgba(127,127,127,0.04)', ...style } }, [
      title ? el('div', { style: { fontWeight: 650, marginBottom: '8px' } }, [title]) : null,
      child || el('div', { style: { minHeight: `${height}px`, display: 'grid', placeItems: 'center', color: 'var(--halo-canvas-muted)' } }, ['No chart data'])
    ]);
  }
  function chartLegend(series) {
    return el('div', { style: { display: 'flex', gap: '10px', flexWrap: 'wrap', marginTop: '8px', color: 'var(--halo-canvas-muted)', fontSize: '12px' } }, series.map(entry => el('span', { style: { display: 'inline-flex', alignItems: 'center', gap: '5px' } }, [
  ...
  function BarChart(props = {}) {
    ...
    const axis = [
      svg('line', { x1: padding.left, y1: padding.top + innerHeight, x2: width - padding.right, y2: padding.top + innerHeight, stroke: 'var(--halo-canvas-border)' }),
      svg('line', { x1: padding.left, y1: padding.top, x2: padding.left, y2: padding.top + innerHeight, stroke: 'var(--halo-canvas-border)' }),
      svg('text', { x: padding.left - 8, y: padding.top + 10, 'text-anchor': 'end', fill: 'var(--halo-canvas-muted)', 'font-size': 11 }, [String(max)]),
      ...labels.map((label, index) => svg('text', { x: padding.left + index * groupWidth + groupWidth / 2, y: height - 12, 'text-anchor': 'middle', fill: 'var(--halo-canvas-muted)', 'font-size': 11 }, [label])),
    ];
  ...
  function LineChart(props = {}) {
    ...
    const axis = [
      ...
      svg('line', { x1: padding.left, y1: padding.top, x2: padding.left, y2: padding.top + innerHeight, stroke: 'var(--halo-canvas-border)' }),
      svg('text', { x: padding.left - 8, y: padding.top + 10, 'text-anchor': 'end', fill: 'var(--halo-canvas-muted)', 'font-size': 11 }, [String(max)]),
      ...labels.map((label, index) => svg('text', { x: padding.left + (labels.length > 1 ? index * step : innerWidth / 2), y: height - 12, 'text-anchor': 'middle', fill: 'var(--halo-canvas-muted)', 'font-size': 11 }, [label])),
    ];
  ...
  function PieChart(props = {}) {
    ...
    const slices = entries.map(entry => {
      ...
      const path = svg('path', { d: piePath(cx, cy, radius, angle, nextAngle), fill: entry.color, stroke: 'var(--halo-canvas-bg)', 'stroke-width': 2 });
      angle = nextAngle;
      return path;
    });
    const legend = entries.map((entry, index) => {
      ...
      return [
        ...
        svg('text', { x: 348, y, fill: 'var(--halo-canvas-fg)', 'font-size': 12 }, [`${entry.label} (${percent}%)`]),
      ];
    }).flat();
  ...
  function renderDiagramSvg(layout, nodes, edges, label) {
    ...
    const edgeEls = layout.edges.map((edge, index) => {
      ...
      return svg('g', {}, [
        ...
        meta.label ? svg('text', { x: (edge.sourceX + edge.targetX) / 2, y: (edge.sourceY + edge.targetY) / 2 - 4, 'text-anchor': 'middle', fill: 'var(--halo-canvas-muted)', 'font-size': 10 }, [String(meta.label).slice(0, 18)]) : null,
      ]);
    });
    const nodeEls = layout.nodes.map(layoutNode => {
      ...
      return svg('g', { transform: `translate(${layoutNode.x} ${layoutNode.y})` }, [
        svg('rect', { width: layoutNode.width, height: layoutNode.height, rx: 6, fill: 'var(--halo-canvas-bg)', stroke: color, 'stroke-width': 1.25 }),
        svg('text', { x: 12, y: description ? 18 : layoutNode.height / 2 + 4, fill: 'var(--halo-canvas-fg)', 'font-size': 12, 'font-weight': 650 }, [String(title).slice(0, 22)]),
        description ? svg('text', { x: 12, y: 34, fill: 'var(--halo-canvas-muted)', 'font-size': 10 }, [String(description).slice(0, 26)]) : null,
      ]);
    });
  ...
  function diagramShell(title, height, style, child) {
    return el('div', { style: { minWidth: 0, overflow: 'auto', border: '1px solid var(--halo-canvas-border)', borderRadius: '8px', padding: '10px', background: 'rgba(127,127,127,0.04)', ...style } }, [
      title ? el('div', { style: { marginBottom: '10px', color: 'var(--halo-canvas-fg)', fontSize: '12px', fontWeight: 650 } }, [title]) : null,
  ...
  function DependencyGraph(props = {}) {
    ...
    return diagramShell(props.title, props.height || layout.height, props.style, nodes.length ? renderDiagramSvg(layout, nodes, edges, props.title || 'Dependency graph') : el('div', { style: { color: 'var(--halo-canvas-muted)', fontSize: '12px' } }, ['No graph nodes']));
  }
  ...
  function FlowDiagram(props = {}) {
    ...
    return diagramShell(props.title, props.height || layout.height, props.style, nodes.length ? renderDiagramSvg(layout, nodes, edges, props.title || 'Flow diagram') : el('div', { style: { color: 'var(--halo-canvas-muted)', fontSize: '12px' } }, ['No flow steps']));
  }
  function toneColor(tone) {
    return tone === 'muted' || tone === 'secondary' || tone === 'tertiary' || tone === 'quaternary' ? 'var(--halo-canvas-muted)' : tone === 'success' ? 'var(--halo-canvas-success)' : tone === 'warning' ? 'var(--halo-canvas-warning)' : tone === 'danger' ? 'var(--halo-canvas-danger)' : tone === 'info' ? 'var(--halo-canvas-info)' : tone === 'neutral' ? 'var(--halo-canvas-muted)' : 'var(--halo-canvas-fg)';
  }
  function weightValue(weight) { return weight === 'medium' ? 500 : weight === 'semibold' ? 650 : weight === 'bold' ? 700 : 400; }
  function cellStyle(head, align = 'left') { return { textAlign: align, padding: '7px 9px', borderBottom: '1px solid rgba(127,127,127,0.16)', fontWeight: head ? 650 : 400, color: head ? 'var(--halo-canvas-fg)' : undefined }; }
  ...
  window.addEventListener('message', event => {
    ...
    if (data.type === 'halo-canvas-action-result') {
    ...
    if (data.type === 'halo-canvas-design-mode') {
      setDesignMode(data.enabled);
      return;
    }
    if (data.type !== 'halo-canvas-state' && data.type !== 'halo-canvas-load-state-result' && data.type !== 'halo-canvas-save-state-result') return;
