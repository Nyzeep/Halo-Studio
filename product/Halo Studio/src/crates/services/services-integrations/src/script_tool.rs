...
function moduleContext() {
  ...
  return vm.createContext(sandbox, { name: "halo-script-tool" });
}
...
async fn probe_node_version(executable: &PathBuf) -> Result<String, String> {
    ...
    let mut child = ProcessTreeChild::spawn(&mut command)
        .await
        .map_err(|_| "Halo could not start Node.js for external tools".to_string())?;
    let stdout = child
        .take_stdout()
...
mod tests {
    ...
    use halo_runtime_ports::{ScriptToolRuntime, ScriptToolRuntimeAvailability};
    use std::path::PathBuf;
