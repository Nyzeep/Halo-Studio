...
pub use halo_product_domains::miniapp::host_routing::is_host_primitive;
...
use halo_product_domains::miniapp::permission_policy::{
    resolve_policy_with_request, MiniAppPermissionPolicyRequest,
};
use halo_product_domains::miniapp::types::MiniAppPermissions;
...
async fn dispatch_shell(
    ...
    let mut cmd = if let Some(argv) = plan.argv.as_ref() {
        ...
        let mut c =
            halo_services_core::process_manager::create_tokio_command(program.as_os_str());
        if argv.len() > 1 {
            c.args(&argv[1..]);
        }
        c
        ...
        {
            let mut c = halo_services_core::process_manager::create_tokio_command("cmd");
            c.args(["/C", &plan.command]);
            c
        }
        #[cfg(not(target_os = "windows"))]
        {
            let mut c = halo_services_core::process_manager::create_tokio_command("sh");
            c.args(["-c", &plan.command]);
            c
        }
    };
...
mod tests {
    use super::*;
    use halo_product_domains::miniapp::types::{MiniAppPermissions, ShellPermissions};
