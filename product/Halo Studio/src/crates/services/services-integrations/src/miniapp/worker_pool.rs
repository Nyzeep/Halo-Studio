...
use halo_product_domains::miniapp::runtime::{detect_runtime, DetectedRuntime};
use halo_product_domains::miniapp::types::{NodePermissions, NpmDep};
...
impl JsWorkerPool {
    ...
    pub async fn install_deps_in_dir(
        ...
        let output = halo_services_core::process_manager::create_tokio_command(command.program)
            .args(command.args)
            .current_dir(app_dir)
            .output()
            .await
