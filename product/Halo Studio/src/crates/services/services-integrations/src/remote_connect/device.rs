...
//! `<HALO_HOME>/device_identity.json` (normally `~/.halo-studio/device_identity.json`).
...
fn identity_file_path() -> Result<PathBuf> {
    ...
    if let Ok(path) = std::env::var("HALO_DEVICE_IDENTITY_PATH") {
        let path = path.trim();
        if !path.is_empty() {
    ...
    let home = super::halo_home_dir()
        .ok_or_else(|| anyhow!("cannot determine Halo home directory"))?;
