...
/// synchronously during application exit even if async cleanup did not run.
...
fn find_ngrok() -> Option<PathBuf> {
    ...
    let candidates: Vec<PathBuf> = vec![
        ...
        dirs::home_dir()
            .map(|home| home.join("ngrok"))
            .unwrap_or_default(),
        dirs::home_dir()
            .map(|home| home.join(".ngrok/ngrok"))
            .unwrap_or_default(),
        dirs::home_dir()
            .map(|home| home.join("bin/ngrok"))
            .unwrap_or_default(),
        ...
        .map(|out| {
            ...
            text.lines()
                .filter_map(|line| {
                    ...
                    parts
                        .get(1)
                        .and_then(|value| value.trim_matches('"').trim().parse::<u32>().ok())
                })
                .collect()
        })
        .unwrap_or_default()
...
pub async fn start_ngrok_tunnel(local_port: u16) -> Result<NgrokTunnel> {
    let ngrok_path = find_ngrok().ok_or_else(|| {
        anyhow!(
            "ngrok is not installed.\n\
             ...
             No need to start ngrok manually \u{2014} Halo will start it automatically.\n\
    ...
    let public_url = match parse_tunnel_url_from_stdout(stdout).await {
        ...
        Err(error) => {
            ...
            return Err(anyhow!(
                "ngrok tunnel failed to establish: {error}\n\
                 Possible causes:\n\
