...
use super::{HaloError, HaloResult, SpeechStoragePaths};
use chrono::Utc;
...
const INSTALL_RECORD_FILE: &str = "halo-model-install.json";
...
impl SpeechModelStore {
    ...
    pub(super) async fn list_statuses(&self) -> HaloResult<Vec<SpeechModelStatus>> {
    ...
    pub(super) async fn delete_model(
        ...
        let resolved = target.canonicalize().map_err(|e| {
            HaloError::service(format!("Failed to resolve speech model path: {e}"))
        })?;
...
pub(super) fn validate_relative_archive_path(path: &Path) -> HaloResult<PathBuf> {
    ...
    if sanitized.as_os_str().is_empty() {
        return Err(HaloError::validation("Archive entry path is empty"));
    }
    Ok(sanitized)
}

pub(super) async fn dir_size(path: &Path) -> HaloResult<u64> {
    fn inner(
        path: &Path,
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = HaloResult<u64>> + Send + '_>> {
        Box::pin(async move {
            if !path.exists() {
                return Ok(0);
            }
...
async fn canonical_or_create(path: &Path) -> HaloResult<PathBuf> {
    fs::create_dir_all(path).await?;
    path.canonicalize()
        .map_err(|e| HaloError::service(format!("Failed to resolve directory: {e}")))
}
