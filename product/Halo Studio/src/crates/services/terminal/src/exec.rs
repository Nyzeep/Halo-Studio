...
mod tests {
    ...
    async fn pipe_exec_returns_output_and_exit_code() {
        ...
        let response = manager
            .exec_command(ExecCommandRequest {
                argv: shell_argv("echo halo_exec_test"),
        ...
        assert!(response.output.contains("halo_exec_test"));
    }
    ...
    async fn control_interrupt_kills_running_pipe_process_group_after_grace() {
        ...
        let pid_file = std::env::temp_dir().join(format!("halo-exec-child-{unique}.pid"));
        let tick_file = std::env::temp_dir().join(format!("halo-exec-child-{unique}.tick"));
...
print("parent_exit", flush=True)"#;
    ...
    async fn assert_git_bash_npm_dev_control(action: ExecControlAction) {
        let Ok(bash_path) = std::env::var("HALO_TEST_WINDOWS_GIT_BASH") else {
            eprintln!("skipping git bash exec regression: HALO_TEST_WINDOWS_GIT_BASH is unset");
            return;
        };
        let Ok(fixture_dir) = std::env::var("HALO_TEST_WINDOWS_NPM_FIXTURE") else {
            eprintln!(
                "skipping git bash exec regression: HALO_TEST_WINDOWS_NPM_FIXTURE is unset"
            );
            return;
        };
