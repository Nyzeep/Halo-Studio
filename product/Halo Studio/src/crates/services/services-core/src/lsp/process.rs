    ...
    fn build_command(
        ...
        match runtime_type {
            RuntimeType::Executable => {
                        ...
                        if ext_lower == "bat" || ext_lower == "cmd" {
                            ...
                            if let Ok(content) = std::fs::read_to_string(server_bin) {
                                ...
                                if let Some(js_path) = script_path {
                                    ...
                                    let mut cmd =
                                        crate::process_manager::create_tokio_command(node_cmd);
                                    cmd.arg(js_path);
                ...
                let mut cmd = crate::process_manager::create_tokio_command(server_bin);
                cmd.args(&config.args);
                cmd.envs(&config.env);
                Ok(cmd)
            }
            RuntimeType::Bash => {
                #[cfg(windows)]
                {
                    ...
                    for path in &bash_paths {
                        if crate::process_manager::create_command(path)
                            .arg("--version")
                            .output()
                            .is_ok()
                        {
                    ...
                    let mut cmd = crate::process_manager::create_tokio_command(&bash_cmd);
                    cmd.arg(server_bin);
                    cmd.args(&config.args);
                    cmd.envs(&config.env);
                    Ok(cmd)
                ...
                #[cfg(not(windows))]
                {
                    let mut cmd = crate::process_manager::create_tokio_command("bash");
                    cmd.arg(server_bin);
                    cmd.args(&config.args);
                    cmd.envs(&config.env);
                    Ok(cmd)
            ...
            RuntimeType::Node => {
                ...
                match crate::process_manager::create_command(node_cmd)
                    .arg("--version")
                    .output()
                {
                    Ok(_) => {}
                ...
                let mut cmd = crate::process_manager::create_tokio_command(node_cmd);
                cmd.arg(server_bin);
                cmd.args(&config.args);
                cmd.envs(&config.env);
                Ok(cmd)
...
mod tests {
    ...
    fn server_config(runtime: Option<&str>) -> ServerConfig {
        ServerConfig {
    ...
    #[test]
    fn detects_runtime_from_manifest_before_extension() {
        let config = server_config(Some("node"));
        let server_bin = PathBuf::from("server.sh");

        assert_eq!(
            LspServerProcess::detect_runtime_type(&config, &server_bin),
            RuntimeType::Node
        );
    }

    #[test]
    fn detects_runtime_from_server_extension() {
        assert_eq!(
            LspServerProcess::detect_runtime_type(
                &server_config(None),
