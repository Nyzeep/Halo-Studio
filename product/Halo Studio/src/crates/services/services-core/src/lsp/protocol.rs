...
mod tests {
    ...
    #[test]
    fn creates_request_and_notification_messages() {
        assert!(matches!(
            create_request(7, "textDocument/hover", Some(json!({ "uri": "file:///a.rs" }))),
            JsonRpcMessage::Request(request)
                if request.id == 7 && request.method == "textDocument/hover"
        ));

        assert!(matches!(
            create_notification("textDocument/didOpen", None),
    ...
    #[test]
    fn extracts_response_result_without_losing_payload() {
        let result = json!({ "capabilities": { "hoverProvider": true } });
        ...
        assert_eq!(extract_result(response).expect("extract result"), result);
    }
}
