...
#[cfg(test)]
mod tests {
    use super::RequestDebouncer;

    #[tokio::test]
    async fn rejects_duplicate_request_inside_debounce_window() {
        let debouncer = RequestDebouncer::new(1_000);

        assert!(
            debouncer
                .should_send("file:///a.rs", "textDocument/hover")
                .await
        );
        assert!(
            !debouncer
        ...
        assert!(
            debouncer
                .should_send("file:///a.rs", "textDocument/definition")
                .await
        );
    }
}
