...
use halo_core::service::remote_ssh::{SSHCommandOptions, SSHConnectionManager};
...
pub(crate) async fn predownload_npm_adapter(package: &str, bin: &str) -> HaloResult<()> {
    let npm_path = find_executable("npm")
        .ok_or_else(|| HaloError::service("npm is not available on PATH".to_string()))?;
    ...
    .await
    {
        ...
        Ok(output) => Err(HaloError::service(format!(
...
pub(crate) async fn install_npm_cli_package(package: &str) -> HaloResult<()> {
    let npm_path = find_executable("npm")
        .ok_or_else(|| HaloError::service("npm is not available on PATH".to_string()))?;
...
pub(crate) async fn install_remote_npm_cli_package(
        ...
        .await
    {
        ...
        Ok(output) if output.timed_out => Err(HaloError::service(format!(
            "Failed to install remote ACP agent CLI '{}': command timed out",
            package
        ))),
        Ok(output) if output.interrupted => Err(HaloError::service(format!(
...
mod tests {
    ...
    fn command_search_paths_keep_configured_path_first() {
        let configured_paths = env::join_paths([
            PathBuf::from("/tmp/halo-acp-first"),
            PathBuf::from("/tmp/halo-acp-second"),
        ])
        .expect("test paths should be joinable");

        let paths = command_search_paths(Some(&configured_paths));

        assert_eq!(paths.first(), Some(&PathBuf::from("/tmp/halo-acp-first")));
        assert_eq!(paths.get(1), Some(&PathBuf::from("/tmp/halo-acp-second")));
    }

    #[test]
    fn find_executable_uses_configured_path() {
        let test_dir = env::temp_dir().join(format!("halo-acp-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&test_dir).expect("test dir should be created");

        #[cfg(windows)]
        let file_name = "halo-test-tool.EXE";
        #[cfg(not(windows))]
        let file_name = "halo-test-tool";

        let executable = test_dir.join(file_name);
        std::fs::write(&executable, b"").expect("test executable should be written");

        let found = find_executable_with_path("halo-test-tool", Some(test_dir.as_os_str()));
