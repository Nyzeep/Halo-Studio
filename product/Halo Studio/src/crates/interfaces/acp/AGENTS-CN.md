...
`halo-acp` 负责基于已组装产品 runtime 的 Agent Client Protocol 入口与 ACP client 行为。ACP protocol / client 细节留在这里或应用入口 adapter 中；跨层只共享稳定 capability facts。

CLI 托管的 ACP 服务端已通过 `ProductAssembler` 消费 `DeliveryProfile::Acp`，并使用 Agent Runtime SDK
完成会话创建/列举、活动会话模型/模式更新、轮次提交/取消、交互响应和 Agent 事件订阅。`halo-acp` 仍直接依赖
`halo-core/product-full`，用于一次性恢复完整持久化历史、模型/模式目录与提供方配置读取、MCP 配置，
...
- ACP external-agent tool 的命名、schema、validation、presentation 和 result shape 属于 `halo-agent-tools` 的 portable contract；ACP 应调用这些 helper，不要在本层重复定义。
...
```bash
cargo check -p halo-acp
cargo test -p halo-acp
```
