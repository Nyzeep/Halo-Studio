...
`halo-acp` owns the Agent Client Protocol surface over the assembled product
...
responses, and agent event subscription. `halo-acp` still depends directly on
`halo-core` with `product-full` for single-pass full persisted-history restore,
...
- ACP external-agent tool naming, schema, validation, presentation, and result
  shape are portable contracts owned by `halo-agent-tools`; ACP should call
