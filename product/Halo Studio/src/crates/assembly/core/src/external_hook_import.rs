...
use halo_services_integrations::hook_import::{
...
pub(crate) async fn enabled_imported_hook_layers(
    workspace: Option<&Path>,
) -> ExternalSourceOperationResult<Vec<halo_agent_runtime::native_hooks::AgentHookSettingsLayer>>
{
    let stores = stores_for(workspace).await?;
...
async fn import_state(
    ...
    catalog_sources: &[halo_product_domains::external_hook_catalog::ExternalHookSource],
