...
struct OpaquePiProviderReadiness;

#[async_trait]
impl PiProviderReadinessPort for OpaquePiProviderReadiness {
    async fn check(&self) -> PortResult<PiProviderReadiness> {
        // Pi owns provider/model selection and credential resolution. Halo
        // deliberately does not read auth.json or any provider secret. The
        // real provider failure is surfaced by the controlled Pi process.
        Ok(PiProviderReadiness { available: true })
    }
}

fn selected_pi_rpc_adapter() -> Arc<dyn PiRpcPort> {
    Arc::new(PiRpcAdapter::new())
}

/// Builds the sole P0 Halo Workbench Runtime composition.
///
/// The selected adapter is fixed to the local Pi RPC implementation;
...
mod tests {
    ...
    use bitfun_runtime_ports::{
        PiProviderReadinessPort, PiRpcCommand, PiRpcFailureKind, PiRpcReply, PiRpcWorkspace,
        PortErrorKind, WorkbenchWorkspaceFactsRequest,
    };
    use chrono::Utc;

    use super::{project_workspace_facts, selected_pi_rpc_adapter, OpaquePiProviderReadiness};
    use crate::service::workspace::{
    ...
    #[tokio::test]
    async fn p0_selection_is_fixed_and_provider_readiness_remains_opaque() {
        let adapter =
            bitfun_pi_rpc_adapter::PiRpcAdapter::with_config(bitfun_pi_rpc_adapter::PiRpcConfig {
        ...
        let readiness = OpaquePiProviderReadiness.check().await.unwrap();
