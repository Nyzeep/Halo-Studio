...
impl WorkspaceProvider {
    ...
    pub async fn open(&self, path: &str) -> HaloResult<WorkspaceInfo> {
    ...
    pub async fn switch(&self, workspace_id: &str) -> HaloResult<()> {
    ...
    pub async fn close_current(&self) -> HaloResult<()> {
    ...
    /// Quick cleanup.
    pub async fn quick_cleanup(&self) -> HaloResult<WorkspaceCleanupResult> {
    ...
    pub async fn detect_project_type(&self, path: &str) -> HaloResult<WorkspaceType> {
        let path_buf = PathBuf::from(path);

        if !path_buf.exists() {
            return Err(HaloError::service("Path does not exist".to_string()));
        }
    ...
    pub async fn get_file_stats(
        &self,
        workspace_id: &str,
    ) -> HaloResult<Option<WorkspaceStatistics>> {
    ...
    /// Rescans a workspace.
    pub async fn rescan(&self, workspace_id: &str) -> HaloResult<WorkspaceInfo> {
