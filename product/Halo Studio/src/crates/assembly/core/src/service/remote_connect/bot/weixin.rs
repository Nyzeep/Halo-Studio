...
//!
//! Provider HTTP/CDN/QR/message parsing lives in `halo-services-integrations`.
//! This module keeps product pairing, command routing, persistence, and agent
//! turn orchestration.

use anyhow::{anyhow, Result};
use base64::{engine::general_purpose::STANDARD as B64, Engine as _};
use halo_services_integrations::remote_connect::bot::weixin as weixin_provider;
use halo_services_integrations::remote_connect::bot::weixin::WeixinProviderClient;
pub use halo_services_integrations::remote_connect::bot::weixin::{
            ...
            if let Some(msgs) = resp["msgs"].as_array() {
                for msg in msgs {
                    ...
                    if text.len() == 6 && text.chars().all(|c| c.is_ascii_digit()) {
                        ...
                        let err = if language.is_chinese() {
                            "\u{8bf7}\u{8f93}\u{5165} Halo \u{684c}\u{9762}\u{7aef}\u{8fdc}\u{7a0b}\u{8fde}\u{63a5}\u{4e2d}\u{663e}\u{793a}\u{7684} 6 \u{4f4d}\u{914d}\u{5bf9}\u{7801}\u{3002}"
                        } else {
                            "Please send the 6-digit pairing code from Halo Desktop Remote Connect."
                        };
