...
impl MCPServerManager {
    ...
    pub async fn refresh_server_resource_catalog(&self, server_id: &str) -> HaloResult<usize> {
        let connection = self.get_connection(server_id).await.ok_or_else(|| {
            HaloError::NotFound(format!("MCP server connection not found: {}", server_id))
        })?;
        self.refresh_resources_catalog(server_id, connection).await
    }

    /// Refreshes prompts catalog cache for one server.
    pub async fn refresh_server_prompt_catalog(&self, server_id: &str) -> HaloResult<usize> {
        let connection = self.get_connection(server_id).await.ok_or_else(|| {
            HaloError::NotFound(format!("MCP server connection not found: {}", server_id))
        })?;
