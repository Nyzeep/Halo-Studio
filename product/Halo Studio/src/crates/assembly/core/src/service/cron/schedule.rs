...
pub(super) fn validate_schedule(schedule: &CronSchedule, created_at_ms: i64) -> HaloResult<()> {
...
fn parse_at_timestamp_ms(schedule: &CronSchedule) -> HaloResult<i64> {
    ...
    let parsed = DateTime::parse_from_rfc3339(at).map_err(|error| {
        HaloError::validation(format!("Invalid ISO-8601 timestamp '{}': {}", at, error))
    })?;
    Ok(parsed.timestamp_millis())
}

fn compute_every_next_run_ms(every_ms: u64, anchor_ms: i64, after_ms: i64) -> HaloResult<i64> {
    if every_ms == 0 {
        return Err(HaloError::validation(
    ...
    i64::try_from(next)
        .map_err(|_| HaloError::service("Recurring schedule next run timestamp overflowed i64"))
}

fn compute_cron_next_run_ms(expr: &str, tz: Option<&str>, after_ms: i64) -> HaloResult<i64> {
    let normalized_expr = normalize_cron_expr(expr)?;
    let schedule = Schedule::from_str(&normalized_expr).map_err(|error| {
        HaloError::validation(format!("Invalid cron expression '{}': {}", expr, error))
    })?;
...
fn parse_timezone(tz_name: &str) -> HaloResult<Tz> {
    Tz::from_str(tz_name).map_err(|error| {
        HaloError::validation(format!("Invalid timezone '{}': {}", tz_name, error))
    })
}

fn normalize_cron_expr(expr: &str) -> HaloResult<String> {
    let fields = expr.split_whitespace().collect::<Vec<_>>();
    match fields.len() {
        5 => Ok(format!("0 {}", expr)),
        6 | 7 => Ok(expr.to_string()),
        other => Err(HaloError::validation(format!(
