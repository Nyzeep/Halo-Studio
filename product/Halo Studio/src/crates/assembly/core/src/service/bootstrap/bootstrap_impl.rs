...
async fn ensure_markdown_placeholder(path: &Path, content: &str) -> HaloResult<bool> {
    if path.exists() {
        return Ok(false);
    }
    ...
    fs::write(path, normalized_content)
        .await
        .map_err(|e| HaloError::service(format!("Failed to create {}: {}", path.display(), e)))?;

    Ok(true)
}

fn gitignore_already_ignores_halo(content: &str) -> bool {
    content.lines().any(|line| {
        let entry = line.trim();
        !entry.starts_with('#')
            && matches!(entry, ".halo-studio" | ".halo-studio/" | "/.halo-studio" | "/.halo-studio/")
    })
}

pub(crate) async fn ensure_workspace_gitignore_ignores_halo(
    workspace_root: &Path,
) -> HaloResult<bool> {
    let gitignore_path = workspace_root.join(".gitignore");
    let halo_entry = ".halo-studio/";
    ...
    if gitignore_already_ignores_halo(&content) {
        return Ok(false);
    }
    ...
    debug!(
        "Added workspace .gitignore entry for .halo: path={}",
        gitignore_path.display()
    );
...
async fn ensure_workspace_gitignore_ignores_halo_best_effort(workspace_root: &Path) -> bool {
    match ensure_workspace_gitignore_ignores_halo(workspace_root).await {
        Ok(updated) => updated,
        Err(e) => {
            warn!(
                "Failed to ensure workspace .gitignore ignores .halo: workspace={}, error={}",
...
pub(crate) async fn initialize_workspace_persona_files(workspace_root: &Path) -> HaloResult<()> {
    let gitignore_updated =
        ensure_workspace_gitignore_ignores_halo_best_effort(workspace_root).await;
...
pub async fn reset_workspace_persona_files_to_default(workspace_root: &Path) -> HaloResult<()> {
...
mod tests {
    use super::{
        ensure_workspace_gitignore_ignores_halo, ensure_workspace_persona_files_for_prompt,
    ...
    #[tokio::test]
    async fn ensure_workspace_gitignore_ignores_halo_skips_when_gitignore_missing() {
        let workspace_root = unique_workspace("halo-gitignore-missing");
        fs::create_dir_all(&workspace_root)
            .await
            .expect("Failed to create temp workspace");

        let updated = ensure_workspace_gitignore_ignores_halo(&workspace_root)
            .await
    ...
    #[tokio::test]
    async fn ensure_workspace_gitignore_ignores_halo_appends_without_clobbering() {
        let workspace_root = unique_workspace("halo-gitignore-append");
        fs::create_dir_all(&workspace_root)
            .await
            .expect("Failed to create temp workspace");
        ...
        ensure_workspace_gitignore_ignores_halo(&workspace_root)
            .await
            .expect("Failed to ensure .gitignore");
    ...
    #[tokio::test]
    async fn ensure_workspace_gitignore_ignores_halo_is_idempotent() {
        let workspace_root = unique_workspace("halo-gitignore-idempotent");
        fs::create_dir_all(&workspace_root)
            .await
            .expect("Failed to create temp workspace");
        fs::write(workspace_root.join(".gitignore"), "target/\n.halo-studio/\n")
    ...
    async fn initialize_workspace_persona_files_creates_all_four_files() {
        let workspace_root = unique_workspace("halo-bootstrap-init");
    ...
    async fn ensure_workspace_persona_files_for_prompt_preserves_completed_bootstrap() {
        let workspace_root = unique_workspace("halo-bootstrap-preserve");
