use crate::service::bootstrap::ensure_workspace_gitignore_ignores_halo;
use crate::service::config::{get_global_config_service, types::WorkspaceConfig};
use crate::util::errors::{HaloError, HaloResult};
use async_trait::async_trait;
use halo_services_integrations::workspace_search as owner;
...
impl WorkspaceSearchService {
    ...
    ) -> HaloResult<owner::WorkspaceIndexStatus> {
        self.inner
            .open_repo(repo_root)
            .await
    ...
    pub async fn build_index(
        &self,
        repo_root: impl AsRef<Path>,
    ) -> HaloResult<owner::IndexTaskHandle> {
        self.inner
    ...
    ) -> HaloResult<owner::ContentSearchResult> {
        self.inner
    ...
    ) -> HaloResult<owner::GlobSearchResult> {
        self.inner
            .glob(request)
            .await
            .map_err(map_workspace_search_error)
...
impl owner::WorkspaceSearchRuntimeHooks for CoreWorkspaceSearchRuntimeHooks {
    ...
    async fn ensure_workspace_ready(&self, repo_root: &Path) -> owner::WorkspaceSearchResult<()> {
        ensure_workspace_gitignore_ignores_halo(repo_root)
            .await
            .map(|_| ())
...
fn map_workspace_search_error(error: String) -> HaloError {
