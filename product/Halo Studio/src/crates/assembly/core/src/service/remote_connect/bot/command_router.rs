...
///
/// Remote SSH workspaces store sessions under `~/.halo-studio/remote_ssh/{host}/...`,
...
pub async fn execute_forwarded_turn(
    ...
    use halo_services_integrations::remote_connect::RemoteConnectSubmissionSource;
...
mod menu_tests {
    ...
    fn assistant_mode_body_uses_display_name_not_dir_name() {
        let mut state = BotChatState::new("c".into());
        state.current_assistant = Some("/tmp/halo_assistants/workspace-abc123".to_string());
...
mod handle_chat_tests {
    ...
    async fn chat_message_forwards_silently_without_processing_menu() {
        ...
        let s = strings_for(BotLanguage::ZhCN);
        let result = handle_chat(&mut state, "hello halo", vec![], s).await;
