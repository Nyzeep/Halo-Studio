...
pub use halo_services_integrations::mcp::protocol::{
...
pub fn parse_response_result<T>(response: &MCPResponse) -> crate::util::errors::HaloResult<T>
where
    T: serde::de::DeserializeOwned,
{
    if let Some(error) = &response.error {
        return Err(crate::util::errors::HaloError::MCPError(format!(
            "MCP Error {}: {}",
    ...
    let result = response.result.as_ref().ok_or_else(|| {
        crate::util::errors::HaloError::MCPError("Missing result in MCP response".to_string())
    })?;

    serde_json::from_value(result.clone()).map_err(|e| {
        crate::util::errors::HaloError::Deserialization(format!(
