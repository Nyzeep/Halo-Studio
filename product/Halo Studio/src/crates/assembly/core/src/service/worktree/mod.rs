...
use halo_services_core::session::{SessionMetadata, SessionMetadataStore, SessionStatus};
...
impl WorktreeService {
    ...
    /// worktrees beneath the configured Halo worktree root. Invalid and
    ...
    ) -> Result<halo_services_core::json_store::JsonFileCrossProcessLock, WorktreeError> {
        JsonFileStore
...
fn resolve_managed_root(
    ...
    if portable_configured.is_empty() || portable_configured == "~/.halo-studio/worktrees" {
...
mod tests {
    ...
    fn managed_directory_name_includes_project_name_and_short_worktree_id() {
        let project = Path::new("projects").join("Halo");
    ...
    async fn managed_target_path_uses_readable_leaf_and_rejects_collisions() {
        ...
        let project = root.path().join("projects").join("Halo");
    ...
    fn windows_style_default_root_uses_the_managed_path_contract() {
        let user_root = std::env::temp_dir().join("halo-worktree-root-test");
        let path_manager = PathManager::with_user_root_for_tests(user_root);
        let settings = WorktreeSettings {
            root_path: r"~\.halo-studio\worktrees".to_string(),
            ..WorktreeSettings::default()
        };
