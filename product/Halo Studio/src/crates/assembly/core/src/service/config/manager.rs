...
type ConfigMigrationFn = fn(Value) -> HaloResult<Value>;
...
fn normalize_legacy_theme_id(theme_id: &str) -> String {
    match theme_id.trim() {
        "dark" => "halo-dark".to_string(),
        "light" => "halo-light".to_string(),
...
fn config_value_for_persistence(config: &GlobalConfig) -> HaloResult<Value> {
...
fn prune_default_memories_config(config_value: &mut Value) -> HaloResult<()> {
...
impl ConfigManager {
    ...
    pub async fn new(settings: ConfigManagerSettings) -> HaloResult<Self> {
        ...
        {
            halo_ai_adapters::diagnostics::set_include_sensitive_diagnostics(
    ...
    async fn load_or_create_config(&mut self) -> HaloResult<()> {
    ...
    async fn create_default_config(&mut self) -> HaloResult<()> {
    ...
    async fn load_existing_config(&mut self) -> HaloResult<()> {
        let content = fs::read_to_string(&self.config_file)
            .await
            .map_err(|e| HaloError::config(format!("Failed to read config file: {}", e)))?;

        let mut config_value: Value = serde_json::from_str(&content).map_err(|e| {
            HaloError::config(format!("Failed to parse config file as JSON: {}", e))
        })?;
    ...
    async fn smart_merge_config_from_value(&mut self, user_value: Value) -> HaloResult<()> {
        ...
        let base_value = serde_json::to_value(&base_config).map_err(|e| {
            HaloError::config(format!("Failed to serialize default config: {}", e))
        })?;
        ...
        let mut config: GlobalConfig = serde_json::from_value(merged_value).map_err(|e| {
            HaloError::config(format!("Failed to deserialize merged config: {}", e))
        })?;
    ...
    async fn save_config(&self) -> HaloResult<()> {
        let content = serde_json::to_string_pretty(&config_value_for_persistence(&self.config)?)
            .map_err(|e| HaloError::config(format!("Config serialization failed: {}", e)))?;
    ...
    pub fn get<T>(&self, path: &str) -> HaloResult<T>
    where
    ...
    pub async fn set<T>(&mut self, path: &str, value: T) -> HaloResult<()>
    where
        T: serde::Serialize,
    {
        let old_config = self.config.clone();
        let mut json_value = serde_json::to_value(value)
            .map_err(|e| HaloError::config(format!("Failed to serialize config value: {}", e)))?;
    ...
    pub async fn reset(&mut self, path: Option<&str>) -> HaloResult<()> {
        let old_config = self.config.clone();
    ...
    pub fn export_config(&self) -> HaloResult<serde_json::Value> {
        serde_json::to_value(&self.config)
            .map_err(|e| HaloError::config(format!("Failed to export config: {}", e)))
    }

    /// Imports configuration.
    pub async fn import_config(&mut self, config_data: serde_json::Value) -> HaloResult<()> {
        let old_config = self.config.clone();
        ...
        let imported_config: GlobalConfig = serde_json::from_value(config_data)
            .map_err(|e| HaloError::config(format!("Failed to parse imported config: {}", e)))?;
    ...
    pub async fn create_backup(&self) -> HaloResult<PathBuf> {
        let timestamp = chrono::Utc::now().format("%Y%m%d_%H%M%S");
        let backup_dir = self.config_dir.join("backups");

        if !backup_dir.exists() {
            fs::create_dir_all(&backup_dir).await.map_err(|e| {
                HaloError::config(format!("Failed to create backup directory: {}", e))
            })?;
        }
        ...
        let content = serde_json::to_string_pretty(&config_value_for_persistence(&self.config)?)
            .map_err(|e| HaloError::config(format!("Failed to serialize backup: {}", e)))?;

        fs::write(&backup_file, content)
            .await
            .map_err(|e| HaloError::config(format!("Failed to write backup: {}", e)))?;
    ...
    fn get_value_by_path(&self, path: &str) -> HaloResult<serde_json::Value> {
    ...
    fn get_value_by_path_from_config(
        ...
        for key in keys {
            current = current.get(key).ok_or_else(|| {
                HaloError::NotFound(format!("Config path '{}' not found", path))
            })?;
        }
    ...
    fn set_value_by_path(&mut self, path: &str, value: serde_json::Value) -> HaloResult<()> {
        if path.is_empty() {
            self.config = serde_json::from_value(value)
                .map_err(|e| HaloError::config(format!("Failed to deserialize config: {}", e)))?;
            return Ok(());
        }
        ...
        let last_key = keys.last().ok_or_else(|| {
            HaloError::config(format!("Config path '{}' does not contain any keys", path))
        })?;
        ...
        self.config = serde_json::from_value(config_value).map_err(|e| {
            HaloError::config(format!("Failed to deserialize updated config: {}", e))
        })?;
    ...
    async fn check_and_broadcast_sensitive_diagnostics_change(&self, old_config: &GlobalConfig) {
        ...
        if old_include != new_include {
            ...
            #[cfg(feature = "ai-adapter-runtime")]
            {
                halo_ai_adapters::diagnostics::set_include_sensitive_diagnostics(new_include);
            }
...
pub(crate) fn migrate_0_0_0_to_1_0_0(mut config: Value) -> HaloResult<Value> {
...
mod tests {
    ...
    fn legacy_theme_id_moves_to_themes_current_when_missing() {
        ...
        assert_eq!(normalized["themes"]["current"], "halo-dark");
        assert!(
    ...
    fn legacy_theme_id_does_not_override_existing_theme_selection() {
        ...
        assert_eq!(normalized["themes"]["current"], "halo-cyber");
    ...
    fn legacy_theme_id_fills_empty_existing_theme_selection() {
        ...
        assert_eq!(normalized["themes"]["current"], "halo-light");
        assert!(normalized.get("theme").is_none());
    }
