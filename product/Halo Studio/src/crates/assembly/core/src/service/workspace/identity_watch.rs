...
impl WorkspaceIdentityWatchService {
    ...
    pub async fn set_event_emitter(&self, emitter: Arc<dyn EventEmitter>) -> HaloResult<()> {
        {
    ...
    pub async fn sync_watched_workspaces(&self) -> HaloResult<()> {
    ...
    async fn create_watcher(&self) -> HaloResult<()> {
        ...
        let mut watcher = RecommendedWatcher::new(tx, Config::default()).map_err(|e| {
            HaloError::service(format!("Failed to create identity watcher: {}", e))
        })?;
