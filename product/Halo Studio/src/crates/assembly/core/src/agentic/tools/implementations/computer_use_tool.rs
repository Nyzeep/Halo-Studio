...
use halo_agent_tools::computer_use::{
...
const COMPUTER_USE_DEBUG_SUBDIR: &str = ".halo-studio/computer_use_debug";
/// Set to `1` to enable on-disk debug copies of Computer use screenshots.
const COMPUTER_USE_DEBUG_SCREENSHOTS_ENV: &str = "HALO_COMPUTER_USE_DEBUG_SCREENSHOTS";
...
**`key_chord` / `type_text` / `scroll` / `wait`:** standard desktop automation without any screenshot step.\n",
    ...
    fn require_multimodal_tool_output_for_screenshot(ctx: &ToolUseContext) -> HaloResult<()> {
    ...
    fn ensure_click_has_no_coordinate_fields(input: &Value) -> HaloResult<()> {
    ...
    ) -> HaloResult<Vec<ScreenOcrTextMatch>> {
    ...
    async fn resolve_target_point(
        host_ref: &dyn crate::agentic::tools::computer_use_host::ComputerUseHost,
        input: &Value,
    ) -> HaloResult<ResolvedDesktopTarget> {
    ...
    ) -> HaloResult<(Value, ToolImageAttachment, String)> {
...
fn parse_ocr_region_native(
    input: &Value,
) -> HaloResult<Option<crate::agentic::tools::computer_use_host::OcrRegionNative>> {
    let v = input
    ...
    let x0 = o.get("x0").and_then(|x| x.as_i64()).ok_or_else(|| {
        HaloError::tool("ocr_region_native.x0 (integer) is required.".to_string())
    })? as i32;
    let y0 = o.get("y0").and_then(|x| x.as_i64()).ok_or_else(|| {
        HaloError::tool("ocr_region_native.y0 (integer) is required.".to_string())
    })? as i32;
    let width = o.get("width").and_then(|x| x.as_u64()).ok_or_else(|| {
        HaloError::tool("ocr_region_native.width (positive integer) is required.".to_string())
    })? as u32;
    let height = o.get("height").and_then(|x| x.as_u64()).ok_or_else(|| {
        HaloError::tool("ocr_region_native.height (positive integer) is required.".to_string())
    })? as u32;
    if width == 0 || height == 0 {
        return Err(HaloError::tool(
...
**`type_text`:** Type text; prefer clipboard for long content. Does **not** move the pointer — **Enter** **`key_chord`** may follow without a mandatory `screenshot` unless you moved the pointer since the last capture. If **`screenshot`** shows the correct chat is already open and the input may be focused, **try `type_text` first** before spending steps on `click_element` / `move_to_text`.",
    ...
    async fn call_impl(
        ...
        let host = context.computer_use_host.as_ref().ok_or_else(|| {
            HaloError::tool(
                "Computer use is only available in the Halo desktop app.".to_string(),
            )
        })?;
        ...
        match action {
            ...
            "type_text" => {
                let text = input
                    .get("text")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| HaloError::tool("text is required".to_string()))?;
            ...
            "wait" => {
                let ms = input
                    .get("ms")
                    .and_then(|v| v.as_u64())
                    .ok_or_else(|| HaloError::tool("ms is required".to_string()))?;
            ...
            "open_app" => {
                let app_name = input
                    ...
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| {
                        HaloError::tool("open_app requires `app_name` parameter.".to_string())
                    })?;
            ...
            "run_apple_script" => {
                ...
                #[cfg(target_os = "macos")]
                {
                    ...
                    .await
                    .map_err(|e| HaloError::tool(format!("spawn: {}", e)))?
                    .map_err(|e| HaloError::tool(format!("osascript: {}", e)))?;
            ...
            _ => Err(HaloError::tool(format!("Unknown action: {}", action))),
        }
    }
}
...
fn req_i32(input: &Value, key: &str) -> HaloResult<i32> {
    input
        .get(key)
        .and_then(|v| v.as_i64())
        .map(|v| v as i32)
        .ok_or_else(|| HaloError::tool(format!("{} is required (integer)", key)))
}
...
mod tests {
    ...
    fn not_expected<T>() -> HaloResult<T> {
        Err(HaloError::tool(
    ...
    impl ComputerUseHost for ChromeForegroundHost {
        ...
        fn map_image_coords_to_pointer(&self, _x: i32, _y: i32) -> HaloResult<(i32, i32)> {
            not_expected()
        }
        fn map_normalized_coords_to_pointer(&self, _x: i32, _y: i32) -> HaloResult<(i32, i32)> {
            not_expected()
        }
        async fn mouse_move(&self, _x: i32, _y: i32) -> HaloResult<()> {
            not_expected()
        }
        async fn pointer_move_relative(&self, _dx: i32, _dy: i32) -> HaloResult<()> {
            not_expected()
        }
        async fn mouse_click(&self, _button: &str) -> HaloResult<()> {
            not_expected()
        }
        async fn scroll(&self, _delta_x: i32, _delta_y: i32) -> HaloResult<()> {
            not_expected()
        }
        async fn key_chord(&self, _keys: Vec<String>) -> HaloResult<()> {
            not_expected()
        }
        async fn type_text(&self, _text: &str) -> HaloResult<()> {
            not_expected()
        }
        async fn wait_ms(&self, _ms: u64) -> HaloResult<()> {
            not_expected()
        }
