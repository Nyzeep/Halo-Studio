...
fn map_get_tool_spec_execution_error(error: GetToolSpecExecutionError) -> HaloError {
    match error {
        GetToolSpecExecutionError::MissingToolName => HaloError::tool(error.to_string()),
        GetToolSpecExecutionError::Detail(message) => HaloError::Validation(message),
    }
}
...
mod tests {
    ...
    fn deferred_tools_context_lists_names_without_short_descriptions() {
        ...
        let description = GetToolSpecTool::build_deferred_tools_context_section(&[
            halo_agent_tools::GetToolSpecDeferredToolSummary {
    ...
    async fn reloading_already_loaded_tool_returns_assistant_hint() {
        ...
        let context = ToolUseContext {
            ...
            dialog_turn_id: None,
            workspace: None,
            loaded_deferred_tool_specs: vec![halo_agent_tools::LoadedDeferredToolSpec {
