...
impl MemoryDatabase {
    ...
    pub async fn initialize(&self) -> HaloResult<()> {
        let db_path = self.db_path.clone();
        task::spawn_blocking(move || -> HaloResult<()> {
        ...
        .await
        .map_err(|error| HaloError::service(format!("Memories init task failed: {}", error)))?
    }

    pub async fn reset_memory_state(&self) -> HaloResult<()> {
        let db_path = self.db_path.clone();
        task::spawn_blocking(move || -> HaloResult<()> {
            if !db_path.exists() {
                return Ok(());
        ...
        .await
        .map_err(|error| HaloError::service(format!("Memories reset task failed: {}", error)))?
    }

    pub async fn upsert_memory(&self, row: &MemoryRow) -> HaloResult<()> {
        let db_path = self.db_path.clone();
        let row = row.clone();
        task::spawn_blocking(move || -> HaloResult<()> {
            let conn = open_connection(&db_path)?;
        ...
        .await
        .map_err(|error| HaloError::service(format!("Memories write task failed: {}", error)))?
    }
    ...
    pub async fn phase1_source_needs_update(
        ...
        task::spawn_blocking(move || -> HaloResult<bool> {
            let conn = open_connection(&db_path)?;
            initialize_schema(&conn)?;
            phase1_source_needs_update_in_conn(&conn, &session_id, session_finished_at_unix_secs)
        })
        .await
        .map_err(|error| HaloError::service(format!("Memories read task failed: {}", error)))?
    }
    ...
    ) -> HaloResult<MemoryPhase1ClaimOutcome> {
        let db_path = self.db_path.clone();
        let session_id = session_id.to_string();
        let worker_id = worker_id.to_string();
        task::spawn_blocking(move || -> HaloResult<MemoryPhase1ClaimOutcome> {
    ...
    pub async fn release_phase1_claim_without_watermark(
        ...
        task::spawn_blocking(move || -> HaloResult<bool> {
            ...
            let rows_affected = conn
                ...
                .map_err(|error| {
                    HaloError::io(format!("Failed to release memory phase1 claim: {}", error))
                })?;
            Ok(rows_affected > 0)
        })
        .await
    ...
    pub async fn list_phase2_candidates(
        ...
        limit: usize,
        max_unused_days: i64,
    ) -> HaloResult<Vec<MemoryPhase2CandidateRow>> {
    ...
    pub async fn prune_stage1_outputs_for_retention(
        ...
        let db_path = self.db_path.clone();
        task::spawn_blocking(move || -> HaloResult<usize> {
            let conn = open_connection(&db_path)?;
            initialize_schema(&conn)?;
        ...
        .await
        .map_err(|error| HaloError::service(format!("Memories prune task failed: {}", error)))?
    }
    ...
    async fn list_phase2_candidates_inner(
        ...
        let db_path = self.db_path.clone();
        task::spawn_blocking(move || -> HaloResult<Vec<MemoryPhase2CandidateRow>> {
            let conn = open_connection(&db_path)?;
            initialize_schema(&conn)?;
            let cutoff = current_unix_secs() - max_unused_days.max(0) * 24 * 60 * 60;
    ...
    pub async fn upsert_phase2_selection(
    ...
    ) -> HaloResult<Option<MemoryPhase2SelectionRow>> {
        let db_path = self.db_path.clone();
        ...
        let max_source_updated_at = selected_rows
            ...
            .max()
            .unwrap_or_default();
        task::spawn_blocking(move || -> HaloResult<Option<MemoryPhase2SelectionRow>> {
            let conn = open_connection(&db_path)?;
            initialize_schema(&conn)?;
            let existing =
    ...
    pub async fn phase2_selected_for_session(&self, session_id: &str) -> HaloResult<bool> {
        let db_path = self.db_path.clone();
        let session_id = session_id.to_string();
        task::spawn_blocking(move || -> HaloResult<bool> {
            let conn = open_connection(&db_path)?;
    ...
    pub async fn enqueue_phase2_job(
        ...
        task::spawn_blocking(move || -> HaloResult<()> {
            ...
            .map_err(|error| {
                HaloError::io(format!("Failed to enqueue memory phase2 job: {}", error))
            })?;
            Ok(())
        })
        .await
    ...
    pub async fn list_recent(&self, limit: usize) -> HaloResult<Vec<MemoryRow>> {
        if limit == 0 {
            return Ok(Vec::new());
        }

        let db_path = self.db_path.clone();
        task::spawn_blocking(move || -> HaloResult<Vec<MemoryRow>> {
            let conn = open_connection(&db_path)?;
            initialize_schema(&conn)?;
            let mut stmt = conn
                .prepare(
                ...
                .map_err(|error| {
                    HaloError::io(format!("Failed to prepare memory list query: {}", error))
                })?;
            let rows = stmt
                .query_map(params![limit as i64], row_to_memory)
                .map_err(|error| {
                    HaloError::io(format!("Failed to query memory rows: {}", error))
                })?;
    ...
    pub async fn get_phase2_job(&self, job_key: &str) -> HaloResult<Option<MemoryJobRow>> {
        let db_path = self.db_path.clone();
        let job_key = job_key.to_string();
        task::spawn_blocking(move || -> HaloResult<Option<MemoryJobRow>> {
            let conn = open_connection(&db_path)?;
            initialize_schema(&conn)?;
            get_job_in_connection(&conn, JOB_KIND_MEMORY_CONSOLIDATE_GLOBAL, &job_key)
        })
    ...
    pub async fn claim_phase2_job(
        ...
        input_watermark: i64,
        lease_seconds: i64,
    ) -> HaloResult<MemoryPhase2ClaimOutcome> {
        let db_path = self.db_path.clone();
        let job_key = job_key.to_string();
        let ownership_token = ownership_token.to_string();
        task::spawn_blocking(move || -> HaloResult<MemoryPhase2ClaimOutcome> {
            let mut conn = open_connection(&db_path)?;
            initialize_schema(&conn)?;
            let tx = conn
                .transaction_with_behavior(TransactionBehavior::Immediate)
            ...
            .map_err(|error| {
                HaloError::io(format!("Failed to claim memory phase2 job: {}", error))
            })?;
            tx.commit().map_err(|error| {
                HaloError::io(format!(
    ...
    async fn complete_phase2_job(
        ...
        task::spawn_blocking(move || -> HaloResult<bool> {
            ...
            let rows_affected = conn
                ...
                .map_err(|error| {
                    HaloError::io(format!("Failed to complete memory phase2 job: {}", error))
                })?;
            Ok(rows_affected > 0)
        })
        .await
    ...
    pub async fn complete_phase2_job_failure(
        ...
        task::spawn_blocking(move || -> HaloResult<bool> {
            ...
            let rows_affected = conn
                ...
                .map_err(|error| {
                    HaloError::io(format!("Failed to fail memory phase2 job: {}", error))
                })?;
            Ok(rows_affected > 0)
        })
        .await
...
fn open_connection(path: &Path) -> HaloResult<Connection> {
...
fn initialize_schema(conn: &Connection) -> HaloResult<()> {
    ...
    .map_err(|error| HaloError::io(format!("Failed to initialize memories schema: {}", error)))?;
    Ok(())
}
...
fn table_columns(conn: &Connection, table_name: &str) -> HaloResult<Vec<String>> {
    let mut stmt = conn
...
fn clear_memory_state_in_conn(conn: &Connection) -> HaloResult<()> {
    conn.execute_batch(
        r#"
        DELETE FROM stage1_outputs;
        DELETE FROM jobs;
    ...
    .map_err(|error| HaloError::io(format!("Failed to clear memories state: {}", error)))?;
    Ok(())
}
...
fn upsert_stage1_output_conn(
    ...
    .map_err(|error| {
        HaloError::io(format!("Failed to upsert memory stage1 output: {}", error))
    })?;
    Ok(())
}
...
fn try_claim_phase1_job_in_tx(
    ...
    .map_err(|error| HaloError::io(format!("Failed to claim memory phase1 job: {}", error)))?;
...
fn get_job_in_connection(
    ...
    kind: &str,
    job_key: &str,
) -> HaloResult<Option<MemoryJobRow>> {
    conn.query_row(
        r#"
    ...
    .optional()
    .map_err(|error| HaloError::io(format!("Failed to query memory job: {}", error)))
}
...
fn collect_rows<T>(
    ...
    for row in rows {
        items.push(
            row.map_err(|error| HaloError::io(format!("Failed to decode {}: {}", label, error)))?,
        );
    }
    Ok(items)
}
