...
impl ReviewPlatformTool {
    ...
    fn repository_path(input: &Value, context: &ToolUseContext) -> HaloResult<String> {
        let requested = input
            .get("repository_path")
        ...
        context
            ...
            .ok_or_else(|| HaloError::tool("repository_path is required".to_string()))
    }

    fn string_field(input: &Value, key: &str) -> HaloResult<String> {
        input
            .get(key)
            .and_then(Value::as_str)
            .map(str::trim)
            .filter(|value| !value.is_empty())
            .map(str::to_string)
            .ok_or_else(|| HaloError::tool(format!("{} is required", key)))
    }
    ...
    fn submit_event(input: &Value) -> HaloResult<ReviewSubmitEvent> {
        match input
            .get("event")
            .and_then(Value::as_str)
            .unwrap_or("comment")
    ...
    fn detail_section(input: &Value) -> HaloResult<ReviewPlatformDetailSection> {
        match input
            .get("section")
            .and_then(Value::as_str)
            .unwrap_or("overview")
    ...
    fn platform_kind(input: &Value) -> HaloResult<ReviewPlatformKind> {
    ...
    async fn resolve_remote_id(
        ...
        repository_path: &str,
        input: &Value,
    ) -> HaloResult<Result<String, Value>> {
...
When returning pull request results to the user, include the provider web URL so the chat UI can open the pull request detail panel naturally."#.to_string())
    ...
    async fn call_impl(
        ...
        let data = match action.as_str() {
            ...
            _ => return Err(HaloError::tool(format!("Unsupported action: {}", action))),
        };
...
mod tests {
    ...
    fn different_provider_repositories_return_typed_remote_selection() {
        let remotes = vec![
            github_remote("origin-id", "origin", "limityan/Halo"),
            github_remote("upstream-id", "upstream", "GCWing/BitFun"),
        ];
        let supported = canonical_supported_remotes(&remotes);

        let selection = remote_selection_result(ACTION_GET, "D:/workspace/Halo", &supported);
