//! Host abstraction for desktop automation (implemented in `halo-desktop`).
...
pub use halo_agent_tools::computer_use::{
...
pub trait ComputerUseHost: Send + Sync + std::fmt::Debug {
    ...
    async fn accessibility_hit_at_global_point(
        ...
        _gx: f64,
        _gy: f64,
    ) -> HaloResult<Option<OcrAccessibilityHit>> {
        Ok(None)
    }
    ...
    /// `button`: "left" | "right" | "middle"
    async fn mouse_down(&self, _button: &str) -> HaloResult<()> {
        Err(HaloError::tool(
    ...
    /// `button`: "left" | "right" | "middle"
    async fn mouse_up(&self, _button: &str) -> HaloResult<()> {
        Err(HaloError::tool(
    ...
    async fn open_app(&self, _app_name: &str) -> HaloResult<OpenAppResult> {
        Err(HaloError::tool(
    ...
    async fn focus_display(&self, _display_id: Option<u32>) -> HaloResult<()> {
        Err(HaloError::tool(
    ...
    async fn list_apps(&self, _include_hidden: bool) -> HaloResult<Vec<AppInfo>> {
        Ok(vec![])
    }
    ...
    /// unsupported.
    async fn get_app_shortcuts(&self, _app: AppSelector) -> HaloResult<AppShortcutsSnapshot> {
        Err(HaloError::tool(
    ...
    async fn app_click(&self, _params: AppClickParams) -> HaloResult<AppStateSnapshot> {
        Err(HaloError::tool(
