...
impl GitTool {
    ...
    /// `-b` no longer fires on branch names like `feat/my-halo-x`.
        ...
        if let Some(dir) = working_directory {
            ...
            if trimmed.is_empty() {
                return context
                    ...
                    .ok_or_else(|| HaloError::tool("No workspace path available".to_string()));
            }
    ...
    async fn execute_remote_git_cli(
        ...
        let shell = context.ws_shell().ok_or_else(|| {
            HaloError::tool("Remote Git requires workspace shell (SSH)".to_string())
        })?;
        ...
        let (stdout, stderr, exit_code) = shell
            ...
            .await
            .map_err(|e| HaloError::tool(format!("Remote git failed: {}", e)))?;
    ...
    async fn execute_status(repo_path: &str) -> HaloResult<Value> {
        let status = GitService::get_status(repo_path)
            .await
            .map_err(|e| HaloError::tool(format!("Git status failed: {}", e)))?;
    ...
    async fn execute_diff(repo_path: &str, args: Option<&str>) -> HaloResult<Value> {
        ...
        let diff_output = GitService::get_diff(repo_path, &params)
            .await
            .map_err(|e| HaloError::tool(format!("Git diff failed: {}", e)))?;
    ...
    async fn execute_log(repo_path: &str, args: Option<&str>) -> HaloResult<Value> {
        ...
        let raw = execute_git_command_raw(repo_path, &command_arg_refs)
            .await
            .map_err(|e| HaloError::tool(format!("Git log failed: {e}")))?;
    ...
    async fn execute_add(repo_path: &str, args: Option<&str>) -> HaloResult<Value> {
        ...
        let result = GitService::add_files(repo_path, params)
            .await
            .map_err(|e| HaloError::tool(format!("Git add failed: {}", e)))?;
    ...
    async fn execute_commit(repo_path: &str, args: Option<&str>) -> HaloResult<Value> {
        ...
        let result = GitService::commit(repo_path, params)
            .await
            .map_err(|e| HaloError::tool(format!("Git commit failed: {}", e)))?;
    ...
    async fn execute_push(repo_path: &str, args: Option<&str>) -> HaloResult<Value> {
        ...
        let result = GitService::push(repo_path, params)
            .await
            .map_err(|e| HaloError::tool(format!("Git push failed: {}", e)))?;
    ...
    async fn execute_pull(repo_path: &str, args: Option<&str>) -> HaloResult<Value> {
        let tokens = Self::tokenize_args(args.unwrap_or(""));
        let parts = Self::positional_tokens(&tokens);
        ...
        let result = GitService::pull(repo_path, params)
            .await
            .map_err(|e| HaloError::tool(format!("Git pull failed: {}", e)))?;
    ...
    async fn execute_checkout(
        ...
        .map_err(|e| HaloError::tool(format!("Git checkout failed: {}", e)))?;
    ...
    async fn execute_branch(repo_path: &str, args: Option<&str>) -> HaloResult<Value> {
        let args_str = args.unwrap_or("");
        let tokens = Self::tokenize_args(args_str);
        ...
        if is_list {
            ...
            let branches = GitService::get_branches(repo_path, include_remote)
                .await
                .map_err(|e| HaloError::tool(format!("Git branch failed: {}", e)))?;
            ...
            || Self::tokens_contain_flag(&tokens, Some("-D"), None)
        {
            ...
            let branch_name = Self::positional_tokens(&tokens)
                ...
                .copied()
                .ok_or_else(|| {
                    HaloError::tool("Branch name is required for deletion".to_string())
                })?;

            let result = GitService::delete_branch(repo_path, branch_name, force)
                .await
                .map_err(|e| HaloError::tool(format!("Git branch delete failed: {}", e)))?;
...
- Do not add generated-by or co-author footers unless the user or repository convention asks for them"#.to_string())
    ...
    fn permission_intents(
        ...
        let operation = normalized
            ...
            .ok_or_else(|| HaloError::validation("operation is required".to_string()))?;
        let args = normalized
            .get("args")
            .and_then(Value::as_str)
            .map(str::trim)
    ...
    async fn call_impl(
        ...
        let operation = input
            .get("operation")
            .and_then(|v| v.as_str())
            .ok_or_else(|| HaloError::tool("operation is required".to_string()))?;
...
mod tests {
    ...
    fn flag_matching_ignores_substrings_inside_words() {
        let tokens = GitTool::tokenize_args("-c feat/my-halo-pages-manager upstream/main");
    ...
    fn plan_checkout_switch_create_with_start_point() {
        assert_eq!(
            GitTool::plan_checkout("switch", "-c feat/my-halo-pages-manager upstream/main"),
            CheckoutPlan::Create {
                branch: "feat/my-halo-pages-manager".to_string(),
