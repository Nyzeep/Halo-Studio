...
//!
//! When Halo runs on Windows but the open workspace is a **remote SSH** (POSIX) tree,
...
pub use halo_agent_tools::{
    build_halo_current_session_uri, is_halo_current_session_uri, is_halo_runtime_uri,
    is_halo_tool_uri, ParsedHaloCurrentSessionUri, ParsedHaloRuntimeUri,
    HALO_CURRENT_SESSION_URI_PREFIX, HALO_RUNTIME_URI_PREFIX,
};
use std::path::Path;

pub fn normalize_path(path: &str) -> String {
    halo_agent_tools::normalize_host_path(path)
}
...
pub fn resolve_path_with_workspace(
    ...
    halo_agent_tools::resolve_host_path_with_workspace(path, workspace_root)
        .map_err(|error| HaloError::tool(error.to_string()))
}

pub fn resolve_path(path: &str) -> HaloResult<String> {
    halo_agent_tools::resolve_host_path(path)
        .map_err(|error| HaloError::tool(error.to_string()))
}

pub fn normalize_runtime_relative_path(path: &str) -> HaloResult<String> {
    halo_agent_tools::normalize_runtime_relative_path(path)
        .map_err(|error| HaloError::tool(error.to_string()))
}

pub fn parse_halo_runtime_uri(path: &str) -> HaloResult<ParsedHaloRuntimeUri> {
    halo_agent_tools::parse_halo_runtime_uri(path)
        .map_err(|error| HaloError::tool(error.to_string()))
}
...
pub fn build_halo_runtime_uri(
    ...
    halo_agent_tools::build_halo_runtime_uri(workspace_scope, relative_path)
        .map_err(|error| HaloError::tool(error.to_string()))
}

/// POSIX absolute: after normalizing backslashes, path starts with `/`.
pub fn posix_style_path_is_absolute(path: &str) -> bool {
    halo_agent_tools::posix_style_path_is_absolute(path)
}
...
pub fn posix_resolve_path_with_workspace(
    ...
    halo_agent_tools::posix_resolve_path_with_workspace(path, workspace_root)
        .map_err(|error| HaloError::tool(error.to_string()))
}
...
pub fn resolve_workspace_tool_path(
    ...
    halo_agent_tools::resolve_workspace_tool_path(path, workspace_root, workspace_is_remote)
        .map_err(|error| HaloError::tool(error.to_string()))
}
...
mod tests {
    ...
    fn runtime_uri_round_trips_and_normalizes_separators() {
        let uri = build_halo_runtime_uri("workspace-123", r"plans\demo.plan.md").unwrap();
        assert_eq!(uri, "halo://runtime/workspace-123/plans/demo.plan.md");

        let parsed = parse_halo_runtime_uri(&uri).unwrap();
    ...
    fn runtime_uri_rejects_parent_directory_escape() {
        let err = build_halo_runtime_uri("workspace-123", "../secret.txt")
