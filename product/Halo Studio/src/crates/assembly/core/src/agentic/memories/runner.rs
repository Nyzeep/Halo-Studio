...
impl MemoryPhase2Consolidator for InternalAgentMemoryPhase2Consolidator {
    async fn consolidate(
        ...
        let request = InternalAgentExecutionRequest {
            ...
            context: HashMap::new(),
            delegation_policy: halo_runtime_ports::DelegationPolicy::top_level().spawn_child(),
...
impl MemoryPhase2Runner {
    ...
    pub async fn run_once(&self) -> HaloResult<Option<Phase2RunReport>> {
    ...
    ) -> HaloResult<Option<Phase2RunReport>> {
        let started_at = std::time::Instant::now();
        info!(
    ...
    async fn run_consolidation_agent(&self) -> HaloResult<String> {
        ...
        let config: crate::service::config::types::GlobalConfig = config_service
            .get_config(None)
            .await
            .map_err(|error| HaloError::service(format!("Failed to load config: {}", error)))?;
    ...
    async fn prune_prompt_artifacts(&self) -> HaloResult<()> {
    ...
    async fn confirm_phase2_job_ownership(&self, ownership_token: &str) -> HaloResult<()> {
        let job = self.db.get_phase2_job(PHASE2_JOB_KEY).await?;
    ...
    fn confirm_phase2_job_completion(&self, completed: bool) -> HaloResult<()> {
        if completed {
            Ok(())
        } else {
            Err(HaloError::service(
...
async fn phase2_retry_delay_seconds() -> HaloResult<i64> {
    let config = get_phase2_runtime_config().await;
    Ok(config
        .memories
...
async fn phase2_lease_seconds() -> HaloResult<i64> {
    let config = get_phase2_runtime_config().await;
...
fn select_phase2_model_id(
    ...
    model_ref
        ...
        .ok_or_else(|| {
            HaloError::service("No enabled model available for memory phase2".to_string())
        })
}
