...
impl LaunchReviewAgentTool {
    ...
    fn parse_invocation(input: &Value) -> HaloResult<LaunchReviewAgentInvocation> {
        ...
        let required_string = |field: &str| -> HaloResult<String> {
            let value = input
                .get(field)
                ...
                .ok_or_else(|| {
                    HaloError::tool(format!("{field} is required for LaunchReviewAgent"))
                })?;
            Ok(value.to_string())
        };
        ...
        let model_id = match input.get("model_id") {
            Some(value) => {
                let value = value
                    .as_str()
                    .ok_or_else(|| HaloError::tool("model_id must be a string".to_string()))?
                    .trim();
        ...
        let packet_id = match input.get("packet_id") {
            Some(value) => {
                let value = value
                    .as_str()
                    .ok_or_else(|| HaloError::tool("packet_id must be a string".to_string()))?
                    .trim();
                (!value.is_empty()).then(|| value.to_string())
            }
            None => None,
...
impl Tool for LaunchReviewAgentTool {
    ...
    ) -> HaloResult<Vec<crate::agentic::tools::framework::PermissionIntent>> {
        let subagent_type = input
            .get("subagent_type")
            .and_then(Value::as_str)
            .map(str::trim)
            .filter(|subagent_type| !subagent_type.is_empty())
            .ok_or_else(|| HaloError::validation("subagent_type is required".to_string()))?;
        Ok(vec![
