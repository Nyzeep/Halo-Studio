...
pub fn is_local_path_within_root(path: &Path, root: &Path) -> HaloResult<bool> {
...
pub(crate) fn canonicalize_local_path_best_effort(path: &Path) -> HaloResult<PathBuf> {
    if path.exists() {
...
mod tests {
    ...
    fn tool_restriction_errors_map_to_tool_errors() {
        let error: HaloError = ToolRestrictionError::Denied {
    ...
    fn local_path_containment_handles_missing_children() {
        let root =
            std::env::temp_dir().join(format!("halo-restrictions-{}", uuid::Uuid::new_v4()));
