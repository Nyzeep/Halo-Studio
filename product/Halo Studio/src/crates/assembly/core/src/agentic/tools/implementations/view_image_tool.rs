...
impl ViewImageTool {
    ...
    fn require_multimodal_tool_output(ctx: &ToolUseContext) -> HaloResult<()> {
    ...
    fn mime_type_for_image(bytes: &[u8]) -> HaloResult<&'static str> {
    ...
    fn path_from_input(input: &Value) -> HaloResult<&str> {
        input
            .get("path")
...
impl Tool for ViewImageTool {
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "path": {
                    "type": "string",
                    "description": "Path to an image file. Use an absolute local path, a workspace-relative path, or an exact halo:// URI returned by another tool."
                },
                "detail": {
                    "type": "string",
                    "enum": ["original"],
                    "description": "Optional detail override. Supported value: original. Halo preserves image detail when possible and may optimize bytes to fit the active provider limits."
                }
            },
            "required": ["path"],
    ...
    async fn call_impl(
        ...
        let processed = optimize_image_for_provider(bytes, &provider, Some(original_mime_type))
            .map_err(|err| {
                HaloError::tool(format!("unable to prepare image for model vision: {}", err))
            })?;
...
mod tests {
    ...
    fn remote_context(provider: &str, supports_images: bool) -> ToolUseContext {
        ...
        context.runtime_handles = halo_runtime_ports::ToolRuntimeHandles::new(
