...
impl Tool for AnalyzeImageTool {
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "path": {
                    "type": "string",
                    "description": "Path to the image file. Use an absolute local path, a workspace-relative path, or an exact halo:// URI."
                },
                "prompt": {
                    "type": "string",
    ...
    async fn call_impl(
        ...
        let original_mime_type = detect_mime_type_from_bytes(&bytes, None)
            .map_err(|err| HaloError::tool(format!("unsupported image file: {}", err)))?;
        ...
        .map_err(|err| HaloError::tool(format!("unable to prepare image: {}", err)))?;
        ...
        let client = get_global_ai_client_factory()
            .await
            .map_err(|err| HaloError::service(format!("AI client factory unavailable: {err}")))?
        ...
        let response = client
            ...
            .await
            .map_err(|err| HaloError::service(format!("Image analysis failed: {err}")))?;
