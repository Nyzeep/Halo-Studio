...
use halo_agent_runtime::skills::{SkillData, SkillInfo};
...
pub async fn resolve_review_capability(
    ...
    key: &str,
    fingerprint: &str,
) -> HaloResult<ResolvedReviewCapability> {
    ...
    if let Some(agent_key) = key.strip_prefix("agent:") {
        let agent = get_agent_registry()
            ...
            .ok_or_else(|| HaloError::tool("Review agent is no longer available".to_string()))?;
        let workspace_root = (!context.is_remote())
            .then(|| context.workspace_root())
            .flatten();
...
async fn load_review_skill(context: &ToolUseContext, skill_key: &str) -> HaloResult<SkillData> {
    ...
    if context.is_remote() {
        let fs = context.ws_fs().ok_or_else(|| {
            HaloError::tool("Remote review skill loading is unavailable".to_string())
        })?;
        let root = context
            .workspace
            .as_ref()
...
async fn load_discovered_review_skill(
    ...
    let markdown = if context.is_remote() {
        context
            ...
            .await
            .map_err(|error| HaloError::tool(format!("Failed to read review skill: {error}")))?
    } else {
...
fn capability_changed_error() -> HaloError {
    HaloError::tool(
...
fn bounded_selected_guidance(guidance: &str) -> HaloResult<String> {
