...
impl ImageAnalyzer {
    ...
    ) -> HaloResult<Vec<ImageAnalysisResult>> {
    ...
    async fn analyze_single_image(
        ...
        workspace_path: Option<PathBuf>,
        ai_client: Arc<AIClient>,
    ) -> HaloResult<ImageAnalysisResult> {
        ...
        let ai_response = ai_client.send_message(messages, None).await.map_err(|e| {
            ...
            HaloError::service(format!("Image analysis AI call failed: {}", e))
        })?;
    ...
    ) -> HaloResult<(Vec<u8>, Option<String>)> {
