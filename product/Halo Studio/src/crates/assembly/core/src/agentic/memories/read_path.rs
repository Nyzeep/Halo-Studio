...
You have access to a local Halo memory folder with guidance from prior runs. Use it when it is likely to help with the current request.
...
- If any memory files beyond the injected MEMORY_SUMMARY were used, append exactly one <halo-mem-citation> block as the very last content of the final reply.
- Use this structure:
<halo-mem-citation>
<citation_entries>
...
mod tests {
    ...
    async fn memory_read_path_wraps_summary_with_usage_instructions() {
        ...
        assert!(reminder.contains("<halo-mem-citation>"));
