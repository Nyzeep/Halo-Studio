...
use halo_services_integrations::web_tools::WebToolNetworkProvider;
...
- Get API data: {"url": "https://api.example.com/data", "format": "json"}"#
    ...
    fn permission_intents(
        ...
        let url = input
            ...
            .ok_or_else(|| HaloError::validation("url is required".to_string()))?;
    ...
    async fn call_impl(
        ...
        let url = input
            .get("url")
            .and_then(|v| v.as_str())
            .ok_or_else(|| HaloError::tool("url is required".to_string()))?;
        ...
        let (processed_content, content_representation, extractor, title) = match requested_format {
            ...
            RequestedFormat::Json => {
                serde_json::from_str::<Value>(&content)
                    .map_err(|e| HaloError::tool(format!("Invalid JSON response: {}", e)))?;
