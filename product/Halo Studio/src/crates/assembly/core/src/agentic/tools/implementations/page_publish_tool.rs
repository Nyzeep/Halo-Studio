//! PagePublish tool — create/update a Halo Page from inline files or a directory,
...
impl Tool for PagePublishTool {
    ...
    async fn description(&self) -> HaloResult<String> {
        Ok(
            r#"Publish a Halo Page to the account relay: upload content, freeze an immutable version, and optionally deploy it to production.

Requires a logged-in Halo account. This tool is only available after account login. Published Pages can be reviewed and managed later from the Pages scene.

When you produce self-contained publishable web content (landing page, docs site, or a Page with server/worker.js) and the user is logged in, proactively ask whether they want it published to Halo Page (suggest a slug and visibility). If they already said publish/deploy/上线, proceed with permission confirmation.

IMPORTANT — content source:
- Prefer `files` (inline path→UTF-8 content). For agent-authored pages, pass HTML/JS directly in `files` and call PagePublish. Do NOT Write/Edit page files into the user workspace just to publish, and do NOT create folders like halo-page/ unless the user explicitly asked to keep a local copy.
- Use `directory` only when the user already has (or explicitly wants) page sources on disk in a local workspace. Remote workspaces must use `files` so Halo never mistakes a remote path for a local path.
...
Use PageDeploy only to switch an already-saved version_id (rollback / promote a prior version)."#
    ...
    fn short_description(&self) -> String {
        "Publish a Halo Page (upload, save version, deploy).".to_string()
    }
    ...
    async fn call_impl(
        ...
        if !account_login_available() {
            return Err(HaloError::tool(
                "PagePublish requires a logged-in Halo account".to_string(),
            ));
        }
        ...
        let slug = input
            ...
            .map(str::trim)
            .filter(|s| !s.is_empty())
            .ok_or_else(|| HaloError::tool("slug is required".to_string()))?
            .to_string();
...
fn publish_result_for_assistant(
    ...
    if visibility != "public" {
        ...
        return format!(
            "Halo Page '{slug}' version '{version_id}' was {state} with {visibility} visibility. Open or copy it from the Pages scene/tool card so Halo can create a scoped browser-access link; do not share the raw Page URL."
        );
    }
    ...
    if deployed {
        if url.is_empty() {
            format!("Published public Halo Page '{slug}' version '{version_id}' to production.")
        } else {
            format!(
                "Published public Halo Page '{slug}' version '{version_id}'. Production URL: {url} \n\
                 Share this full absolute URL with the user, and keep a trailing space after the URL."
            )
        }
    } else if preview.is_empty() {
        format!("Saved public Halo Page '{slug}' version '{version_id}' (not deployed).")
    } else {
        format!(
            "Saved public Halo Page '{slug}' version '{version_id}' (not deployed). Preview URL: {preview} \n\
             Share this full absolute URL with the user, and keep a trailing space after the URL."
        )
    }
}

fn parse_files_map(value: Option<&Value>) -> HaloResult<Option<HashMap<String, String>>> {
    ...
    let obj = value.as_object().ok_or_else(|| {
        HaloError::tool("files must be an object of path→content strings".to_string())
    })?;
    if obj.is_empty() {
        return Err(HaloError::tool("files must not be empty".to_string()));
    }
