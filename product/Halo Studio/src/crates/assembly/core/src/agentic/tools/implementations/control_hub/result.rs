...
/// receives the structured error JSON instead of a plain HaloError text).
...
/// Builds a `HaloError` whose message is prefixed with `[CODE]` using the
...
/// back into.
pub fn coded_tool_error(code: ErrorCode, message: impl std::fmt::Display) -> HaloError {
    HaloError::tool(format!("[{}] {}", code.as_str(), message))
}

/// Convenience: lift a `HaloError` into the structured envelope using the
