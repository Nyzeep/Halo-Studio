...
#[cfg(test)]
use halo_agent_tools::{DEFAULT_MAX_TOOL_RESULT_CHARS, PERSISTED_OUTPUT_TAG};
use log::{debug, warn};
...
async fn persist_tool_result(
    ...
    preview_chars: usize,
    content_override: Option<String>,
) -> HaloResult<PersistedToolOutput> {
    let session_id = context.session_id.as_deref().ok_or_else(|| {
        HaloError::tool("A session id is required to persist tool results".to_string())
    })?;
...
async fn write_once(path: &Path, content: &str) -> HaloResult<()> {
...
fn serialize_tool_result_content(result: &ToolResult) -> HaloResult<(String, bool)> {
    ...
    serde_json::to_string_pretty(&result.result)
        ...
        .map(|text| (text, true))
        .map_err(|error| {
            HaloError::serialization(format!("Failed to serialize tool result: {}", error))
        })
}
