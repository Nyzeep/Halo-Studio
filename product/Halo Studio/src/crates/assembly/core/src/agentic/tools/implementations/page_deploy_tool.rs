//! PageDeploy tool — deploy a saved Halo Page version to production.
...
impl Tool for PageDeployTool {
    ...
    async fn description(&self) -> HaloResult<String> {
        Ok(
            r#"Switch the production pointer of an existing Halo Page to a previously saved version_id (rollback or promote a prior version).

Requires a logged-in Halo account. This tool is only available after account login. To create or update page content and publish, use PagePublish instead. Existing versions can also be reviewed from the Pages scene.
...
Preview a version at /p/{username}/{slug}/@v/{version_id}."#
    ...
    fn short_description(&self) -> String {
        "Deploy a saved Halo Page version to production.".to_string()
    }
    ...
    async fn call_impl(
        ...
        if !account_login_available() {
            return Err(HaloError::tool(
                "PageDeploy requires a logged-in Halo account".to_string(),
            ));
        }
        ...
        let version_id = input
            ...
            .map(str::trim)
            .filter(|s| !s.is_empty())
            .ok_or_else(|| HaloError::tool("version_id is required".to_string()))?
            .to_string();
...
fn deploy_result_for_assistant(
    ...
    if visibility != "public" {
        return format!(
            "Deployed Halo Page '{slug}' version '{version_id}' with {visibility} visibility. Open or copy it from the Pages scene/tool card so Halo can create a scoped browser-access link; do not share the raw Page URL."
        );
    }
    // Trailing space after URL keeps chat linkifiers from eating the next char.
    if url.is_empty() {
        format!("Deployed public Halo Page '{slug}' version '{version_id}' to production.")
    } else {
        format!(
            "Deployed public Halo Page '{slug}' version '{version_id}'. Production URL: {url} \n\
