...
use halo_agent_runtime::remote_file_delivery::user_workspace_relative_file_link;
use halo_core_types::SessionExecutionTargetKind;
...
",
    ...
    async fn get_language_preference(&self) -> HaloResult<String> {
    ...
    fn format_language_instruction(lang_code: &str) -> HaloResult<String> {
...
"
    ...
    /// - `{MEMORY_ROOT}` - Halo memory workspace root, used by internal memory agents
    ...
    pub async fn build_prompt_from_template(&self, template: &str) -> HaloResult<String> {
        ...
        // work_dir under .halo-studio/sessions/{SESSION_ID}/research/. Falls back to a
        ...
        if result.contains(PLACEHOLDER_DEEP_RESEARCH_REPORT_LINK) {
            ...
            let report_link = user_workspace_relative_file_link(
                &format!(".halo-studio/sessions/{session_id}/research/report.md"),
...
mod tests {
    ...
    async fn runtime_context_includes_computer_use_info_only_when_needed() {
        ...
        assert!(runtime_context.contains("## Local Client"));
        assert!(runtime_context.contains("Local Halo client OS:"));
    ...
    async fn runtime_context_omits_workspace_root_for_remote_execution() {
        ...
        assert!(runtime_context.contains("## Local Client"));
        assert!(runtime_context.contains("Local Halo client OS:"));
        assert!(runtime_context.contains("Computer use and UI automation operate on the local Halo desktop, even when workspace file and shell tools target a remote host."));
    ...
    async fn runtime_context_omits_local_client_os_for_remote_with_only_control_hub() {
        ...
        // The agent must NOT see "Local Halo client OS" because that signal
    ...
    async fn deep_research_report_link_defaults_to_workspace_relative_path() {
        ...
        assert_eq!(
            prompt,
            "[View full report](.halo-studio/sessions/session-1/research/report.md)"
        );
    }
    ...
    async fn deep_research_report_link_uses_computer_scheme_for_remote_delivery() {
        ...
        assert_eq!(
            prompt,
            "[View full report](computer://.halo-studio/sessions/session-1/research/report.md)"
        );
    }
    ...
    async fn workspace_context_identifies_the_managed_worktree_binding() {
        let execution_target = SessionExecutionTarget {
            ...
            root_path: "/managed/Halo-wt-1".to_string(),
        ...
        let workspace = WorkspaceBinding::new(
            ...
            PathBuf::from("/managed/Halo-wt-1"),
        )
        .with_project_root_path(PathBuf::from("/projects/Halo"))
        ...
        assert!(workspace_context.contains("/projects/Halo"));
