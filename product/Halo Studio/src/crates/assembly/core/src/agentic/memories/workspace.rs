...
use halo_services_core::session::MemoryWorkspaceGitError;
...
- This extension contains ad-hoc notes to add, update, or forget Halo memories.
...
pub async fn ensure_memory_workspace(root: &Path) -> HaloResult<()> {
...
pub async fn sync_phase2_workspace_inputs(root: &Path, rows: &[MemoryRow]) -> HaloResult<()> {
...
pub async fn prepare_memory_workspace(root: &Path) -> HaloResult<()> {
    ensure_memory_workspace(root).await?;
    seed_ad_hoc_memory_extension(root).await?;
    remove_phase2_workspace_diff(root).await?;
    halo_services_core::session::ensure_memory_workspace_git_baseline(root)
        .await
        .map_err(map_memory_workspace_baseline_error)
}

pub async fn seed_ad_hoc_memory_extension(root: &Path) -> HaloResult<()> {
...
pub async fn memory_workspace_diff(root: &Path) -> HaloResult<MemoryWorkspaceDiff> {
    remove_phase2_workspace_diff(root).await?;
    halo_services_core::session::memory_workspace_diff(root)
        .await
        .map_err(map_memory_workspace_diff_error)
}

pub async fn write_workspace_diff(root: &Path, diff: &MemoryWorkspaceDiff) -> HaloResult<()> {
    ensure_memory_workspace(root).await?;
    let path = phase2_workspace_diff_file(root);
    write_text_file_if_changed(
        &path,
        &halo_services_core::session::render_memory_workspace_diff_file(diff),
    )
    .await
}

pub async fn reset_memory_workspace_baseline(root: &Path) -> HaloResult<()> {
    remove_phase2_workspace_diff(root).await?;
    halo_services_core::session::reset_memory_workspace_git_baseline(root)
        .await
        .map_err(map_memory_workspace_baseline_error)
}

pub async fn clear_phase2_workspace_diff(root: &Path) -> HaloResult<()> {
    remove_phase2_workspace_diff(root).await
}

pub async fn remove_phase2_workspace_diff(root: &Path) -> HaloResult<()> {
    let path = phase2_workspace_diff_file(root);
...
fn map_memory_workspace_baseline_error(error: MemoryWorkspaceGitError) -> HaloError {
    map_memory_workspace_git_error("Memory workspace baseline task failed", error)
}

fn map_memory_workspace_diff_error(error: MemoryWorkspaceGitError) -> HaloError {
...
fn map_memory_workspace_git_error(
    ...
    match error {
        MemoryWorkspaceGitError::Join { source } => {
            HaloError::service(format!("{join_context}: {source}"))
        }
        error => HaloError::io(error.to_string()),
    }
}

async fn rebuild_raw_memories(root: &Path, rows: &[MemoryRow]) -> HaloResult<()> {
...
async fn sync_rollout_summaries(root: &Path, rows: &[MemoryRow]) -> HaloResult<()> {
...
async fn prune_rollout_summaries(dir: &Path, keep: &HashSet<String>) -> HaloResult<()> {
...
fn format_error(error: std::fmt::Error) -> HaloError {
    HaloError::service(format!("Failed to format memory workspace file: {}", error))
}

async fn write_text_file_if_changed(path: &Path, body: &str) -> HaloResult<()> {
...
pub async fn clear_directory_contents(root: &Path) -> HaloResult<()> {
    tokio::fs::create_dir_all(root).await.map_err(|error| {
        HaloError::io(format!(
...
pub async fn reset_memory_workspace(root: &Path) -> HaloResult<()> {
...
mod tests {
    ...
    async fn sync_phase2_workspace_inputs_writes_current_rows_and_prunes_stale_summaries() {
        ...
        assert!(!raw.contains("rollout_path: halo-session:session-a"));
        ...
        assert!(!summary_a.contains("rollout_path: halo-session:session-a"));
