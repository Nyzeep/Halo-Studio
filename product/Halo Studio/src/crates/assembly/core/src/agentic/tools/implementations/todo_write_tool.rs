...
"###.to_string())
    ...
    async fn call_impl(
        ...
        let todos = input
            ...
            .ok_or(HaloError::validation("Missing required field: todos"))?;
        ...
        for todo in todos {
            ...
            if let Some(obj) = todo_obj.as_object_mut() {
                if !obj.contains_key("status") {
                    return Err(HaloError::validation("Todo item missing status field"));
                }
                if !obj.contains_key("content") {
                    return Err(HaloError::validation("Todo item missing content field"));
                }
