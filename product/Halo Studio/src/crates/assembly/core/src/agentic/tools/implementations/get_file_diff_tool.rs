...
use halo_agent_runtime::deep_review::{
...
impl GetFileDiffTool {
    ...
    fn target_evidence(context: &ToolUseContext) -> HaloResult<Option<ReviewTargetEvidence>> {
        ...
        ReviewTargetEvidence::from_context_value(manifest).map_err(|error| {
            HaloError::tool(format!("Invalid prepared Review target evidence: {error}"))
        })
    }
    ...
    fn pull_request_file_diff_route(
        ...
        logical_path: &str,
    ) -> HaloResult<ProviderFileDiffRoute> {
        let pull_request = evidence.pull_request().ok_or_else(|| {
            HaloError::tool("Prepared pull request Review identity is unavailable".to_string())
        })?;
        ...
        let base_revision = evidence.base_revision().ok_or_else(|| {
            HaloError::tool("Prepared pull request base revision is unavailable".to_string())
        })?;
        let head_revision = evidence.head_revision().ok_or_else(|| {
            HaloError::tool("Prepared pull request head revision is unavailable".to_string())
        })?;
    ...
    fn exact_review_target(
        relative_path: &str,
        context: &ToolUseContext,
    ) -> HaloResult<Option<ExactReviewTarget>> {
        ...
        let Some((base, head)) = evidence.diff_revisions_for_path(relative_path) else {
            if evidence.source()
                != halo_agent_runtime::deep_review::ReviewTargetEvidenceSource::Workspace
            {
                return Err(HaloError::tool(
    ...
    async fn exact_review_diff(&self, request: ExactReviewDiffRequest<'_>) -> HaloResult<Value> {
    ...
    async fn try_git_diff(
        ...
        let prepared_workspace_root = if review_safe {
            match review_workspace_root {
                Some(root) => Some(root),
                None => {
                    return Some(Err(HaloError::tool(
        ...
        let repo_root = match get_repository_root(repository_discovery_root) {
            ...
            Err(e) => {
                if review_safe {
                    return Some(Err(HaloError::tool(format!(
    ...
    fn return_full_content(&self, file_path: &Path) -> HaloResult<Value> {
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "file_path": {
                    "type": "string",
                    "description": "The file to get diff for. Use a workspace-relative path, an absolute path inside the current workspace, or an exact halo:// URI returned by another tool."
                }
            },
            "required": ["file_path"],
    ...
    async fn call_impl(
        ...
        if let Some((base_revision, head_revision, paths, fingerprint)) = exact_target {
            let workspace_root = context.workspace_root().ok_or_else(|| {
                HaloError::tool("Workspace root is required for Review target diff".to_string())
            })?;
            let data = self
        ...
        if resolved.uses_remote_workspace_backend() {
            let ws_fs = context.ws_fs().ok_or_else(|| {
                HaloError::tool("Workspace file system not available for remote diff".to_string())
            })?;
            let content = ws_fs
...
mod tests {
    ...
    async fn provider_acquisition_budget_stops_pull_request_diff_before_provider_io() {
        ...
        for _ in
            0..halo_agent_runtime::deep_review::REVIEW_PROVIDER_DIFF_MAX_ACQUISITIONS_PER_TURN
        {
