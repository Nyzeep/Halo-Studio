...
name: halo-canvas
description: >-
  A Halo Studio Canvas is a live React app that the user can open beside the chat.
  ...
  read this skill whenever you create, edit, or debug a Halo Studio Canvas artifact.
metadata:
  surfaces:
    - ide
---
A canvas is a single TSX source artifact that Halo Studio compiles so the user can open it beside the chat. Follow the workflow below in order.
...
Be creative. The SDK gives you expressive building blocks — use them in whatever combination best serves the content. But avoid slop: no gradients, no emojis, no box-shadows, no rainbow coloring. Halo Studio canvases are flat, minimal, and purposeful.
...
Whenever you mention a canvas to the user — one you created, updated, or want them to open — **always** include the `halo-canvas://...` artifact reference returned by the Canvas tool. Use the artifact title or a short descriptive label near the reference; do not refer to a canvas by name alone without the reference.

When you create a canvas, add a short note in your chat response telling the user they can open it beside the chat, with that `halo-canvas://...` reference:
