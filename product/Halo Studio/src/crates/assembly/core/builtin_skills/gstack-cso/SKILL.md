...
When this skill is invoked by Halo Studio Team Mode, this skill supplies the security-review lens. Use existing Task sub-agents for independent security evidence gathering, then make final severity and remediation calls in the main Team session.
...
The bash blocks throughout this skill show WHAT patterns to search for, not HOW to run them. Use Halo Studio's Grep tool (which handles permissions and access correctly) rather than raw bash grep. The bash blocks are illustrative examples — do NOT copy-paste them into a terminal. Do NOT use `| head` to truncate results.
...
Scan installed Halo Studio skills for malicious patterns. 36% of published skills have security flaws, 13.4% are outright malicious (Snyk ToxicSkills research).
...
**Trend Tracking:** If prior reports exist in `.halo-studio/team/security-reports/`:
```
...
```bash
mkdir -p .halo-studio/team/security-reports
```

Write findings to `.halo-studio/team/security-reports/{date}-{HHMMSS}.json` using this schema:
...
If `.halo-studio/team/` is not in `.gitignore`, note it in findings — security reports should stay local.
