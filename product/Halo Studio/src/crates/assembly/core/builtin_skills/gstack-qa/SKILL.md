...
When this skill is invoked by Halo Studio Team Mode, this skill supplies the QA methodology. Use existing Task sub-agents for independent testing tracks, then keep triage and fix ownership explicit in the main Team session.
...
| Mode | full | `--regression .halo-studio/team/qa-reports/baseline.json` |
| Output dir | `.halo-studio/team/qa-reports/` | `Output to /tmp/qa` |
...
```bash
mkdir -p .halo-studio/team/qa-reports/screenshots
```
...
1. **Project-scoped test plans:** Check `$HOME/.halo-studio/team/projects/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   setopt +o nomatch 2>/dev/null || true  # zsh compat
   SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" | tr -cd A-Za-z0-9._-)
   ls -t $HOME/.halo-studio/team/projects/$SLUG/*-test-plan-*.md 2>/dev/null | head -1
   ```
...
- Include full attribution comment:
  ...
  // Report: .halo-studio/team/qa-reports/qa-report-{domain}-{date}.md
  ```
...
**Local:** `.halo-studio/team/qa-reports/qa-report-{domain}-{YYYY-MM-DD}.md`
...
```
Write to `$HOME/.halo-studio/team/projects/{slug}/{user}-{branch}-test-outcome-{datetime}.md`
