...
Generates a comprehensive engineering retrospective analyzing commit history, work patterns, and code quality metrics. Team-aware: identifies the user running the command, then analyzes every contributor with per-person praise and growth opportunities. Designed for a senior IC/CTO-level builder using Halo Studio as a force multiplier.

## Halo Studio Team Mode Dispatch

When this skill is invoked by Halo Studio Team Mode, this skill supplies the retrospective methodology. Use existing Task sub-agents for independent read-only analysis tracks, then keep the final retro narrative in the main Team session.
...
cat $HOME/.halo-studio/team/greptile-history.md 2>/dev/null || true
...
cat $HOME/.halo-studio/team/analytics/skill-usage.jsonl 2>/dev/null || true
...
**Greptile signal (if history exists):** Read `$HOME/.halo-studio/team/greptile-history.md` (fetched in Step 1, command 8). Filter entries within the retro time window by date. Count entries by type: `fix`, `fp`, `already-fixed`. Compute signal ratio: `(fix + already-fixed) / (fix + already-fixed + fp)`. If no entries exist in the window or the file doesn't exist, skip the Greptile metric row. Skip unparseable lines silently.
...
**Skill Usage (if analytics exist):** Read `$HOME/.halo-studio/team/analytics/skill-usage.jsonl` if it exists. Filter entries within the retro time window by `ts` field. Separate skill activations (no `event` field) from hook fires (`event: "hook_fire"`). Aggregate by skill name. Present as:
...
**Eureka Moments (if logged):** Read `$HOME/.halo-studio/team/analytics/eureka.jsonl` if it exists. Filter entries within the retro time window by `ts` field. For each eureka moment, show the skill that flagged it, the branch, and a one-line summary of the insight. Present as:
...
cat $HOME/.halo-studio/team/projects/$SLUG/*-reviews.jsonl 2>/dev/null | grep '"skill":"ship"' | grep '"plan_items_total"' || echo "NO_PLAN_DATA"
```
...
**AI collaboration note:** If many commits have `Co-Authored-By` AI trailers (e.g., Halo Studio, Copilot), note the AI-assisted commit percentage as a team metric. Frame it neutrally — "N% of commits were AI-assisted" — without judgment.
...
Use Halo Studio's built-in session/project metadata and ordinary filesystem inspection.
Do not locate, build, or run external `global session discovery` binaries. If Halo Studio
...
- Behavioral patterns (e.g., "outside-voice sub-agent used exclusively for myapp, Halo Studio for everything else")
...
- Halo Studio: N sessions across M repos — patterns observed
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t $HOME/.halo-studio/team/retros/global-*.json 2>/dev/null | head -5
```
...
```bash
mkdir -p $HOME/.halo-studio/team/retros
```
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls $HOME/.halo-studio/team/retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `$HOME/.halo-studio/team/retros/global-${today}-${next}.json`:
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `$HOME/.halo-studio/team/retros/` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
