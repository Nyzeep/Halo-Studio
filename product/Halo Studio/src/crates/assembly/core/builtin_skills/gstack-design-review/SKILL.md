...
When this skill is invoked by Halo Studio Team Mode, this skill supplies the live design-audit methodology. Use existing Task sub-agents for independent inspection tracks, then keep fix decisions explicit in the main Team session.
...
**Browser/desktop QA tooling:** Use agent-browser for browser QA and Halo Studio ComputerUse only for native desktop surfaces it cannot reach. Save QA artifacts under `.halo-studio/team/qa-reports/`.
...
**Find the Halo Studio image/design capability (optional — enables target mockup generation):**
...
If `Halo Studio image/design capability is available`: during the fix loop, you can generate "target mockups" showing what a finding should look like after fixing. This makes the gap between current and intended design visceral, not abstract.

If `Halo Studio image/design capability is unavailable`: skip mockup generation — the fix loop works without it.

**Create output directories:**

```bash
SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" | tr -cd A-Za-z0-9._-)
REPORT_DIR=$HOME/.halo-studio/team/projects/$SLUG/designs/design-audit-$(date +%Y%m%d)
...
**Local:** `.halo-studio/team/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:**
```bash
SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" | tr -cd A-Za-z0-9._-) && mkdir -p $HOME/.halo-studio/team/projects/$SLUG
```
Write to: `$HOME/.halo-studio/team/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md`
...
```
$HOME/.halo-studio/team/projects/$SLUG/designs/design-audit-{YYYYMMDD}/
...
### 8a.5. Target Mockup (if Halo Studio image/design capability is available)

If the Halo Studio image/design capability is available and the finding involves visual layout, hierarchy, or spacing (not just a CSS value fix like wrong color or font-size), generate a target mockup showing what the corrected version should look like:

```bash
Halo Studio image/design capability generate --brief "<description of the page/component with the finding fixed, referencing DESIGN.md constraints>" --output "$REPORT_DIR/screenshots/finding-NNN-target.png"
```
...
SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" | tr -cd A-Za-z0-9._-) && mkdir -p $HOME/.halo-studio/team/projects/$SLUG
```
Write a one-line summary to `$HOME/.halo-studio/team/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
