...
## Halo Studio Team Mode Dispatch

When this skill is invoked by Halo Studio Team Mode, this skill supplies the design-review lens. Use existing Task sub-agents for independent UI/UX discovery only when they add evidence, then keep design decisions in the main Team session.
...
### The Halo Studio image/design capability — YOUR PRIMARY TOOL

You have the **Halo Studio image/design capability**, an AI mockup generator that creates real visual mockups
...
Setup is handled by the DESIGN SETUP section below. If `Halo Studio image/design capability is available` is printed,
...
Use Halo Studio built-in image/design and browser/computer-use capabilities. Do not install, build, or call external `design` or `browse` binaries. Generate mockups, comparison boards, screenshots, and visual QA artifacts through Halo Studio tools; if a visual generation capability is not available in the current session, fall back to HTML wireframes and code-level design review.

**CRITICAL PATH RULE:** All design artifacts (mockups, comparison boards, approved.json)
MUST be saved to `$HOME/.halo-studio/team/projects/$SLUG/designs/`, NEVER to `.context/`,
...
## Step 0.5: Visual Mockups (DEFAULT when Halo Studio image/design capability is available)

If the plan involves any UI — screens, pages, components, visual changes — AND the
Halo Studio image/design capability is available (`Halo Studio image/design capability is available` was printed during setup), **generate
mockups immediately.** Do not ask permission. This is the default behavior.

Tell the user: "Generating visual mockups with the Halo Studio image/design capability. This is how we
review design — real visuals, not text descriptions."

The ONLY time you skip mockups is when:
- `Halo Studio image/design capability is unavailable` was printed (visual generation unavailable)
...
`$HOME/.halo-studio/team/projects/$SLUG/designs/` (user config directory, not project files).
...
- `mkdir -p $HOME/.halo-studio/team/projects/$SLUG/designs/...`
- `Halo Studio image/design capability generate`, `Halo Studio image/design capability variants`, `Halo Studio image/design capability compare`, `Halo Studio image/design capability iterate`, `Halo Studio image/design capability evolve`, `Halo Studio image/design capability check`
...
_DESIGN_DIR=$HOME/.halo-studio/team/projects/$SLUG/designs/<screen-name>-$(date +%Y%m%d)
mkdir -p "$_DESIGN_DIR"
...
```bash
Halo Studio image/design capability variants --brief "<description assembled from plan + DESIGN.md constraints>" --count 3 --output-dir "$_DESIGN_DIR/"
```

After generation, run a cross-model quality check on each variant:

```bash
Halo Studio image/design capability check --image "$_DESIGN_DIR/variant-A.png" --brief "<the original brief>"
```
...
```bash
Halo Studio image/design capability compare --images "$_DESIGN_DIR/variant-A.png,$_DESIGN_DIR/variant-B.png,$_DESIGN_DIR/variant-C.png" --output "$_DESIGN_DIR/design-board.html" --serve
```
...
3. Generate new variants with `Halo Studio image/design capability iterate` or `Halo Studio image/design capability variants` using updated brief
4. Create new board: `Halo Studio image/design capability compare --images "..." --output "$_DESIGN_DIR/design-board.html"`
...
**POLLING FALLBACK:** Only use polling if `Halo Studio image/design capability serve` fails (no port available).
...
**If `Halo Studio image/design capability is unavailable`:** Tell the user: "The Halo Studio image/design capability isn't set up yet. Proceeding with text-only review, but you're missing the best part." Then proceed to review passes with text-based review.
...
**If a suitable Halo Studio outside-voice or review sub-agent is available**, launch both voices simultaneously:

1. **outside-voice sub-agent design voice** (via Bash):
```bash
TMPERR_DESIGN=$(mktemp /tmp/codex-design-XXXXXXXX)
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
Use the Halo Studio Task tool to dispatch this prompt to a suitable independent read-only outside-voice sub-agent.
...
2. **Independent design subagent** (via Halo Studio Task tool):
...
═══════════════════════════════════════════════════════════════
  Check                                    Halo Studio  outside-voice sub-agent  Consensus
  ─────────────────────────────────────── ─────── ─────── ─────────
...
**Log the result:**
```bash
true # Halo Studio Team Mode has no external review-log helper
```
...
### "Show me what 10/10 looks like" (uses Halo Studio image/design capability)

If `Halo Studio image/design capability is available` was printed during setup AND a dimension rates below 7/10,
offer to generate a visual mockup showing what the improved version would look like:

```bash
Halo Studio image/design capability generate --brief "<description of what 10/10 looks like for this dimension>" --output /tmp/gstack-ideal-<dimension>.png
```
...
If the Halo Studio image/design capability is not available, skip this and continue with text-based
...
Use only Halo Studio in-session memory, project docs, `.halo-studio/team/` artifacts, git history, TODO files, and prior design/review artifacts. Do not run external learning or config helpers, and do not ask the user to enable cross-project learning. If a relevant prior artifact is found, cite it as: `Prior Halo Studio context applied: <source>`.
...
If yes, use `Halo Studio image/design capability iterate` with feedback summarizing the changes, or `Halo Studio image/design capability variants` with an updated brief. Save to the same `$_DESIGN_DIR` directory.
...
* **NEVER use AskUserQuestion to ask which variant the user prefers.** Always create a comparison board first (`Halo Studio image/design capability compare --serve`) and open it in the browser. The board has rating controls, comments, remix/regenerate buttons, and structured feedback output. Use AskUserQuestion ONLY to notify the user the board is open and wait for them to finish — not to present variants inline and ask "which do you prefer?" That is a degraded experience.
...
|----------------|-------------|-----------|-------|
| [screen name]  | $HOME/.halo-studio/team/projects/$SLUG/designs/[folder]/[filename].png | [brief description] | [constraints from review] |
```
...
`$HOME/.halo-studio/team/` (user config directory, not project files). The skill preamble
already writes to `$HOME/.halo-studio/team/sessions/` and `$HOME/.halo-studio/team/analytics/` — this is
...
```bash
true # Halo Studio Team Mode reads review context from the current session
```
...
- **Adversarial Review (automatic):** Always-on for every review. Every diff gets both Halo Studio adversarial subagent and outside-voice sub-agent adversarial challenge. Large diffs (200+ lines) additionally get outside-voice sub-agent structured review with P1 gate. No configuration needed.
...
| outside-voice sub-agent Review | \`Halo Studio Task outside-voice review\` | Independent 2nd opinion | {runs} | {status} | {findings} |
...
- **CROSS-MODEL:** (only if both Halo Studio and outside-voice sub-agent reviews exist) — overlap analysis
...
```bash
true # Halo Studio Team Mode has no external telemetry helper
```
...
`inferred` (AI deduction), `cross-model` (both Halo Studio and outside-voice sub-agent agree).
