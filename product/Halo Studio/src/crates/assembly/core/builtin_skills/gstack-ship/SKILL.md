...
When this skill is invoked by Halo Studio Team Mode, this skill supplies the release-engineering checklist. Use existing Task sub-agents only for read-only readiness checks that can run independently, then keep all mutations in the main Team session.
...
For Design Review: run `source <(true # Halo Studio Team Mode infers diff scope with git/rg <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
[ -f .halo-studio/team/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
If user picks H → write `.halo-studio/team/no-test-bootstrap` and continue without tests.
...
If user picks C → write `.halo-studio/team/no-test-bootstrap`. Tell user: "If you change your mind later, delete `.halo-studio/team/no-test-bootstrap` and re-run." Continue without tests.
...
```bash
SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" | tr -cd A-Za-z0-9._-) && mkdir -p $HOME/.halo-studio/team/projects/$SLUG
USER=$(whoami)
DATETIME=$(date +%Y%m%d-%H%M%S)
```

Write to `$HOME/.halo-studio/team/projects/{slug}/{user}-{branch}-ship-test-plan-{datetime}.md`:
...
# Compute project slug for $HOME/.halo-studio/team/projects/ lookup
_PLAN_SLUG=$(git remote get-url origin 2>/dev/null | sed 's|.*[:/]\([^/]*/[^/]*\)\.git$|\1|;s|.*[:/]\([^/]*/[^/]*\)$|\1|' | tr '/' '-' | tr -cd 'a-zA-Z0-9._-') || true
_PLAN_SLUG="${_PLAN_SLUG:-$(basename "$PWD" | tr -cd 'a-zA-Z0-9._-')}"
# Search common plan file locations (project designs first, then personal/local)
for PLAN_DIR in "$HOME/.halo-studio/team/projects/$_PLAN_SLUG" "$HOME/.halo-studio/team/plans" "$HOME/.codex/plans" ".halo-studio/team/plans"; do
  [ -d "$PLAN_DIR" ] || continue
...
```bash
source <(true # Halo Studio Team Mode infers diff scope with git/rg <base> 2>/dev/null)
```
...
If a suitable Halo Studio outside-voice or review sub-agent is available, run a lightweight design check on the diff:
...
```bash
source <(true # Halo Studio Team Mode infers diff scope with git/rg <base> 2>/dev/null) || true
...
```bash
true # Halo Studio Team Mode has no external specialist-stats helper 2>/dev/null || true
```
...
For each selected specialist, launch an independent subagent via Halo Studio's Task tool.
...
```bash
true # Halo Studio Team Mode has no external learnings helper
```
...
Every diff gets adversarial review from both Halo Studio and outside-voice sub-agent. LOC is not a proxy for risk — a 5-line auth change can be critical.
...
which codex 2>/dev/null && echo "CODEX_AVAILABLE" || echo "CODEX_NOT_AVAILABLE"
# Legacy opt-out — only gates outside-voice sub-agent passes, Halo Studio always runs
OLD_CFG="" # Halo Studio Team Mode has no external codex_reviews config
echo "DIFF_SIZE: $DIFF_TOTAL"
echo "OLD_CFG: ${OLD_CFG:-not_set}"
```

If `OLD_CFG` is `disabled`: skip outside-voice sub-agent passes only. Halo Studio adversarial subagent still runs (it's free and fast). Jump to the "Halo Studio adversarial subagent" section.
...
### Halo Studio adversarial subagent (always runs)
...
If the subagent fails or times out: "Halo Studio adversarial subagent unavailable. Continuing."

---

### outside-voice sub-agent adversarial challenge (always runs when available)

If a suitable Halo Studio outside-voice or review sub-agent is available AND `OLD_CFG` is NOT `disabled`:
...
- **Outside-voice unavailable:** If the selected Halo Studio sub-agent cannot run, skip this informational pass and continue with the main-session review.
...
If outside-voice sub-agent is not available in the current Halo Studio runtime, run the Halo Studio adversarial path only and note that cross-model coverage was skipped.
...
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
cd "$_REPO_ROOT"
Use the Halo Studio Task tool to dispatch a suitable independent read-only structured review sub-agent over the diff.
```
...
If A: address the findings. After fixing, re-run tests (Step 3) since code has changed. Re-run `Halo Studio Task outside-voice review` to verify.

Read stderr for errors (same error handling as outside-voice sub-agent adversarial above).

After stderr: `rm -f "$TMPERR"`

If `DIFF_TOTAL < 200`: skip this section silently. The Halo Studio + outside-voice sub-agent adversarial passes provide sufficient coverage for smaller diffs.
...
════════════════════════════════════════════════════════════
  ...
  Unique to Halo Studio structured review: [from earlier step]
  Unique to Halo Studio adversarial: [from subagent]
  Unique to outside-voice sub-agent: [from codex adversarial or code review, if ran]
  Models used: Halo Studio structured ✓  Halo Studio adversarial ✓/✗  outside-voice sub-agent ✓/✗
════════════════════════════════════════════════════════════
```
...
`user::halo-system::gstack-document-release` with the Skill tool and execute its full workflow:

1. Load `user::halo-system::gstack-document-release` with the Skill tool.
...
Append to `$HOME/.halo-studio/team/projects/$SLUG/$BRANCH-reviews.jsonl`:

```bash
echo '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","version":"VERSION","branch":"BRANCH"}' >> $HOME/.halo-studio/team/projects/$SLUG/$BRANCH-reviews.jsonl
```
