...
3. `--halo-*` 主题变量（见 SKILL.md 的"主题集成"章节）——所有颜色都优先 `var(--halo-xxx, fallback)`
 ...
 * Palette:
 *   - dominant: var(--halo-bg) / var(--halo-text)
 *   - supporting: var(--halo-bg-secondary), var(--halo-border)
 *   - accent: var(--halo-accent)  // 仅用于关键 CTA / 选中态
 * Typography:
 *   - heading: 600, 18-22px
 *   - body:    400, 13-14px
 *   - caption: 400, 11-12px, --halo-text-muted
...
| 反模式 | 替代方案 |
|---|---|
| 默认蓝紫渐变 / Aurora 风背景 | 用 `var(--halo-bg)` 或单色 + 一处微妙强调 |
...
| Inter / Roboto / Arial 兜底就完事 | `var(--halo-font-sans)` 优先，fallback 写完整：`-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif` |
...
1. **首选**：直接 `var(--halo-*)` 系列，让小应用与宿主主题协同。
...
```css
:root {
  --font-heading: var(--halo-font-sans, -apple-system, 'Segoe UI', sans-serif);
  --font-body:    var(--halo-font-sans, -apple-system, 'Segoe UI', sans-serif);
  --font-mono:    var(--halo-font-mono, ui-monospace, SFMono-Regular, monospace);
}
```
...
- 圆角档位：`var(--halo-radius)`（卡片）+ `var(--halo-radius-lg)`（浮层）；输入框可固定 4-6px。
...
## 七、与 Halo Studio 工具型 MiniApp 的契合度

绝大多数 Halo Studio 用户产出的小应用是**工具型**（regex 调试 / git 视图 / 编码自拍 / 计算器…），它们的设计调性应当：
...
读它们的 `style.css` 顶部注释和 `meta.json` 的 `i18n` 块，是最快理解"Halo Studio 味道"的方式。
