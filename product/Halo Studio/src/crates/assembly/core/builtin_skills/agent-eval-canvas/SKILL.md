...
description: >-
  Create a Halo Studio Canvas for single-case agent evaluation and trajectory
...
Use this skill to produce a session-scoped Halo Studio Canvas for a **single agent
...
Read and follow `halo-canvas` first. It defines the Canvas tool workflow,
...
final response, give the returned `halo-canvas://...` artifact reference and
