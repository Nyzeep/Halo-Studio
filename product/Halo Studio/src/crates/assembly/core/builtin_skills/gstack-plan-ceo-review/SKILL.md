...
When this skill is invoked by Halo Studio Team Mode, this skill supplies the CEO/product-review lens. Use existing Task sub-agents to collect independent evidence, then make the final CEO judgment in the main Team session.
...
DESIGN=$(ls -t $HOME/.halo-studio/team/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t $HOME/.halo-studio/team/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
HANDOFF=$(ls -t $HOME/.halo-studio/team/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
```bash
SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" | tr -cd A-Za-z0-9._-) && mkdir -p $HOME/.halo-studio/team/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p $HOME/.halo-studio/team/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv $HOME/.halo-studio/team/projects/$SLUG/ceo-plans/{old-plan}.md $HOME/.halo-studio/team/projects/$SLUG/ceo-plans/archive/
```

Write to `$HOME/.halo-studio/team/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> $HOME/.halo-studio/team/analytics/spec-review.jsonl 2>/dev/null || true
```
...
On any outside-voice sub-agent error, fall back to the Halo Studio adversarial subagent.
...
SOURCE = "codex" if outside-voice sub-agent ran, "subagent" if a Halo Studio Task sub-agent ran.
...
rm -f $HOME/.halo-studio/team/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null || true
```
...
- **B)** Keep in `$HOME/.halo-studio/team/projects/` only (local, personal reference)
- **C)** Skip
