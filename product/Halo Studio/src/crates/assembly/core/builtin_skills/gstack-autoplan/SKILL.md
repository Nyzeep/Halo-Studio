...
This is qualitatively different from taste decisions. When Halo Studio and outside-voice sub-agent both
...
All prompts sent to outside-voice sub-agent (via `Halo Studio Task outside-voice dispatch` or `Halo Studio Task outside-voice review`) MUST be prefixed with
...
echo "RESTORE_PATH=$HOME/.halo-studio/team/projects/$SLUG/${BRANCH}-autoplan-restore-${DATETIME}.md"
```
...
- Discover design docs: `ls -t $HOME/.halo-studio/team/projects/$SLUG/*-design-*.md 2>/dev/null | head -1`
...
- Detect DX scope: grep the plan for developer-facing terms (API, endpoint, REST,
  ...
  package, npm, pip, import, require, SKILL.md, skill template, Halo Studio, MCP, agent,
  ...
  on top of) or if an AI agent is the primary user (OpenClaw actions, Halo Studio skills,
  MCP servers).

### Step 3: Load review skills

Load each stable key with the Skill tool:
- `user::halo-system::gstack-plan-ceo-review`
- `user::halo-system::gstack-plan-design-review` (only if UI scope detected)
- `user::halo-system::gstack-plan-eng-review`
...
Load `user::halo-system::gstack-plan-ceo-review` with the Skill tool and follow all sections at full depth.
...
Load `user::halo-system::gstack-plan-design-review` with the Skill tool and follow all 7 dimensions at full depth.
Override: every AskUserQuestion → auto-decide using the 6 principles.
...
Load `user::halo-system::gstack-plan-eng-review` with the Skill tool and follow all sections at full depth.
Override: every AskUserQuestion → auto-decide using the 6 principles.
...
- Test plan: generate artifact at `$HOME/.halo-studio/team/projects/$SLUG/{user}-{branch}-test-plan-{datetime}.md`
...
- [ ] Test plan artifact written to disk at $HOME/.halo-studio/team/projects/$SLUG/
- [ ] "NOT in scope" section written
- [ ] "What already exists" section written
