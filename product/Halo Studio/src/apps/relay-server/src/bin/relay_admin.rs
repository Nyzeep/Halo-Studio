...
//! (e.g. a Halo client self-deploying its relay) only sends derived
...
async fn main() -> Result<()> {
    ...
    let pool = halo_relay_service::db::connect_for_admin(&cli.db).await?;

    match cli.command {
        Command::AddUser { username, password } => {
            let password = resolve_password(password)?;
            let user_id =
                halo_relay_service::admin::add_user(&pool, &username, &password).await?;
            println!("Created account: username='{username}' user_id={user_id}");
        }
        Command::ListUsers => {
            let users = halo_relay_service::admin::list_users(&pool).await?;
            if users.is_empty() {
        ...
        Command::DeleteUser { username } => {
            halo_relay_service::admin::delete_user(&pool, &username).await?;
            println!("Deleted account: {username}");
        }
        Command::ResetPassword { username, password } => {
            let password = resolve_password(password)?;
            halo_relay_service::admin::reset_password(&pool, &username, &password).await?;
        ...
        Command::RenameUser {
            ...
            halo_relay_service::admin::rename_user(&pool, &username, &new_username).await?;
        ...
        Command::ImportUser { file } => {
            ...
            let import: halo_relay_service::admin::ImportableAccount =
                serde_json::from_str(&json).map_err(|e| anyhow!("parse import JSON: {e}"))?;
            let user_id = halo_relay_service::admin::import_user(&pool, &import).await?;
            println!(
