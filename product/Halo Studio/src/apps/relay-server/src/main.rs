...
use halo_relay_service::{DiskAssetStore, RoomManager, WebAssetStore};
...
async fn main() -> anyhow::Result<()> {
    ...
    info!("Halo Relay Server v{}", env!("CARGO_PKG_VERSION"));
    ...
    let db = if let Some(path) = &cfg.db_path {
        let pool = halo_relay_service::db::connect(path)
            .await
            .with_context(|| {
    ...
    let page_browser_auth = match (
        ...
        (Some(public_base_url), Some(auth_base_url)) => Some(
            halo_relay_service::PageBrowserAuthConfig::new(public_base_url, auth_base_url)
    ...
    let mut app = halo_relay_service::build_relay_router_with_page_data_origins_and_page_auth(
        room_manager,
        asset_store,
        start_time,
        db,
