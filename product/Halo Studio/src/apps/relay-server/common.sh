#!/usr/bin/env bash
# Shared helpers for Halo relay-server deploy/start/stop/restart scripts.
# Sourced by the other *.sh files in this directory (not executed directly).

CONTAINER_NAME="${CONTAINER_NAME:-halo-relay}"
RELAY_ADMIN_DB="${RELAY_ADMIN_DB:-/app/data/halo_relay.db}"
...
# ~/.halo-studio/docker-config and its config.json owned by root, and every later
