...
impl ExecMode {
    ...
    pub(super) fn get_git_diff_from_baseline(
        ...
        let repo_root_output = halo_core::util::process_manager::create_command("git")
        ...
        let mut tracked_command = halo_core::util::process_manager::create_command("git");
        tracked_command
        ...
        let untracked = halo_core::util::process_manager::create_command("git")
        ...
        for relative_path in untracked.stdout.split(|byte| *byte == 0) {
            ...
            let untracked_patch = halo_core::util::process_manager::create_command("git")
                .args([
                    "diff",
                    "--no-index",
                    "--binary",
...
pub(super) fn repository_root(workspace: &Path) -> Option<PathBuf> {
    let output = halo_core::util::process_manager::create_command("git")
        .args(["rev-parse", "--show-toplevel"])
        .current_dir(workspace)
        .output()
        .ok()?;
...
pub(super) fn git_diff_base(workspace: &Path) -> Option<String> {
    let head = halo_core::util::process_manager::create_command("git")
    ...
    let empty_tree = halo_core::util::process_manager::create_command("git")
