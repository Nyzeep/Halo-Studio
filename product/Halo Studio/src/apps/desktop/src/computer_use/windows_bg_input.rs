...
/// value defaults to left). Surfaces a `HaloError::Service` on
/// `PostMessageW` failure or a UIPI block.
fn post_click(root: HWND, x: i32, y: i32, button: &str, click_count: usize) -> HaloResult<()> {
    if root.is_invalid() {
        return Err(HaloError::service("post_click: invalid HWND"));
    }
    ...
    if let Some(uipi) = post_message_blocked_by_uipi(target, down_msg) {
        return Err(HaloError::service(uipi));
    }
...
fn post_key(hwnd: HWND, vk: u16, scan: u32, down: bool) -> HaloResult<()> {
    if hwnd.is_invalid() {
        return Err(HaloError::service("post_key: invalid HWND"));
    }
...
fn post_char(hwnd: HWND, ch: char) -> HaloResult<()> {
    if hwnd.is_invalid() {
        return Err(HaloError::service("post_char: invalid HWND"));
    }
...
pub(super) fn inject_text_cloaked(hwnd: HWND, text: &str) -> HaloResult<()> {
    if hwnd.is_invalid() {
        return Err(HaloError::service("inject_text_cloaked: invalid HWND"));
    }
    if let Some(uipi) = post_message_blocked_by_uipi(hwnd, WM_CHAR) {
        return Err(HaloError::service(uipi));
    }
...
pub(super) fn inject_key_cloaked(hwnd: HWND, keycode: u16, modifiers: &[u16]) -> HaloResult<()> {
    if hwnd.is_invalid() {
        return Err(HaloError::service("inject_key_cloaked: invalid HWND"));
    }
    if let Some(uipi) = post_message_blocked_by_uipi(hwnd, WM_KEYDOWN) {
        return Err(HaloError::service(uipi));
    }
...
/// `HaloError`. Logged at `error` on failure.
fn post_msg(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> HaloResult<()> {
    unsafe {
        match PostMessageW(Some(hwnd), msg, wparam, lparam) {
            Ok(()) => Ok(()),
            Err(e) => {
                let name = message_name(msg);
                let err = HaloError::service(format!(
...
unsafe fn send_unicode(text: &str) -> HaloResult<()> {
...
unsafe fn send_key_combo(keycode: u16, modifiers: &[u16]) -> HaloResult<()> {
...
fn send_key_combo_posted(hwnd: HWND, keycode: u16, modifiers: &[u16]) -> HaloResult<()> {
    for &m in modifiers {
...
pub(super) fn post_click_screen(
    ...
    if root.is_invalid() {
        return Err(HaloError::service("post_click_screen: invalid HWND"));
    }
...
pub(super) fn post_scroll_screen(
    ...
    if root.is_invalid() {
        return Err(HaloError::service("post_scroll_screen: invalid HWND"));
    }
    let target = deepest_child(root, sx, sy);
...
pub(super) fn post_drag_screen(
    ...
    if root.is_invalid() {
        return Err(HaloError::service("post_drag_screen: invalid HWND"));
    }
...
fn vk_for_key(key: &str) -> HaloResult<u16> {
    let vk: u16 = match key.to_lowercase().as_str() {
        ...
        _ => {
            ...
            let ch = key
                .chars()
                .next()
                .ok_or_else(|| HaloError::tool("empty key name".to_string()))?;
            let scan = unsafe { VkKeyScanW(ch as u16) };
            if scan == -1 {
                return Err(HaloError::tool(format!("unknown key: {key}")));
            }
            (scan & 0xFF) as u16
        }
    };
...
pub(super) fn parse_key_chord(keys: &[String]) -> HaloResult<(Vec<u16>, u16)> {
    if keys.is_empty() {
        return Err(HaloError::tool("empty key chord".to_string()));
    }
