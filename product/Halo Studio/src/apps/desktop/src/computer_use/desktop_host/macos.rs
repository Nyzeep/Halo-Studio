...
/// a `HaloError` we can return to the caller. The closure must itself
/// return a `HaloResult<T>` so we can flatten the two error sources
/// (ObjC exception + Rust-side error) into one.
pub(super) fn run_on_main_for_enigo<F, T>(f: F) -> HaloResult<T>
where
    F: FnOnce() -> HaloResult<T> + Send,
    T: Send,
{
...
pub(super) fn catch_objc<F, T>(f: F) -> HaloResult<T>
where
    F: FnOnce() -> HaloResult<T> + Send,
    T: Send,
{
...
/// screenshot buffer).
pub(super) fn catch_objc_local<F, T>(f: F) -> HaloResult<T>
where
    F: FnOnce() -> HaloResult<T>,
{
    catch_only(f)
}

fn catch_only<F, T>(f: F) -> HaloResult<T>
where
    F: FnOnce() -> HaloResult<T>,
{
    ...
    match objc2::exception::catch(AssertUnwindSafe(f)) {
        Ok(inner) => inner,
        Err(Some(exc)) => Err(HaloError::tool(format!("Objective-C exception: {}", exc))),
        Err(None) => Err(HaloError::tool("Objective-C exception (nil)".to_string())),
    }
}
...
pub(super) fn quartz_mouse_location() -> HaloResult<(f64, f64)> {
    unsafe {
...
/// looks like "nothing here" rather than "Halo lacks permission". Fail
...
pub(super) fn require_ax_trust_for(retry_hint: &str) -> HaloResult<()> {
    if ax_trusted() {
        return Ok(());
    }
    request_ax_prompt();
    Err(HaloError::tool(format!(
        "[PERMISSION_DENIED] macOS Accessibility permission not granted to Halo. \
         The system has been asked to surface the permission dialog (System Settings → \
         Privacy & Security → Accessibility → enable Halo). {}",
        retry_hint
    )))
}
