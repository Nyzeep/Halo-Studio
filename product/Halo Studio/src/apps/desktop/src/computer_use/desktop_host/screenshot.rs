...
impl MacPointerGeo {
    ...
    fn full_pixel_center_to_global_f64(&self, cx: f64, cy: f64) -> HaloResult<(f64, f64)> {
...
impl PointerMap {
    ...
    pub(super) fn map_image_to_global_f64(&self, x: i32, y: i32) -> HaloResult<(f64, f64)> {
        if self.image_w == 0
            || self.image_h == 0
            || self.content_w == 0
            || self.content_h == 0
    ...
    pub(super) fn map_normalized_to_global_f64(&self, x: i32, y: i32) -> HaloResult<(f64, f64)> {
...
fn to_workspace_rgba(captured: screenshots::image::RgbaImage) -> HaloResult<image::RgbaImage> {
    let (w, h) = captured.dimensions();
    image::RgbaImage::from_raw(w, h, captured.into_raw())
        .ok_or_else(|| HaloError::tool("Screenshot buffer conversion failed".to_string()))
}

impl DesktopComputerUseHost {
    fn encode_jpeg(rgb: &RgbImage, quality: u8) -> HaloResult<Vec<u8>> {
        let mut buf = Vec::new();
        ...
        .map_err(|e| HaloError::tool(format!("JPEG encode: {}", e)))?;
        Ok(buf)
    }
    ...
    fn ocr_full_primary_display_region() -> HaloResult<OcrRegionNative> {
        let screen = Screen::from_point(0, 0)
            .map_err(|e| HaloError::tool(format!("Screen capture init (OCR raw): {}", e)))?;
    ...
    fn screenshot_raw_native_region(region: OcrRegionNative) -> HaloResult<ComputerScreenshot> {
        ...
        let rgba = screen
            .capture()
            .map_err(|e| HaloError::tool(format!("Screenshot failed (OCR raw): {}", e)))
    ...
    fn crop_rgb(src: &RgbImage, x0: u32, y0: u32, w: u32, h: u32) -> HaloResult<RgbImage> {
        let (sw, sh) = src.dimensions();
        if x0.saturating_add(w) > sw || y0.saturating_add(h) > sh {
            return Err(HaloError::tool("Tile crop out of bounds.".to_string()));
        }
    ...
    ) -> HaloResult<(ComputerScreenshot, PointerMap, Option<ComputerUseNavFocus>)> {
    ...
    fn resolve_screenshot_capture(
    ...
    ) -> HaloResult<(image::RgbaImage, Screen)> {
        ...
        let screen = if let Some(id) = preferred_display_id {
            Self::find_screen_by_id(id)
                ...
                .ok_or_else(|| {
                    HaloError::tool("Screen capture init: no display available".to_string())
                })?
        } else {
            Screen::from_point(mx, my)
                .or_else(|_| Screen::from_point(0, 0))
                .map_err(|e| HaloError::tool(format!("Screen capture init: {}", e)))?
        };
        let rgba = screen
            .capture()
            .map_err(|e| {
                HaloError::tool(format!(
                    "Screenshot failed (on macOS grant Screen Recording for Halo): {}",
                    e
                ))
            })
            .and_then(to_workspace_rgba)?;
...
impl DesktopComputerUseHost {
    ...
    pub(super) async fn screenshot_for_foreground_window(
        ...
        let img = image::load_from_memory(&cap.png)
            .map_err(|e| HaloError::tool(format!("decode window capture PNG: {}", e)))?;
        let rgb = img.to_rgb8();
...
impl DesktopComputerUseHost {
    ...
    pub(super) async fn screenshot_peek_full_display_impl(
        ...
        let (shot, _map, _) = tokio::task::spawn_blocking(move || {
            ...
            let rgba = screen
                .capture()
                .map_err(|e| HaloError::tool(format!("Screenshot failed (peek): {}", e)))
                .and_then(to_workspace_rgba)?;
            Self::screenshot_sync_tool_with_capture(
                ComputerUseScreenshotParams::default(),
                None,
    ...
    pub(super) async fn ocr_find_text_matches_impl(
        ...
        Ok(desktop_matches
            .into_iter()
            .map(
                |m| halo_core::agentic::tools::computer_use_host::OcrTextMatch {
                    text: m.text,
