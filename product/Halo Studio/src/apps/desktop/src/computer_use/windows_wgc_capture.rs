...
pub(super) fn capture_window_bgra(hwnd: HWND) -> HaloResult<(Vec<u8>, u32, u32)> {
    if hwnd.is_invalid() {
    ...
    unsafe {
        ...
        let interop = windows::core::factory::<GraphicsCaptureItem, IGraphicsCaptureItemInterop>()
            .map_err(|e| HaloError::service(format!("WGC factory: {e}")))?;
        let item: GraphicsCaptureItem = interop
            .CreateForWindow(hwnd)
            .map_err(|e| HaloError::service(format!("WGC CreateForWindow: {e}")))?;

        let size = item
            .Size()
            .map_err(|e| HaloError::service(format!("WGC item Size: {e}")))?;
        ...
        .map_err(|e| HaloError::service(format!("WGC CreateFreeThreaded: {e}")))?;

        let session = frame_pool
            .CreateCaptureSession(&item)
            .map_err(|e| HaloError::service(format!("WGC CreateCaptureSession: {e}")))?;
        session
            .StartCapture()
            .map_err(|e| HaloError::service(format!("WGC StartCapture: {e}")))?;
        ...
        let result = loop {
            ...
            if Instant::now() >= deadline {
                break Err(HaloError::service(format!(
...
unsafe fn create_d3d11_device() -> HaloResult<(ID3D11Device, ID3D11DeviceContext)> {
    ...
    .is_err()
    {
        ...
        .map_err(|e| HaloError::service(format!("D3D11CreateDevice (WARP): {e}")))?;
    }

    let device = device.ok_or_else(|| {
        HaloError::service("D3D11CreateDevice returned null device".to_string())
    })?;
    let context = context.ok_or_else(|| {
        HaloError::service("D3D11CreateDevice returned null context".to_string())
    })?;
    Ok((device, context))
}

unsafe fn create_winrt_d3d_device(d3d_device: &ID3D11Device) -> HaloResult<IDirect3DDevice> {
    let dxgi_device: IDXGIDevice = d3d_device
        .cast()
        .map_err(|e| HaloError::service(format!("IDXGIDevice cast: {e}")))?;
    ...
    let inspectable = unsafe { CreateDirect3D11DeviceFromDXGIDevice(&dxgi_device) }
        .map_err(|e| HaloError::service(format!("CreateDirect3D11DeviceFromDXGIDevice: {e}")))?;
    inspectable
        .cast()
        .map_err(|e| HaloError::service(format!("IDirect3DDevice cast: {e}")))
}
...
unsafe fn copy_frame_to_bgra(
...
) -> HaloResult<(Vec<u8>, u32, u32)> {
    let surface = frame
        .Surface()
        .map_err(|e| HaloError::service(format!("WGC frame Surface: {e}")))?;
    let access: IDirect3DDxgiInterfaceAccess = surface
        .cast()
        .map_err(|e| HaloError::service(format!("IDirect3DDxgiInterfaceAccess cast: {e}")))?;
    ...
    let src_texture: ID3D11Texture2D = unsafe { access.GetInterface::<ID3D11Texture2D>() }
        .map_err(|e| HaloError::service(format!("GetInterface ID3D11Texture2D: {e}")))?;
    ...
    unsafe { d3d_device.CreateTexture2D(&staging_desc, None, Some(&mut staging)) }
        .map_err(|e| HaloError::service(format!("CreateTexture2D staging: {e}")))?;
    let staging = staging.ok_or_else(|| {
        HaloError::service("CreateTexture2D returned null staging texture".to_string())
    })?;
    ...
    unsafe { d3d_context.Map(&staging, 0, D3D11_MAP_READ, 0, Some(&mut mapped)) }
        .map_err(|e| HaloError::service(format!("Map staging texture: {e}")))?;
