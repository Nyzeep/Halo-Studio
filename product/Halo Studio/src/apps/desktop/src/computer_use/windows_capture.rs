...
fn encode_bgra_to_png(bgra: &[u8], width: u32, height: u32) -> HaloResult<Vec<u8>> {
    ...
    DynamicImage::ImageRgba8(buf)
        ...
        .map_err(|e| HaloError::io(format!("PNG encode failed: {e}")))?;
    Ok(out)
}
...
fn screenshot_window_via_wgc(hwnd: HWND) -> HaloResult<(Vec<u8>, u32, u32)> {
...
unsafe fn screenshot_via_screen_region(hwnd: HWND) -> HaloResult<(Vec<u8>, i32, i32)> {
    let mut rect = RECT::default();
    ...
    unsafe { GetWindowRect(hwnd, &mut rect) }.map_err(|e| {
        HaloError::service(format!("screen-region fallback: GetWindowRect failed: {e}"))
    })?;
...
pub(super) fn screenshot_window_capture(hwnd: HWND) -> HaloResult<WindowCapture> {
    unsafe { screenshot_window_bytes_unsafe(hwnd) }
}

unsafe fn screenshot_window_bytes_unsafe(hwnd: HWND) -> HaloResult<WindowCapture> {
    if hwnd.is_invalid() {
        return Err(HaloError::service(
