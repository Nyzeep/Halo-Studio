...
//!
//! Unlike the cua daemon, Halo is a Tauri GUI app, so COM is initialized with
...
impl UiaNode {
    /// Convert to Halo's [`AxNode`] for `get_app_state` integration.
    ///
...
unsafe fn build_cache_request(
...
) -> HaloResult<IUIAutomationCacheRequest> {
    ...
    let cache_req = unsafe { automation.CreateCacheRequest() }
        .map_err(|e| HaloError::tool(format!("UI Automation CreateCacheRequest: {}.", e)))?;
...
) -> HaloResult<IUIAutomationElement> {
...
unsafe fn walk_tree_full(
    ...
    let uncached = unsafe { automation.ElementFromHandle(hwnd) }.map_err(|e| {
        HaloError::tool(format!("UI Automation ElementFromHandle failed: {}.", e))
    })?;
...
unsafe fn walk_cached_bounded(
    ...
    *total += 1;
    ...
    if is_actionable || has_content {
        ...
        // for actionable nodes; Halo's `locate_ui_element_center` needs it).
...
pub(super) fn accessibility_hit_at_global_point(
    ...
    let automation: IUIAutomation = unsafe {
        CoCreateInstance(&CUIAutomation, None, CLSCTX_INPROC_SERVER)
            .map_err(|e| HaloError::tool(format!("UI Automation (CoCreateInstance): {}.", e)))?
    };
    let pt = POINT {
        x: gx.round() as i32,
        y: gy.round() as i32,
