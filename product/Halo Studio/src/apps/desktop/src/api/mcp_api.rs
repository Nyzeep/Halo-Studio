...
use halo_core::service::mcp::auth::{
    has_stored_oauth_credentials, MCPRemoteOAuthSessionSnapshot,
};
use halo_core::service::mcp::config::MCPConfigService;
use halo_core::service::mcp::protocol::{
    MCPPrompt, MCPResource, PromptsGetResult, ResourcesReadResult,
};
use halo_core::service::mcp::MCPServerType;
use halo_core::service::runtime::{RuntimeManager, RuntimeSource};
...
async fn load_mcp_resources(
    mcp_service: &halo_core::service::mcp::MCPService,
    server_id: &str,
    refresh: bool,
...
async fn ensure_unscoped_host_mcp_access(
    manager: &halo_core::service::mcp::MCPServerManager,
    server_id: &str,
) -> Result<(), String> {
    manager
...
pub async fn get_mcp_servers(state: State<'_, AppState>) -> Result<Vec<MCPServerInfo>, String> {
    ...
    for config in configs {
        ...
        let (command, command_available, command_source, command_resolved_path) =
            if transport == halo_core::service::mcp::MCPServerTransport::Stdio {
...
pub async fn load_mcp_json_config(
    state: State<'_, AppState>,
) -> Result<halo_core::service::mcp::config::MCPJsonConfigSnapshot, String> {
    let mcp_service = state
        .mcp_service
        .as_ref()
        .ok_or_else(|| "MCP service not initialized".to_string())?;
...
pub async fn get_mcp_tool_ui_uri(
    ...
    let registry = halo_core::agentic::tools::registry::get_global_tool_registry();
