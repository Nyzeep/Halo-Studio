...
//!
//! Lets a user deploy the open-source Halo relay server to their own host
...
//!
//! Orchestration: `halo_services_integrations::remote_ssh::relay_deploy`.
//! Product invariants / wizard entry points:
//! `src/web-ui/src/features/relay-deploy/README.md`.

use halo_core::service::remote_ssh::relay_deploy::{
...
pub async fn relay_deploy_register(
    ...
    let account = halo_relay_service::admin::provision(&username, &password)
        .map_err(|e| format!("provision account: {e}"))?;
    let import = halo_relay_service::admin::ImportableAccount { username, account };
