...
fn desktop_product_name() -> &'static str {
...
impl ThemeConfig {
    ...
    fn startup_messages_json(locale: &str) -> String {
        let product_name = desktop_product_name();
        let messages = match locale {
            "en-US" | "en" => serde_json::json!({
                "loadingApp": format!("Starting {product_name}..."),
                "minimize": "Minimize",
                "maximize": "Maximize",
                "close": "Close",
            ...
            "zh-TW" | "zh-Hant-TW" => serde_json::json!({
                "loadingApp": format!("正在啟動 {product_name}..."),
                "minimize": "最小化",
                "maximize": "最大化",
                "close": "關閉",
                "petLoading": "正在載入助手..."
            ...
            _ => serde_json::json!({
                "loadingApp": format!("正在启动 {product_name}..."),
                "minimize": "最小化",
                "maximize": "最大化",
                "close": "关闭",
                "petLoading": "正在加载助手..."
...
pub async fn show_agent_companion_desktop_pet(app: tauri::AppHandle) -> Result<(), String> {
    if cfg!(feature = "halo-local-coding") {
        debug!("Agent companion window is disabled in Halo local-coding scope");
        return Ok(());
    }
