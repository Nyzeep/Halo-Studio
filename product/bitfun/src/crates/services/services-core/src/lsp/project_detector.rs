...
#[cfg(test)]
mod tests {
    use super::ProjectDetector;
    use tokio::fs;

    #[tokio::test]
    async fn detects_rust_workspace_from_manifest_and_sources() {
        ...
        let info = ProjectDetector::detect(root).await.expect("detect project");

        assert!(info.languages.contains(&"rust".to_string()));
        assert!(info.project_types.contains(&"rust".to_string()));
    }

    #[test]
    fn prestarts_all_languages_for_small_projects() {
        ...
        assert_eq!(
            ProjectDetector::should_prestart(&info),
