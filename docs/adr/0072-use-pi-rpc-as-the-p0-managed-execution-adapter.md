...
- Pi extension UI 通过 `extension_ui_request`/`extension_ui_response` 工作；P0 只加载 Halo 第一方、固定版本、来源和许可证已审计的 extension，并用 `--no-extensions` 加显式 `-e` 路径阻止发现其他 extension。项目本地和用户全局 extension、Pi package、任意 Provider extension 不进入受管路径。
