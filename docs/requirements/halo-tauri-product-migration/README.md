# Halo Studio/Tauri 产品迁移

**Status:** ready-for-agent

> 历史记录/上游对照（已归档）：本目录记录 Halo Studio 从上游（原 BitFun）迁移的规格、工单与证据；其中出现的上游名称仅作历史对照。

本迁移把 Halo Studio 建立为上游（原 BitFun）的受控下游产品，并将已验证的受管交付行为迁入 Halo 品牌的 Tauri 工作台。

- [迁移规格](00-halo-tauri-product-migration-spec.md)
- [实施工单](issues/)

## 当前检查点

- 工单 01–03A1 已建立可迁移能力基线、受跟踪的上游（原 BitFun）产品树、Halo Tauri 入口和正式 Halo Studio Web UI；UI 对齐不等于模型或 Agent 执行链已经接入。
