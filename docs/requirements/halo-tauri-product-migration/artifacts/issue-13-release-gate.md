...
- 候选参考树：`<HALO_HALO_REFERENCE_ROOT>`（只读 evidence locator
  `readonly-evidence://halo-latest`），`main` 与 `origin/main` 均为
...
| `cargo tree --manifest-path "product/Halo Studio/Cargo.toml" -p halo-pi-rpc-adapter` | `PASS` | 0 | 依赖树可解析；workspace member 重复仍由审计 gate 单独阻断 |
...
| `cargo test --manifest-path "product/Halo Studio/Cargo.toml" -p halo-pi-rpc-adapter` | `PASS` | 0 | Serial rerun passed 17/17 (exit 0); a parallel invocation transiently reported 16/17 on `unknown_event` as `Transport` instead of `Protocol`, with no source change. |
...
| `$env:HALO_HALO_REFERENCE_ROOT='<matching read-only checkout for readonly-evidence://halo-latest>'; node "product/Halo Studio/scripts/pi-extension-audit.mjs" --json` | `BLOCKED` | 1 | Matching locator produced 12 findings: `release-gate-declared-blocked`, `upstream-candidate-release-gate-blocked`, `upstream-history-boundary-untrusted`, `upstream-base-commit-unresolved`, `upstream-ancestry-unproven`, `upstream-initial-import-tree-mismatch`, `workspace-member-duplicate`, `built-in-extension-provenance-missing`, `host-source-provenance-missing`, `host-license-evidence-not-release`, `host-dependency-closure-incomplete`, and `release-artifact-evidence-missing`. |
| `Remove-Item Env:HALO_HALO_REFERENCE_ROOT; node "product/Halo Studio/scripts/pi-extension-audit.mjs" --json` | `BLOCKED` | 1 | Missing locator is reported as `upstream-reference-tree-unavailable`; it is not interpreted as candidate provenance or a pass. |
...
| Base-tree Workbench Runtime Rust contracts: `cargo test --manifest-path "product/Halo Studio/Cargo.toml" -p halo-agent-runtime --test workbench_runtime_contracts` | `PASS` | 0 | 16/16 passed on the existing Halo base tree; this is not candidate validation. |
...
- 冲突决策：不自动 merge、不 cherry-pick、不向 Halo Studio 上游提交或推送；保留 Halo
...
- 依赖边界：`@earendil-works/pi-coding-agent` 仅出现在 extension 源码第 1 行的
  ...
  Cargo.toml/Cargo.lock 中无命中（退出 1）；`cargo tree -p halo-pi-rpc-adapter`
  无可疑运行时依赖。
...
`node "product/Halo Studio/scripts/pi-extension-audit.mjs" --json`（`HALO_HALO_REFERENCE_ROOT=<matching read-only checkout for readonly-evidence://halo-latest>`）。
...
- Halo Studio 上游候选 `9b05dd0e0e751c9e6e83fae3e9a0307bcd79b6b6`：
...
| `cargo metadata --no-deps` / `cargo tree -p halo-pi-rpc-adapter`（--offline） | 0 | workspace 解析正常，member 唯一 |
| `cargo test -p halo-pi-rpc-adapter extension_decision_is_redacted_one_shot_and_duplicate_request_fails_closed` | 0 | 1 passed |
...
| `cargo test -p halo-agent-runtime --test workbench_runtime_contracts` | 0 | 46 passed |
| `cargo tree -p halo-pi-rpc-adapter`（--offline） | 0 | 正常；无 `@earendil-works/pi-coding-agent` 运行时依赖 |
| `cargo test -p halo-pi-rpc-adapter extension_decision_is_redacted_one_shot_and_duplicate_request_fails_closed` | 0 | 1 passed |
| `rg -F '@earendil-works/pi-coding-agent'` 四个 lockfile/manifest | 1 | 无命中（期望） |
| `git diff --check` | 0 | passed |
