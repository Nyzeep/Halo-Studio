...
**What to build:** 在 Halo Studio Desktop/Tauri seam 建立一个深的 Halo Workbench Runtime Module。Halo 前端只通过它的小型 Interface 读取工作区、运行时、会话、权限请求和结构化事件，并提交有限意图；Module 的实现封装 Pi RPC Adapter、工作区信任、配置、任务、证据和生命周期。工单 04 先固定 Interface、状态所有权和可替换测试 Adapter，不在本票启动真实 Pi 模型回合或读取真实凭据。
...
- 在 Module 内定义 P0 专用的 Pi RPC 执行 Interface；生产 Adapter 身份固定为 `pi-rpc-p0`。测试替身只验证同一 Interface，不得演化成通用多执行器分派或生产回退。
...
- 复用 Halo Studio 已有 Agent Runtime owner 与 Tauri 装配点；应用 deletion test，删除新 Module 后复杂度应重新扩散到多个调用方，而不是只少一层转发。
...
- 在 `product/Halo Studio/src/apps/desktop/src/lib.rs` 的 Tauri 装配点注册少量薄的 Halo command/event adapter；不要让前端从现有数百个 Halo Studio command 自行拼装 Halo 状态机。
- 正式前端改动只进入 `product/Halo Studio/src/web-ui` 的 Halo 产品路径；不得恢复 `src/halo-workbench` 静态生产入口。
- Halo Studio ACP、旧 `sidecar/crates/halo-runtime/src/opencode.rs` 和任何旧 HTTP/SSE 适配只作为历史可迁移语义参考，不得被产品树直接依赖或冒充 Pi 执行链。
...
- [ ] 前端只依赖该 Module 的 Interface，不直接调用 Pi RPC Adapter、旧 Sidecar、`src/halo-workbench` 静态壳或多组相互重叠的 Halo Studio command。
- [ ] `halo-local-coding` 正式入口不把 Halo Studio 内置模型执行、ACP 会话或历史 OpenCode Adapter 作为并行权威；可复用 Halo Studio UI/基础设施，但 Halo P0 会话事实只来自 Workbench Runtime 的 Pi RPC 执行链。
...
最后的残留扫描只允许命中明确标注为历史/比较材料的文档；若命中活动 P0 生产代码、装配、Tauri/Web runtime 或未标记的当前规格，验收失败。

## 不在本票

- 不启动真实 `pi --mode rpc` 模型回合，不录入真实凭据，不发送真实模型请求。
- 不复制 `D:\pi-main`、不依赖 `D:\opencode-dev`，旧 OpenCode/Sidecar 文件只作为历史行为参考。
- 不实现第一方 extension 的具体工具策略、Halo Studio 内置 Code Agent、多执行器选择或旧 JSONL 兼容层。
