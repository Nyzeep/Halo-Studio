# Halo Studio/Tauri 产品基座迁移规格

**Status:** ready-for-agent

## Problem Statement

Halo Studio 已把受跟踪的 Halo Studio 下游源码、Halo Tauri 入口和正式 Halo Studio Web UI 纳入产品树；工单 03/03A1 的构建与原生 UI 证据证明界面基座已经对齐，但当前桌面运行链尚未通过新的 Halo Workbench Runtime 接入本机 Pi Agent。旧 PySide6/QML、Rust Sidecar 和 OpenCode 方案只形成可迁移能力或历史决策基线，不能替代目标 Tauri 产品上的 Pi RPC 运行时迁移和真实 UI 验收。

项目需要在保留既有行为证据的同时，将 Halo Studio 建立为持续获取 Halo Studio 上游更新、但独立提交和发布的下游产品。迁移必须避免长期双运行时、外部目录构建依赖、旧 IPC 兼容层和过早删除旧实现。

## Solution

Halo Studio 在受跟踪的 Halo 产品树中纳入完整 Halo Studio 下游源码，建立 Halo 品牌的 Tauri 桌面入口，并只装配本地桌面编码主链。Halo Studio 上游只提供显式同步候选；所有 Halo 改动、验证和发布都发生在 Halo 自己的仓库中。
...
2. 作为本地开发者，我希望保留 Halo Studio 成熟的三栏工作台和高密度交互，以便迁移后仍拥有完整的编码工作流。
...
26. 作为维护者，我希望 Halo 仓库跟踪完整 Halo Studio 源码关系，以便持续同步时减少遗漏和重复冲突。
27. 作为维护者，我希望 Halo Studio 上游远端只用于获取更新，以便 Halo 提交不会意外推送到上游。
28. 作为维护者，我希望每次上游同步都记录精确来源 commit，以便变更、许可证和安全修复可审计。
29. 作为维护者，我希望上游更新通过独立候选和完整验证门槛，以便未经验证的上游变化不会直接进入 Halo 产品。
30. 作为维护者，我希望范围外 Halo Studio 模块不进入构建、路由、导航和初始化，以便保留源码关系而不扩大产品范围。
31. 作为维护者，我希望保留 Halo Studio MIT 许可证和第三方归属，以便源码与发行包满足许可证义务。
...
- Halo Studio 是 Halo Studio 的受控下游产品：上游只拉取，Halo 的提交、远端和发布完全独立。
- 完整 Halo Studio 源码关系进入受跟踪的 Halo 产品树；外部上游参考树只用于获取和检查变更，正式构建不依赖外部绝对路径。
- Halo 产品树是长期产品源码边界，不在迁移完成后再次搬回仓库根目录。
- 范围外 Halo Studio 模块可以保留源码，但不得进入 Halo 首期构建、路由、导航、后台初始化、配置入口或隐藏开关。
- Halo 保留 Halo Studio 工作台交互骨架，并用 Halo 名称、图标、视觉令牌、简体中文文案和产品范围重新表达。
...
- P0 唯一生产受管执行 Adapter 是本机已安装的 Pi RPC；历史 OpenCode Server、Halo Studio 内置 Code Agent 和多执行器交接不进入当前 UI 选择器或发布矩阵。
...
- 上游同步测试至少演练一次新的 Halo Studio commit 候选，证明来源记录、冲突处理、产品裁剪和回归门槛可执行。
- 许可证测试验证 Halo Studio MIT 归属、来源记录和第三方声明进入源码与发行包。
...
- 向 Halo Studio 上游仓库直接提交、推送或发布 Halo 改动。
- 自动无审查地合并 Halo Studio 上游更新。
- 保持旧 Python/QML UI、旧 Sidecar 进程或 `stdio JSONL v1` 的向后兼容。
- 在首期启用办公协作、Mini App、远程、Relay、移动端或其他范围外 Halo Studio 产品模块。
...
- 在 P0 中实现 Pi TUI、Unix/CBOR PiServer、Halo Studio 内置 Code Agent 或多执行器选择/交接。
