...
- extension 清单必须覆盖加载来源、固定版本/hash、依赖、宿主权限、工具影响、许可证和更新责任；清单缺失或审计未完成时，候选不能成为发布版本，也不能把任意路径加入显式 `-e` 加载清单。
...
- [ ] Pi 第一方 extension inventory 记录源文件、来源 commit/tag、固定版本或内容 hash、加载参数、可访问工具/事件、宿主权限、网络/文件/进程影响、直接/传递依赖及许可证；未审计 extension 不得进入 `-e` 加载清单。
...
| 当前门槛 | 没有来源 commit、hash、依赖清单、宿主权限说明和逐项许可证证据时，状态为 `not-a-release-gate`，不能把 `-e` 视为放行证据。 |
...
cargo test --manifest-path "product/Halo Studio/Cargo.toml" -p bitfun-pi-rpc-adapter identity_and_first_party_extension_are_stable
