...
- [x] 将迁移前的 OpenCode 原生 UI 记录为历史比较材料，不再作为当前 P0 门槛；当前目标 Tauri 的真实 Pi RPC 原生 UI 验收仍由工单 14 负责，并保留此前失败的归类与复现证据。
