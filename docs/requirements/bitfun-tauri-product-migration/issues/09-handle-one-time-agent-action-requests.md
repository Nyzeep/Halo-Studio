...
- [ ] 每个请求绑定当前 Halo task scope（在 04 seam 中由当前 Workbench session 与 adapter generation 共同表示）、一次 turn 和一个脱敏 `toolCallId`；决议只能“本次允许”或“拒绝”，不得形成永久、会话级或跨任务授权。原始 Pi session/entry/toolCall ID 只留在 Adapter 内。
...
- [ ] extension 安装使用 `--no-extensions` 加显式 `--extension`，只加载工单 13 审计通过的 Halo 第一方版本；项目/全局 extension、Pi package 和 Provider extension 不得参与 P0。
