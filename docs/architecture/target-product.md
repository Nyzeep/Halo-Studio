...
Halo Studio 是 Halo Studio 的受控下游产品。仓库跟踪完整的 Halo Studio 源码关系并持续获取上游更新，但 Halo 的变更、验证、提交和发布只发生在自己的产品仓库中。

## 产品边界

- `product/` 是长期受跟踪的 Halo 产品树，也是迁移完成后的唯一桌面构建入口。
- 桌面应用使用 Halo 品牌的 Tauri 工作台，并保留 Halo Studio 成熟的本地编码交互骨架。
- 首期只装配本地编码主链；办公协作、Mini App、远程、Relay 和移动端能力不进入构建、路由、导航或初始化。
- 仓库外的上游（原 BitFun）参考树只用于获取和检查候选更新，正式构建不得依赖其绝对路径。

## 运行时边界

Halo Studio 提供产品基座和工作台能力；Halo Workbench Runtime Module 位于 Tauri 接缝，是 Halo 工作区信任、编码会话投影、受管任务、权限决议、Git、配置、凭据引用、脱敏、证据和结构化事件的唯一权威。它向前端提供小而稳定的 command/event 接口。
...
- **标准编码模式**沿用 Halo Studio 原生会话、工具、历史与 Git 能力，不产生 Halo 受管交付结论。
...
具体决策以 ADR-0065 至 ADR-0072 为入口，实施顺序以 Halo Studio/Tauri 迁移规格和工单为准；新的实现入口是 03B → 04。
