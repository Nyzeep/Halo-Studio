...
本文件定义每个模块的**职责边界、公共 API 形状与文件所有权**。并行开发时：只允许修改自己所有的路径；跨模块调用一律以本文件与 [ipc-protocol.md](ipc-protocol.md) 为准。集成阶段允许 `halo-sidecar` 集成者做**签名级适配**（不得改变语义）。
...
pub enum TaskState { Created, Running, AwaitingAction, Finishing, ReviewReady, Accepted, Rejected, Cancelled, Failed, Interrupted }
pub enum TaskEvent { Started, ActionRequested, ActionResolved, Finishing, EvidenceReady, Accept, Reject, CancelledNative, CancelledForced, Fail(String), MarkInterrupted }
